...
{
    /* 3 6 9 12 indicates the position of the colon ':' */
    if ((*(for_check + i) == ':') &&
        (*(for_check + i + 3) == ':') &&
        (*(for_check + i + 6) == ':') &&
        (*(for_check + i + 9) == ':') &&
...
void change_mac(char* mac_input)
{
    ...
    /* start to traverse strings, find MAC address
     * 2: first colon ':'
     */
    size_t i = 2;
    ...
    while (*(mac_input + i + 14) != '\0') {
        ...
        if (symbol_logical(mac_input, i)) {
             ...
             * -2: Moves forward by two digits to point to the start position of mac_buf.
             * 15: last colon ':'
             * 3 : Traversing the position of ':'
             */
            int j = -2;
            while (j != 15) {
                if (j % 3 == 0) {
        ...
        if (!mac_address) {
            ...
            int m = 1;
            /* 9 indicates that the first "i+9" bits of the MAC address are replaced with "*". */
            while (m != 9) {
                if (m % 3 == 0) {
                    ++m;
                    continue;
             ...
             * 'i + 17 + 15': Length of two MAC addresses
             */
            if (i + 17 + 15 > len) {
                break;
            } else {
...
static void log_packet(char* type, void* addr, unsigned char* ext_mac, int mac_len, char* interface,
                       char* string, u32 xid) {
    struct in_addr addr_msg;
    static char mac_buf[OPTION_END] = {0};
    static char ip_buf[OPTION_END] = {0};

    /* addr may be misaligned */
    if (addr) memcpy(&addr_msg, addr, sizeof(addr_msg));

    print_mac(daemon->namebuff, ext_mac, mac_len);

    if (sizeof(mac_buf) < strlen(daemon->namebuff) ||
        sprintf(mac_buf, "%s", daemon->namebuff) < 0) {
        my_syslog(LOG_ERR, "sizeof(mac_buf) < strlen(daemon->namebuff) or mac sprintf fail");
        return;
    }
    change_mac(mac_buf);

    if (sizeof(ip_buf) < strlen(inet_ntoa(addr_msg)) ||
        sprintf(ip_buf, "%s", inet_ntoa(addr_msg)) < 0) {
        my_syslog(LOG_ERR, "sizeof(ip_buf) < strlen(inet_ntoa(addr_msg)) or ip sprintf fail");
        return;
    }
    change_ip(ip_buf);
    ...
    if (daemon->options & OPT_LOG_OPTS) {
        my_syslog(MS_DHCP | LOG_INFO, "%u %s(%s) %s%s%s %s", ntohl(xid), type, interface,
                  addr ? ip_buf : "", addr ? " " : "", mac_buf, string ? string : "");
    } else {
        my_syslog(MS_DHCP | LOG_INFO, "%s(%s) %s%s%s %s", type, interface, addr ? ip_buf : "",
                  addr ? " " : "", mac_buf, string ? string : "");
    }
}
