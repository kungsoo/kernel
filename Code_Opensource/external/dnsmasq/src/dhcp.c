...
void dhcp_packet(time_t now) {
    ...
    /* bind the socket to specific interface which received a UDP packet we want to reply. */
    int mRet = setsockopt(daemon->dhcpfd, SOL_SOCKET, SO_BINDTODEVICE, &ifr, sizeof(ifr));
    if (mRet) {
        my_syslog(LOG_ERR, _("Failed to bind dhcp socket to interface %s, error=%d"), ifr.ifr_name, mRet);
    }

    while (sendmsg(daemon->dhcpfd, &msg, 0) == -1 && retry_send());

    memset(&ifr, 0, sizeof(ifr));
    mRet = snprintf(ifr.ifr_name, sizeof(ifr.ifr_name), "");
    if (mRet == -1) {
        return;
    }
    mRet = setsockopt(daemon->dhcpfd, SOL_SOCKET, SO_BINDTODEVICE, &ifr, sizeof(ifr));
    if (mRet) {
        my_syslog(LOG_ERR, _("Failed to unbind dhcp socket from specific interface, error=%d"), mRet);
    }
}
