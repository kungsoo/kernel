		...
		struct xtc_handle **handle, bool restore)
{
	...
	switch (command) {
	case CMD_APPEND:
		/* if specified rule already existed within mobile_reject_list , wifi_reject_list,
		   bw_penalty_box, bw_happy_box or usb_reject_list, no need to append. */
		if (strcmp(chain, "mobile_reject_list") == 0 || strcmp(chain, "wifi_reject_list") == 0 ||
			strcmp(chain, "bw_penalty_box") == 0 || strcmp(chain, "bw_happy_box") == 0 ||
			strcmp(chain, "usb_reject_list") == 0) {
	...
	case CMD_INSERT:
		/* if specified rule already existed within mobile_reject_list , wifi_reject_list,
		   bw_penalty_box, bw_happy_box or usb_reject_list, no need to insert. */
		if (strcmp(chain, "mobile_reject_list") == 0 || strcmp(chain, "wifi_reject_list") == 0 ||
			strcmp(chain, "bw_penalty_box") == 0 || strcmp(chain, "bw_happy_box") == 0 ||
			strcmp(chain, "usb_reject_list") == 0) {
			ret = check_entry(chain, e,
