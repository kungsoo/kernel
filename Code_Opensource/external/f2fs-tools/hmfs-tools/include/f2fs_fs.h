...
#define HMFS_SUPER_MAGIC	0xF2F52020	/* HMFS Magic Number */
...
#define RESERVED_DEV "RESERVED"

#define HMFS_RSV_SEC	4
...
struct f2fs_configuration {
	u_int32_t original_reserved_segments;
	...
	/*
	 * For mkfs: # of sectors for f2fs, constrained by wanted_total_sectors
	 * For fsck/resize: # of sectors on device(s), equals to max_sectors
	 */
	u_int64_t total_sectors;
	/* # of sectors on device(s) */
	u_int64_t max_sectors;
	...
	/* resize parameters */
	char *device_to_add;
	/* reserve segs for dynamic resize */
	u_int32_t reserved_segs;
#ifdef HAVE_LIBSELINUX
	...
	/* invert segment allocation direction for sload to speed up resize */
	int invertion;

	/* whether fsck has synced curseg from device info */
	int has_curseg_synced;
#ifdef HW_F2FS_IMG_OEM
	/*added for special config for file or dirs*/
	char *target_special_dir;
	char *target_special_file;
#endif

	struct hmfs_dm_set *dm_set;
	int num_dm_blocks;

	int do_gc;

	nid_t cached_ino;
	struct tree_node *tree_node_root;
 ...
 */
#define NR_UNIFY_CURSEG_TYPE	(4)

#define	NR_CURSEG_DATA_TYPE	(3)
#define NR_CURSEG_NODE_TYPE	(3)
#define NR_CURSEG_DM_TYPE	(2)	/* # of data move type */
#define NR_CURSEG_TYPE	(NR_CURSEG_DATA_TYPE + NR_CURSEG_NODE_TYPE + NR_CURSEG_DM_TYPE)
/* From Device:max blocks = superPu * stringPerLayer * （verifyLayerCnt + 1）*/
#define MAX_DM_CACHE_BLOCKS	(200)

#define TLC_MODE                (0)
#define SLC_MODE                (1)
#define SLC_END_SEG_IN_SEC(sbi) ((sbi)->segs_per_sec / 3)
...
enum {
	...
	CURSEG_DATA_MOVE1,	/* data move head 1*/
	CURSEG_DATA_MOVE2,	/* data move head 2*/
	NO_CHECK_TYPE
};

extern const int curseg_dm_type[];
...
enum {
	...
	HOT_NODE_STREAM_ID,
	HOT_DATA_STREAM_ID,
	FS_STREAM_NR,		/* Streams in fs management  */
	HYPER_HOLD_STREAM_ID = FS_STREAM_NR,
	STREAM_NR
};
...
#define MAX_ACTIVE_NODE_LOGS	4
#define MAX_ACTIVE_DM_LOGS	4
...
#define MAX_CACHED_SIZE		(18 * 512 * F2FS_BLKSIZE) /* 18 segments, 36M */

/*
 * For DataMove Opts
 */
#define MAX_DM_CHANNELS 4
#define DM_HEAD_RESV_SIZE (8 +	\
		sizeof(struct hmfs_datamove_info) * MAX_DM_CHANNELS)
#define DM_ENTRIES_IN_HEAD ((PAGE_SIZE - DM_HEAD_RESV_SIZE) / \
			sizeof(struct hmfs_datamove_entry))
#define DM_ENTIRES_IN_BODY (PAGE_SIZE / sizeof(struct hmfs_datamove_entry))

#pragma pack(push, 1)
struct hmfs_datamove_entry {
	__le32 src_blkaddr;
	__le32 dst_blkaddr;
...
/* datamove channel info */
struct hmfs_datamove_info {
	__le32 verified_blkaddr;
	__le32 next_blkaddr;
	__le32 cached_last_blkaddr;
	__le32 reserved;
} __attribute__((packed));

/* 4KB-sized datamove block structure at the head */
struct hmfs_datamove_head {
	__le32 channels;	/* # of datamove channels */
	__le32 count;		/* # of datamove entries */
	struct hmfs_datamove_info dm_info[MAX_DM_CHANNELS];
	struct hmfs_datamove_entry entries[DM_ENTRIES_IN_HEAD];
} __attribute__((packed));

/* 4KB-sized datamove block structure at main body */
struct hmfs_datamove_body {
	struct hmfs_datamove_entry entries[DM_ENTIRES_IN_BODY];
} __attribute__((packed));

struct hmfs_datamove_block {
	union {
		struct hmfs_datamove_head head;
		struct hmfs_datamove_body body;
	};
} __attribute__((packed));
...
#define CP_RESIZEFS_FLAG		0x00010000
#define CP_DATAMOVE_FLAG		0x00008000
...
struct f2fs_checkpoint {
	...
	/* information of current node segments & dmgc segments */
	__le32 cur_node_segno[MAX_ACTIVE_NODE_LOGS];
	__le32 cur_dmgc_segno[MAX_ACTIVE_DM_LOGS];
	__le16 cur_node_blkoff[MAX_ACTIVE_NODE_LOGS];
	__le16 cur_dmgc_blkoff[MAX_ACTIVE_DM_LOGS];
...
/*
 * For datamove operations
 */
#define DM_ENTRY_NUM         (PAGE_SIZE / sizeof(struct f2fs_dm_unverified_entry))

struct f2fs_dm_info {
        __le32 dm_first_info;                           /* num of hmfs_dm_entry in datamove seghead */
        __le32 dm_second_info;                          /* unverified addr of datamove seghead */
} __attribute__((packed));

struct f2fs_dm_unverified_entry {
        __le32 src_blkaddr;
        __le32 dst_blkaddr;
} __attribute__((packed));

struct f2fs_dm_block {
	union {
		struct f2fs_dm_info dm_info;              /* datamove block head */
		struct f2fs_dm_unverified_entry dm_entry;    /* datamove block entry */
        } dm_normal_entry[DM_ENTRY_NUM];
} __attribute__((packed));

struct stor_pwron_info {
	unsigned int dev_stream_addr[STREAM_NR];
	unsigned int dm_stream_addr[NR_CURSEG_DM_TYPE];
	unsigned char io_slc_mode_status;
} __attribute__((packed));

struct stor_dev_section_info {
	block_t section_start_lba;
	int flash_mode;
...
extern void set_bit_le(u32 nr, u8 *addr);
extern int test_and_set_bit_le(u32, u8 *);
extern void clear_bit_le(u32 nr, u8 *addr);
...
extern int f2fs_crc_valid(u_int32_t magic, u_int32_t blk_crc, void *buf, int len);

extern void f2fs_init_configuration(void);
extern int f2fs_devs_are_umounted(void);
extern int f2fs_dev_is_umounted(char *);
extern int f2fs_get_device_info(void);
extern int blk_device_curseg_info_sync(struct stor_pwron_info *);
extern int get_device_info(int);
extern int f2fs_init_sparse_file(void);
extern int hmfs_finalize_device(void);
extern int f2fs_fsync_device(void);
extern int set_section_info(block_t block_addr, int flash_mode, int stream_id);
extern int clear_section_info(block_t block_addr, int stream_id);
extern int set_write_stream_id(int stream_id, block_t block_addr);
extern int get_device_overprovision(block_t block_addr);
extern int device_ftl_reset(void);
extern int fs_sync_done(void);
extern int get_device_segment_mapping_pos(
			struct stor_dev_mapping_partition *mapping_partitions);
extern int set_device_segment_mapping_pos(
			struct stor_dev_mapping_partition *mapping_partitions);

extern int get_device_index(__u64);
extern int dev_read(void *, __u64, size_t);
extern int dev_write(void *, __u64, size_t, int, int, int);
extern int dev_write_block(void *, __u64);
extern int dev_write_block_no_sync(void *, __u64);
...
{
	return (inode->i_inline & HMFS_EXTRA_ATTR);
}
...
static inline int get_inline_xattr_addrs(struct f2fs_inode *inode)
{
	...
	else if (inode->i_inline & HMFS_INLINE_XATTR ||
...
extern block_t get_dm_source_addr(block_t dst_addr);

#define SIZE_ALIGN(val, size)	((val) + (size) - 1) / (size)
#define SEG_ALIGN(blks)		SIZE_ALIGN(blks, c.blks_per_seg)
#define SEC_ALIGN(blks)		SIZE_ALIGN(blks, c.blks_per_seg * \
					c.segs_per_sec)
 ...
 */
static int get_level_interval(unsigned int min_segs,
			unsigned int min_secs, bool max)
{
	int result;
	int sec_count;

	sec_count = (min_segs + c.segs_per_sec - 1) / c.segs_per_sec;
	if (max)
...
/* Reference from the kernel hmfs_adjust_gc_levels */
static int hmfs_get_min_op_sec(unsigned int rsvd_section_count)
{
	int level[ALL_GC_LEVELS];
	 ...
	 */
	level[BG_GC_LEVEL6] = rsvd_section_count;
	level[BG_GC_LEVEL5] = level[BG_GC_LEVEL6] +
		get_level_interval(0, MIN_SECS, 1);
	level[BG_GC_LEVEL4] = level[BG_GC_LEVEL5] + CONCURRENT_SECS +
		get_level_interval(MIN_SEGS, 2, 1);
	level[BG_GC_LEVEL3] = level[BG_GC_LEVEL4] +
		get_level_interval(MIN_SEGS, MIN_SECS, 0);

	return level[BG_GC_LEVEL3];
}
#endif	/*__F2FS_FS_H */
