...
static void update_cur_segs_by_rsvd_dev(void)
{
	u_int32_t cnt1[MAX_DEVICES], cnt2[MAX_DEVICES];
	int i, j;

	cnt1[0] = (c.devices[0].end_blkaddr - get_sb(main_blkaddr) + 1) /
			c.blks_per_seg;
	cnt2[0] = cnt1[0];

	for (i = 1; i < c.ndevs; i++) {
		cnt1[i] = cnt1[i-1] + c.devices[i].total_segments;
		cnt2[i] = cnt2[i-1];
		if (!c.devices[i].reserved)
			cnt2[i] += c.devices[i].total_segments;
	}

	for (j = 0; j < NR_CURSEG_TYPE; j++)
		for (i = 0; i < c.ndevs; i++)
			if (c.cur_seg[j] < cnt2[i]) {
				c.cur_seg[j] += (cnt1[i] - cnt2[i]);
				break;
			}
}
...
static int f2fs_prepare_super_block(void)
{
	...
	u_int64_t segment_align_start_offset;
	u_int64_t total_meta_segments;
	u_int64_t total_meta_blocks;
	u_int32_t sit_bitmap_size, max_sit_bitmap_size;
	u_int32_t max_nat_bitmap_size, max_nat_segments;
	u_int32_t total_zones;
	u_int32_t normal_total_zones;
	enum quota_type qtype;
	int i;

	set_sb(magic, HMFS_SUPER_MAGIC);
	...
	MSG(0, "Info: zone aligned segment0 blkaddr: %x, device start sector: %u\n",
	...
	/*
	 * NOTICE: When multiple devices are supported,
	 * each device must be section aligned. Pay attention
	 * to this point when calculating total_segments.
	 */
	for (i = 0; i < c.ndevs; i++) {
		if (i == 0) {
			c.devices[i].total_segments =
				(c.devices[i].total_sectors *
				c.sector_size - segment_align_start_offset) /
				segment_size_bytes;
	...
	total_zones = (get_sb(segment_count) - total_meta_segments) / c.segs_per_zone;
	...
	c.original_reserved_segments =
		(2 * (100 / c.overprovision + 1) + NR_CURSEG_TYPE + 1);
	c.reserved_segments = HMFS_RSV_SEC * c.segs_per_sec;
	...
	normal_total_zones = total_zones;

	for (i = 0; i < c.ndevs; i++)
		if (c.devices[i].reserved)
			normal_total_zones -=
				c.devices[i].total_segments / c.segs_per_zone;

	if (normal_total_zones <= 6) {
		MSG(1, "\tError: %d (normal) zones: Need more zones "
	...
	if (c.heap) {
		c.cur_seg[CURSEG_HOT_NODE] =
				last_section(last_zone(normal_total_zones));
		...
		c.cur_seg[CURSEG_DATA_MOVE1] = next_zone(CURSEG_WARM_DATA);
		c.cur_seg[CURSEG_DATA_MOVE2] = next_zone(CURSEG_DATA_MOVE1);
	} else {
		...
		c.cur_seg[CURSEG_COLD_DATA] =
				max(last_zone((normal_total_zones >> 2)),
					next_zone(CURSEG_HOT_DATA));
		c.cur_seg[CURSEG_WARM_DATA] =
				max(last_zone((normal_total_zones >> 1)),
					next_zone(CURSEG_COLD_DATA));
		c.cur_seg[CURSEG_DATA_MOVE1] =
				min(last_zone((normal_total_zones >> 3)),
					next_zone(CURSEG_WARM_DATA));
		c.cur_seg[CURSEG_DATA_MOVE2] =
				min(last_zone((normal_total_zones >> 4)),
					next_zone(CURSEG_DATA_MOVE1));
	}

	update_cur_segs_by_rsvd_dev();

	/* if there is redundancy, reassign it */
	verify_cur_segs();

	c.cur_seg[CURSEG_WARM_NODE] = NULL_SEGNO;
	c.cur_seg[CURSEG_WARM_DATA] = NULL_SEGNO;
	...
	if (get_sb(feature) & F2FS_FEATURE_SB_CHKSUM) {
		...
		set_sb(crc, f2fs_cal_crc32(HMFS_SUPER_MAGIC, sb,
						SB_CHKSUM_OFFSET));
...
static int f2fs_write_check_point_pack(void)
{
	...
	struct f2fs_dm_block *dm_block = NULL;
	...
	int ret = -1;
	int dev;
	u_int32_t rsvd_dev_segs = 0;
	int extra_ovp = get_device_overprovision(get_sb(cp_blkaddr));
	int total_ovp;
	int hmfs_min_op_sec;
	...
	set_cp(cur_dmgc_segno[0], c.cur_seg[CURSEG_DATA_MOVE1]);
	set_cp(cur_dmgc_segno[1], c.cur_seg[CURSEG_DATA_MOVE2]);

	for (i = 3; i < MAX_ACTIVE_NODE_LOGS; i++) {
		set_cp(cur_node_segno[i], 0xffffffff);
	}

	for (i = 2; i < MAX_ACTIVE_DM_LOGS; i++) {
		set_cp(cur_dmgc_segno[i], 0xffffffff);
	}

	for (i = 3; i < MAX_ACTIVE_DATA_LOGS; i++) {
	...
	set_cp(overprov_segment_count, (get_sb(segment_count_main) -
			c.original_reserved_segments) * c.overprovision / 100);
	...
	MSG(0, "Info: Overprovision segments = %u (GC reserved = %u,Device ovp %d)\n",
				get_cp(overprov_segment_count),
				c.reserved_segments, extra_ovp);

	for (dev = 0; dev < c.ndevs; dev++)
		if (c.devices[dev].reserved)
			rsvd_dev_segs += c.devices[dev].total_segments;

	/* main segments - reserved segments - (node + data segments) */
	set_cp(free_segment_count, get_sb(segment_count_main) - 6 - rsvd_dev_segs);
	set_cp(user_block_count, ((get_cp(free_segment_count) + 6 -
			get_cp(overprov_segment_count)) * c.blks_per_seg));
	/* cp page (2), data summaries (1), node summaries (3) data move summary (2)*/
	set_cp(cp_pack_total_block_count, 8 + get_sb(cp_payload));
	flags = CP_UMOUNT_FLAG | CP_COMPACT_SUM_FLAG | CP_DATAMOVE_FLAG;
	...
	*/
	...
	journal->sit_j.entries[1].segno = cp->cur_node_segno[2];
	journal->sit_j.entries[1].se.vblocks =
				cpu_to_le16((CURSEG_COLD_NODE << 10));

	/* data sit for root */
	journal->sit_j.entries[2].segno = cp->cur_data_segno[0];
	journal->sit_j.entries[2].se.vblocks =
				cpu_to_le16((CURSEG_HOT_DATA << 10) |
						(1 + c.quota_dnum + c.lpf_dnum));
	f2fs_set_bit(0, (char *)journal->sit_j.entries[2].se.valid_map);
	for (i = 1; i <= c.quota_dnum; i++)
		f2fs_set_bit(i, (char *)journal->sit_j.entries[2].se.valid_map);
	if (c.lpf_dnum)
		f2fs_set_bit(i, (char *)journal->sit_j.entries[2].se.valid_map);

	journal->sit_j.entries[3].segno = cp->cur_data_segno[2];
	...
	/* Fill segment summary for DATAMOVE1 to zero. */
	memset(sum, 0, sizeof(struct f2fs_summary_block));
	SET_SUM_TYPE((&sum->footer), SUM_TYPE_DATA);
	cp_seg_blk++;
	DBG(1, "\tWriting Segment summary for DATAMOVE1, at offset 0x%08"PRIx64"\n",
			cp_seg_blk);
	if (dev_write_block(sum, cp_seg_blk)) {
		MSG(1, "\tError: While writing the sum_blk to disk!!!\n");
		goto free_cp_payload;
	...
	/* Fill segment summary for DATAMOVE2 to zero. */
	memset(sum, 0, sizeof(struct f2fs_summary_block));
	SET_SUM_TYPE((&sum->footer), SUM_TYPE_DATA);
	cp_seg_blk++;
	DBG(1, "\tWriting Segment summary for DATAMOVE2, at offset 0x%08"PRIx64"\n",
			cp_seg_blk);
	if (dev_write_block(sum, cp_seg_blk)) {
		MSG(1, "\tError: While writing the sum_blk to disk!!!\n");
		goto free_cp_payload;
	...
	/* init data move cp block init with 3 zero blocks */
	dm_block = (struct f2fs_dm_block *)sum;
	memset(dm_block, 0, sizeof(struct f2fs_dm_block));
	cp_seg_blk ++;
	DBG(1, "\tWriting data move unverified block for DATAMOVE, at offset 0x%08"PRIx64"\n",
			cp_seg_blk);
	if (dev_write_block(dm_block, cp_seg_blk) ||
		dev_write_block(dm_block, cp_seg_blk + 1) ||
		dev_write_block(dm_block, cp_seg_blk + 2)) {
		MSG(1, "\tError: While writing datamove unverified block to disk!!!\n");
		goto free_cp_payload;
	}
...
static int f2fs_write_super_block(void)
{
	...
	/* used to erase the whole super_block segment */
	DBG(1, "\tErasing super block segment\n");
	for(index = 2; index < min(c.blks_per_seg, get_sb(segment0_blkaddr)); ++index) {
		if (dev_fill_block(zero_buff, index)) {
			MSG(1, "\tError: While erasing super_block "
				"segments on disk!!! index : %d\n", index);
			free(zero_buff);
			return -1;
		}
	}
...
static int f2fs_write_root_inode(void)
{
	...
	if (c.feature & cpu_to_le32(F2FS_FEATURE_EXTRA_ATTR)) {
		raw_node->i.i_inline = HMFS_EXTRA_ATTR;
	...
	DBG(0, "\tWriting root inode (hot node), %x %x %x at offset 0x%08"PRIx64"\n",
			get_sb(main_blkaddr),
			c.cur_seg[CURSEG_HOT_NODE],
			c.blks_per_seg, main_area_node_seg_blk_offset);

	set_write_stream_id(HOT_NODE_STREAM_ID, main_area_node_seg_blk_offset);
...
static int f2fs_write_qf_inode(int qtype)
{
	...
	/* write two blocks */
	set_write_stream_id(HOT_DATA_STREAM_ID, data_blk_nor);
	...
	DBG(1, "\tWriting quota inode (hot node), %x %x %x at offset 0x%08"PRIx64"\n",
			get_sb(main_blkaddr),
			c.cur_seg[CURSEG_HOT_NODE],
			c.blks_per_seg, main_area_node_seg_blk_offset);
...
static int f2fs_update_nat_root(void)
{
	...
	MSG(0,"INFO: set stream 0 meta area.\n");
	DBG(1, "\tWriting nat root, at offset 0x%08"PRIx64"\n",
					nat_seg_blk_offset);

	set_write_stream_id(META_STREAM_ID, nat_seg_blk_offset);
...
static block_t f2fs_add_default_dentry_lpf(void)
{
	...
	set_write_stream_id(HOT_DATA_STREAM_ID, data_blk_offset);
...
static int f2fs_write_lpf_inode(void)
{
	...
	MSG(0,"INFO: set stream 3 hot node (lpf inode).\n");
	DBG(1, "\tWriting lost+found inode (hot node), %x %x %x at offset 0x%08"PRIx64"\n",
			get_sb(main_blkaddr),
			c.cur_seg[CURSEG_HOT_NODE],
			c.blks_per_seg, main_area_node_seg_blk_offset);
...
static int f2fs_add_default_dentry_root(void)
{
	...
	if (c.lpf_ino) {
		...
		memcpy(dent_blk->filename[3], (char *)LPF + F2FS_SLOT_LEN,
				len - F2FS_SLOT_LEN);
	...
	MSG(0,"INFO: set stream 4 hot data (root dentry)\n");
...
int f2fs_format_device(void)
{
	...
	if (c.ndevs > 1) {
		...
		unsigned int main_start_blk, main_start_sec, secs_after_meta;

		secs_after_meta = (c.max_sectors / c.sectors_per_blk -
				get_sb(main_blkaddr)) / c.blks_per_seg /
				c.segs_per_sec;
		main_start_blk = get_sb(main_blkaddr) +
				c.start_sector / DEFAULT_SECTORS_PER_BLOCK;
	...
	if (err < 0) {
		MSG(0, "\tError: Failed to set device segment mapping pos!!!\n");
		goto exit;
	}
	...
	if (c.trim) {
		err = f2fs_trim_range_devices(0, get_sb(main_blkaddr));
		if (err < 0) {
			MSG(0, "\tError: Failed to trim meta area!!!\n");
			goto exit;
		}
	}

	MSG(0,"INFO: set stream 0 meta (sit & nat init).\n");
	...
	MSG(0,"INFO: set stream 0 meta (cp pack).\n");
	set_write_stream_id(META_STREAM_ID, get_sb(segment0_blkaddr));
...
exit:
	...
	DBG(1,"\t device write done set stream 0 (reset 0) !!!!\n");
	set_write_stream_id(META_STREAM_ID, get_sb(segment0_blkaddr));
	return err;
}
