...
int f2fs_set_main_bitmap(struct f2fs_sb_info *sbi, u32 blk, int type)
{
	...
	__set_test_and_inuse(sbi, GET_SEGNO(sbi, blk));
	return f2fs_set_bit(BLKOFF_FROM_MAIN(sbi, blk), fsck->main_area_bitmap);
}

int f2fs_test_main_bitmap(struct f2fs_sb_info *sbi, u32 blk)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
							...
							u32 blk_addr)
{
	...
	if (need_fix && !c.ro) {
		...
		ret2 = f2fs_write_block(sbi, sum_blk, ssa_blk, 1);
		ASSERT(ret2 >= 0);
	}
out:
...
int is_valid_summary(struct f2fs_sb_info *sbi, struct f2fs_summary *sum,
							u32 blk_addr)
{
				  ...
				  struct f2fs_node *node)
{
	...
	size_t max_xattr_size, offs = 0, inline_size = (size_t)inline_xattr_size(inode);
	int err = 0;

	if (inode->i_xattr_nid)
		max_xattr_size = inline_size + MIN_OFFSET;
	else
	...
	if (err) {
		...
		if (xattr_nid && c.fix_on) {
			...
			ret = f2fs_write_block(sbi, (char *)xattr_blk +
						inline_size, ni.blk_addr, 0);
			ASSERT(ret >= 0);
...
void fsck_reada_node_block(struct f2fs_sb_info *sbi, u32 nid)
{
	...
	if (nid != 0 && IS_VALID_NID(sbi, nid)) {
		get_node_info(sbi, nid, &ni);
		if (IS_VALID_BLK_ADDR(sbi, ni.blk_addr))
			queue_reada_block(sbi, ni.blk_addr, NODE);
	}
}
...
{
	int idx = DEF_ADDRS_PER_INODE - get_inline_xattr_addrs(inode);
	u32 magic, refcount;

	if (inode->i_inline & HMFS_INLINE_XATTR)
		return 0;
	if (idx >= DEF_ADDRS_PER_INODE - 1)
		return 0;

	magic = le32_to_cpu(inode->i_addr[idx]);
	refcount = le32_to_cpu(inode->i_addr[idx + 1]);
	if ((magic == HMFS_XATTR_MAGIC) && (refcount == 1))
		return 1;
	return 0;
}
		...
		u32 *blk_cnt, struct node_info *ni, struct child_info *child_d)
{
	...
	if (fsck_chk_xattr_blk(sbi, nid, i_xattr_nid, blk_cnt) && c.fix_on) {
	...
	if (f2fs_has_extra_isize(&node_blk->i)) {
		if (c.feature & cpu_to_le32(F2FS_FEATURE_EXTRA_ATTR)) {
			...
			FIX_MSG("ino[0x%x] remove HMFS_EXTRA_ATTR "
				"flag in i_inline:%u",
		...
		if ((c.feature &
			...
			(node_blk->i.i_inline & HMFS_INLINE_XATTR)) {
	...
	if ((node_blk->i.i_inline & F2FS_INLINE_DATA)) {
		...
		if (i_size > inline_size) {
			...
			FIX_MSG("ino[0x%x] inline_data has wrong i_size %lu, i_blocks is %lu ",
				nid, (unsigned long)i_size, (unsigned long)i_blocks);
			need_fix = 1;
		}
	...
	/* check HMFS_INLINE_XATTR flag before check data */
	if (missing_inline_xattr(&node_blk->i)) {
		ASSERT_MSG("ino[0x%x] missing HMFS_INLINE_XATTR flag\n", nid);
		node_blk->i.i_inline |= HMFS_INLINE_XATTR;
		if (c.fix_on) {
			FIX_MSG("ino[0x%x] set HMFS_INLINE_XATTR\n", nid);
			need_fix = 1;
		}
	}

	if ((node_blk->i.i_inline & F2FS_INLINE_DENTRY) &&
		le32_to_cpu(node_blk->i.i_addr[ofs]) != 0) {
		nid_t xnid = le32_to_cpu(node_blk->i.i_xattr_nid);

		/* has inline dentry, but i_addr[ofs] != 0 */
		if (i_blocks == (1 + (xnid ? 1 : 0)) &&
				i_size <= MAX_INLINE_DATA(node_blk)) {
			FIX_MSG("ino[0x%x] has inline_dentry"
				"with wrong 0'th block = %x\n",
				nid, le32_to_cpu(node_blk->i.i_addr[ofs]));
			node_blk->i.i_addr[ofs] = cpu_to_le32(0);
			need_fix = 1;
			c.bug_on = 1;
		}

		/* no inline dentry, but has F2FS_INLINE_DENTRY flag */
		if (i_blocks > (1 + (xnid ? 1 : 0)) &&
				(i_size == 0 || i_size >= PAGE_SIZE)) {
			FIX_MSG("ino[0x%x] has no inline_dentry"
				"but has inline dentry flag\n", nid);
	...
	if ((node_blk->i.i_inline & F2FS_INLINE_DENTRY)) {
		DBG(3, "ino[0x%x] may have inline dentry, let's check it!\n", nid);
	...
	/* readahead DIR data blocks */
	if (ftype == F2FS_FT_DIR) {
		for (idx = 0; idx < ADDRS_PER_INODE(&node_blk->i); idx++) {
			block_t blkaddr = le32_to_cpu(node_blk->i.i_addr[ofs + idx]);/*lint !e679 !e737*/

			if (blkaddr == 0)
				continue;
			if (IS_VALID_BLK_ADDR(sbi, blkaddr))
				queue_reada_block(sbi, blkaddr, DATA);
		}
	}
	...
	for (idx = 0; idx < ADDRS_PER_INODE(&node_blk->i);
		...
		set_new_dnode(&dn, node_blk, node_blk, nid);
		dn.ofs_in_node = idx;
	...
	for (idx = 0; idx < 5; idx++) {
		...
		fsck_reada_node_block(sbi, _nid);
	}
	...
	for (idx = 0; idx < 5; idx++) {
		...
		set_new_dnode(&dn, node_blk, NULL, 0);
		ret = fsck_chk_node_blk(sbi, &node_blk->i, i_nid,
				ftype, ntype, blk_cnt, &child, &dn);
		if (dn.idirty)
			need_fix = 1;
		if (!ret) {
			*blk_cnt = *blk_cnt + 1;
...
int fsck_chk_idnode_blk(struct f2fs_sb_info *sbi, struct f2fs_inode *inode,
		...
		struct child_info *child, struct dnode_of_data *dn)
{
	int need_fix = 0, ret;
	int i = 0;
	...
	for (i = 0; i < NIDS_PER_BLOCK; i++) {
		...
		ret = fsck_chk_node_blk(sbi, inode,
				le32_to_cpu(node_blk->in.nid[i]), ftype,
				TYPE_DIRECT_NODE, blk_cnt, child, dn);
		if (!ret)
			*blk_cnt = *blk_cnt + 1;
		else if (ret == -EINVAL) {
			if (!c.fix_on)
		...
		struct dnode_of_data *dn)
{
	...
	for (i = 0; i < NIDS_PER_BLOCK; i++) {
		...
		ret = fsck_chk_node_blk(sbi, inode,
				le32_to_cpu(node_blk->in.nid[i]), ftype,
				TYPE_INDIRECT_NODE, blk_cnt, child, dn);
		if (!ret)
			*blk_cnt = *blk_cnt + 1;
		else if (ret == -EINVAL) {
			if (!c.fix_on)
			...
			int max, int last_blk, int enc_name)
{
	...
	for (i = 0; i < max; i++) {
		...
		if (test_bit_le(i, bitmap) == 0)
			continue;
		/* skip . and .. */
		if (le32_to_cpu(dentry[i].ino) == child->p_ino ||
		    le32_to_cpu(dentry[i].ino) == child->pp_ino)
			continue;
	...
	for (i = 0; i < max;) {
		...
		ret = fsck_chk_node_blk(sbi,
				...
				ftype, TYPE_INODE, &blk_cnt, child, NULL);
		...
		struct dnode_of_data *dn)
{
	...
	if (dentries < 0 && !c.ro) {
		ret = f2fs_write_data_block(sbi, de_blk, blk_addr, dn,
				CURSEG_HOT_DATA);
		ASSERT(ret >= 0);
...
int fsck_chk_data_blk(struct f2fs_sb_info *sbi, u32 blk_addr,
		...
		int enc_name, struct dnode_of_data *dn)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
...
int fsck_chk_orphan_node(struct f2fs_sb_info *sbi)
{
	...
	for (i = 0; i < orphan_blkaddr; i++) {
		...
		if (!c.ro && c.fix_on &&
			...
			ret = f2fs_write_block(sbi, new_blk, start_blk + i, 1);
			ASSERT(ret >= 0);
		}
...
int fsck_chk_quota_node(struct f2fs_sb_info *sbi)
{
	...
	for (qtype = 0; qtype < F2FS_MAX_QUOTAS; qtype++) {
		...
		ret = fsck_chk_node_blk(sbi, NULL, ino,
				F2FS_FT_REG_FILE, TYPE_INODE, &blk_cnt, NULL, NULL);
		if (ret) {
...
int fsck_chk_quota_files(struct f2fs_sb_info *sbi)
{
	...
	for (qtype = 0; qtype < F2FS_MAX_QUOTAS; qtype++) {
		...
		if (c.fix_on) {
			...
			hmfs_quota_cache_write_back(sbi);
			hmfs_quota_cache_clean();
		} else {
...
int fsck_chk_meta(struct f2fs_sb_info *sbi)
{
	...
	unsigned int i;
	unsigned int rsvd_dev_segs = 0;
	int devid;

	/* 1. check sit usage with CP: curseg is lost? */
	for (i = 0; i < TOTAL_SEGS(sbi); i++) {
		devid = get_device_index(START_BLOCK(sbi, i));
	...
	for (i = 0; i < c.ndevs; i++)
		if (c.devices[i].reserved)
			rsvd_dev_segs += c.devices[i].total_segments;

	if (fsck->chk.sit_free_segs + sit_valid_segs !=
					TOTAL_SEGS(sbi) - rsvd_dev_segs) {
...
static void build_sum_cache_list(struct f2fs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	int i, j;
...
static void destroy_sum_cache_list(struct f2fs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
...
void fsck_init(struct f2fs_sb_info *sbi)
{
	...
	build_sum_cache_list(sbi);
	init_reada_queue(sbi);
}
...
static void fix_hard_links(struct f2fs_sb_info *sbi)
{
	...
	while (node) {
		...
		if (sanity_check_nid(sbi, node->nid, node_blk,
			...
			FIX_MSG("Failed to fix, rerun fsck.hmfs");
...
void fix_checkpoint(struct f2fs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	struct f2fs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct f2fs_checkpoint *cp = F2FS_CKPT(sbi);
	...
	*/
	...
	if (flags & CP_DATAMOVE_FLAG)
		cp_blocks += NR_CURSEG_DM_TYPE;

	if (fsck->force_drop_recovery)
	...
	*((__le32 *)((unsigned char *)cp + CP_CHKSUM_OFFSET)) = cpu_to_le32(crc);
	...
	if (flags & CP_APPEND_NAT_FLAG) {
		...
		ret = dev_write_block_no_sync((char *)curseg->journal +
			SUM_JOURNAL_SIZE - NAT_JOURNAL_RESERVED, start_blk);
		ASSERT(ret >= 0);
	}

	if (flags & CP_DATAMOVE_FLAG) {
		block_t dm_cache_blk =  __start_cp_addr(sbi) +
			le32_to_cpu(F2FS_CKPT(sbi)->cp_pack_total_block_count);
		if (flags & CP_APPEND_NAT_FLAG)
			dm_cache_blk++;
		...
		for (i = 0; i < c.num_dm_blocks; i++) {
			dm_cache_block = sbi->dm_cache_blocks + i * F2FS_BLKSIZE;
...
int check_curseg_offset(struct f2fs_sb_info *sbi)
{
	...
	for (i = 0; i < NO_CHECK_TYPE; i++) {
		...
		if (f2fs_test_bit(curseg->next_blkoff,
			...
			se->valid_blocks--;
			f2fs_clear_bit(curseg->next_blkoff,
					(char *)se->cur_valid_map);
			ret = -EINVAL;
		}
		...
		for (j = curseg->next_blkoff + 1; j < nblocks; j++) {
			if (f2fs_test_bit(j, (const char *)se->cur_valid_map)) {
				...
				se->valid_blocks--;
				f2fs_clear_bit(j, (char *)se->cur_valid_map);
				ret = -EINVAL;
			}
		}
	}
				  ...
				  struct f2fs_node *fnode)
{
	...
	ret = f2fs_write_block(sbi, fnode, ni.blk_addr, 0);
	ASSERT(ret >= 0);
...
static void print_bitmap(unsigned int segno, const char *type,
	const char *ptr, int len)
{
	int i;

	ERR_MSG("%10s, segno:%d:", type, segno);
	for (i = 0; i < len; i++) {
		MSG(0, "%02x", (unsigned char)ptr[i]);
	}
	MSG(0, "\n");
}

static void print_unmatched_bitmap(struct f2fs_sb_info *sbi,
	const char *sit_area_ptr, const char *main_area_ptr)
{
	unsigned int segno;
	...
	for (segno = 0; segno < TOTAL_SEGS(sbi); segno++) {
		if (memcmp(sit_area_ptr, main_area_ptr, SIT_VBLOCK_MAP_SIZE)) {
			if (unmatched_num < max_printed) {
				print_bitmap(segno, "sit_area",
					sit_area_ptr, SIT_VBLOCK_MAP_SIZE);
				print_bitmap(segno, "main_area",
					main_area_ptr, SIT_VBLOCK_MAP_SIZE);
			}
			unmatched_num++;
		}
		sit_area_ptr += SIT_VBLOCK_MAP_SIZE;
	...
	ERR_MSG("total %d segment bitmap unmatched\n", unmatched_num);
}
...
int fsck_chk_curseg_info(struct f2fs_sb_info *sbi)
{
	...
	for (i = 0; i < NO_CHECK_TYPE; i++) {
		...
		if (i <= CURSEG_COLD_DATA && IS_SUM_DATA_SEG(sum_blk->footer)) {
			...
			continue;
		} else if (i >= CURSEG_DATA_MOVE1 && i < NO_CHECK_TYPE &&
				IS_SUM_DATA_SEG(sum_blk->footer)) {
			continue;
		} else {
			ASSERT_MSG("Incorrect curseg [%d]: segno [0x%x] "
...
int fsck_verify(struct f2fs_sb_info *sbi)
{
	...
	if (memcmp(fsck->sit_area_bitmap, fsck->main_area_bitmap,
		...
		MSG(0, "[Fail]\n");
		print_unmatched_bitmap(sbi, fsck->sit_area_bitmap,
					fsck->main_area_bitmap);
		ret = EXIT_ERR_CODE;
		c.bug_on = 1;
	}
...
void fsck_free(struct f2fs_sb_info *sbi)
{
	...
	exit_reada_queue(sbi);
	destroy_sum_cache_list(sbi);
