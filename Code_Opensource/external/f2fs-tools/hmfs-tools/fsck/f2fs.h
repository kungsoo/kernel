...
struct sit_info {
	...
	struct seg_entry *new_sentries;         /* SIT segment-level cache for resize*/
	struct sec_entry *sec_entries;          /* SIT section-level cache */
	unsigned int next_free_section;         /* used for resize to move data */
	unsigned int free_segments;		/* # of free segments for resize */
...
struct curseg_info {
	...
	__u64 cached_start_addr;                /* the cached start addr */
	unsigned int max_cache_size;            /* max cache size */
	unsigned int cached_size;		/* current cached size */
	char *seg_cache;                        /* segment cache */
};

struct free_segmap_info {
	unsigned int free_segments;	/* # of free segments */
	unsigned int free_sections;	/* # of free sections */
	u8 *free_segmap;	/* free segment bitmap */
	u8 *free_secmap;	/* free section bitmap */
	u8 *prefree_secmap;	/* prefree section bitmap */
	u8 *free_dm_secmap;	/* free data move section bitmap */
};
...
struct f2fs_sm_info {
	...
	unsigned int new_main_segments;
...
struct f2fs_sb_info {
	...
	struct f2fs_summary_block *sum_blks;	/* cached summary blocks for resize */
	char *dm_cache_blocks;
	unsigned int flash_mode[NR_CURSEG_TYPE];
};
...
#define IS_DM_SEG(t)                                                   \
	((t == CURSEG_DATA_MOVE1) || (t == CURSEG_DATA_MOVE2))
...
static inline enum SEG_TYPE get_seg_type(unsigned char type)
{
...
static bool IS_CURSEC(struct f2fs_sb_info *sbi, unsigned int secno)
{
	int i;

	for (i = CURSEG_HOT_DATA; i < NO_CHECK_TYPE; i++) {
		if (secno == (CURSEG_I(sbi, i)->segno / sbi->segs_per_sec)) {
			return 1;
		}
	}
	return 0;
...
static bool IS_CURSEG(struct f2fs_sb_info *sbi, unsigned int segno)
{
	int i;
...
#define NEW_TOTAL_SEGS(sbi) (SM_I(sbi)->new_main_segments)
#define MAX_GC_TIMES 15
...
static inline int __has_curseg_space(struct f2fs_sb_info *sbi,
		...
		unsigned int segno)
{
	...
	set_bit_le(segno, free_i->free_segmap);
	free_i->free_segments--;
	if (!test_and_set_bit_le(secno, free_i->free_secmap))
		...
		unsigned int segno)
{
	...
	if (!test_and_set_bit_le(segno, free_i->free_segmap)) {
		free_i->free_segments--;
		if (!test_and_set_bit_le(secno, free_i->free_secmap))
			free_i->free_sections--;
	}
...
static inline unsigned int min_fsck_free_section(struct f2fs_sb_info *sbi)
{
