			...
			int file_type, block_t p_blkaddr, int inc_link)
{
	...
	memset(&dn, 0, sizeof(struct dnode_of_data));
start:
...
add_dentry:
	...
	ret = f2fs_write_data_block(sbi, dentry_blk, dn.data_blkaddr,
						&dn, CURSEG_HOT_DATA);
	ASSERT(ret >= 0);
	...
	if (dn.ndirty) {
		ret = f2fs_write_node_block(sbi, dn.node_blk, dn.node_blkaddr);
		ASSERT(ret >= 0);
	}

	if (dn.idirty) {
		ASSERT(parent == dn.inode_blk);
		write_inode(sbi, p_blkaddr, dn.inode_blk);
	}
...
static void make_empty_dir(struct f2fs_sb_info *sbi, struct f2fs_node *inode)
{
	...
	set_write_stream_id(get_stream_id(CURSEG_HOT_DATA), blkaddr);
							...
							block_t p_blkaddr)
{
	...
	ret = f2fs_write_block(sbi, node, p_blkaddr, 0);
	ASSERT(ret >= 0);
	...
	if (!dir_level) {
		...
		ret = f2fs_write_data_block(sbi, dentry_blk,
				dn.data_blkaddr, &dn, CURSEG_HOT_DATA);
		ASSERT(ret >= 0);
...
int f2fs_create(struct f2fs_sb_info *sbi, struct dentry *de)
{
	...
	/*
	 * Write data immediately after the block reservation is
	 * completed based on the sequential write requirements
	 * of the hardware component.
	 */
	ret = reserve_new_block(sbi, &blkaddr, &sum, CURSEG_HOT_NODE, 1);
	ASSERT(!ret);

	set_write_stream_id(get_stream_id(CURSEG_HOT_NODE), blkaddr);
