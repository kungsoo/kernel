...
void fsck_usage()
{
	MSG(0, "\nUsage: fsck.hmfs [options] device\n");
	MSG(0, "[options]:\n");
	MSG(0, "  -a check/fix potential corruption, reported by hmfs\n");
...
void dump_usage()
{
	MSG(0, "\nUsage: dump.hmfs [options] device\n");
	MSG(0, "[options]:\n");
	MSG(0, "  -d debug level [default:0]\n");
...
void defrag_usage()
{
	MSG(0, "\nUsage: defrag.hmfs [options] device\n");
	MSG(0, "[options]:\n");
	MSG(0, "  -d debug level [default:0]\n");
...
void resize_usage()
{
	...
	MSG(0, "  -t target sectors, should with -s when target sectors\n"
	       "     belows total sectors [default: device size]\n");
	MSG(0, "  -c add a device at the end of fs\n");
	MSG(0, "  -r reserve segments for dynamic resize\n");
	MSG(0, "  -V print the version number and exit\n");
	exit(1);
}
...
void sload_usage()
{
	MSG(0, "\nUsage: sload.hmfs [options] device\n");
	MSG(0, "[options]:\n");
	MSG(0, "  -C fs_config\n");
...
{
	if (!strcmp("fsck.hmfs", prog) || !strcmp("fsck.hmfs_s", prog))
		fsck_usage();
	else if (!strcmp("dump.hmfs", prog))
		dump_usage();
	else if (!strcmp("defrag.hmfs", prog))
		defrag_usage();
	else if (!strcmp("resize.hmfs", prog) || !strcmp("resize.hmfs_s", prog))
		resize_usage();
	else if (!strcmp("sload.hmfs", prog))
		sload_usage();
	else
		MSG(0, "\nWrong program.\n");
}
...
static void __add_fsck_options(void)
{
	/* -a */
	c.auto_fix = 1;
	c.feature |= cpu_to_le32(F2FS_FEATURE_SB_CHKSUM);
}
...
void f2fs_parse_options(int argc, char *argv[])
{
	...
	/* Allow prog names (e.g, sload_hmfs, fsck_hmfs, etc) */
	for (i = 0; i < strlen(prog) - strlen("f2fs"); i++) {
		if (prog[i] == '_')
			prog[i] = '.';
	}
#endif
	...
	if (!strcmp("fsck.hmfs", prog) || !strcmp("fsck.hmfs_s", prog)) {
		...
		const char *option_string = "d:l:st:c:r:V";
		...
		while ((option = getopt(argc, argv, option_string)) != EOF) {
			...
			case 't':
				if (c.device_to_add) {
					MSG(0, "\tError: Cannot set both -l -t and -c.\n");
					err = EWRONG_OPT;
					break;
				}
				if (strncmp(optarg, "0x", 2))
			...
			case 'c':
				if (c.device_to_add) {
					MSG(0, "\tError: Cannot add multiple devices at a time.\n");
					err = EWRONG_OPT;
					break;
				}
				if (c.target_sectors) {
					MSG(0, "\tError: Cannot set both -t and -c.\n");
					err = EWRONG_OPT;
					break;
				}
				if (strlen(optarg) > MAX_PATH_LEN) {
					MSG(0, "\tError: device path should be no more than "
					        "%d characters\n", MAX_PATH_LEN);
					err = EWRONG_OPT;
					break;
				}
				c.device_to_add = strdup(optarg);
				break;
			case 'r':
				if (!is_digits(optarg)) {
					err = EWRONG_OPT;
				...
				if (!c.device_to_add) {
					MSG(0, "\tError: multiple devices should set in advance.\n");
					err = EWRONG_OPT;
					break;
				}
				/* it should be in special c.devices[1] */
				c.reserved_segs = atoi(optarg);
				break;
			case 'V':
				show_version(prog);
				exit(0);
...
#ifdef HW_F2FS_IMG_OEM
		const char *option_string = "F:P:C:d:f:ip:s:St:T:V";
#else
		const char *option_string = "C:d:f:ip:s:St:T:V";
#endif
...
static int do_fsck(struct f2fs_sb_info *sbi)
{
	...
	if (c.no_fix && (c.fix_on || c.bug_on)) {
		MSG(0, "hmfs tools:The background FSCK does not need to go on.\n");
		fsck_free(sbi);
		return -1;
	}
...
int main(int argc, char **argv)
{
	...
	/* set supplementary group AID_DSM for fsck.hmfs process to
	 * access /dev/dsm
	 */
	...
	if (c.device_to_add) {
		...
		if (f2fs_dev_is_umounted(c.device_to_add) < 0) {
...
retry:
	...
	ret = hmfs_finalize_device();
	if (ret < 0)
		ERR_MSG("\tError: hmfs_finalize_device failed!\n");
	else
		MSG(0, "\nDone.\n");
...
out_err:
	...
	dmd_report(client, DMD_HMFSCK_ERR);
