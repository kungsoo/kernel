...
static void write_checkpoint(struct f2fs_sb_info *sbi)
{
	struct f2fs_checkpoint *cp = F2FS_CKPT(sbi);
	struct f2fs_super_block *sb = F2FS_RAW_SUPER(sbi);
...
block_t get_dm_source_addr(block_t dst_addr)
{
	struct hmfs_dm_entry *dm_entry = NULL;

	if (c.dm_set == NULL)
		return dst_addr;
	if (dst_addr < c.dm_set->start || dst_addr > c.dm_set->end)
		return dst_addr;

	dm_entry = c.dm_set->dm_entry + (dst_addr - c.dm_set->start);
	return dm_entry->src_blkaddr;
}
