...
static int64_t sstr2ll(const char *s, const char *start)
{
    char *string = strstr(s, start);
    if (s) {//the character is '0'--'9'
        return atoll(string+strlen(start));
    }

    return 0;
}

static int parse_fd(const char *f, FileContext *c, int *is_streamed)
{
    int fd;
    int64_t offset, length;

    fd = (int)sstr2ll(f, "FileDescriptor=");
    offset = sstr2ll(f, "Offset=");
    length = sstr2ll(f, "Length=");
    ...
    struct stat st;
    *is_streamed = !fstat(fd, &st) && S_ISFIFO(st.st_mode);

    //LOGI("%s c->fd:%d fd:%d offset:%lld length:%lld", __FUNCTION__, c->fd, fd, offset, length);
    return 0;
}
static int file_open(URLContext *h, const char *filename, int flags)
{
    ...
    int fd;
    struct stat st;
    const char *proto = "file://?FileDescriptor";
    if (!strncmp(filename, proto, strlen(proto))) {
        return parse_fd(filename, c, &h->is_streamed);
    }
...
static int64_t file_seek(URLContext *h, int64_t pos, int whence)
{
    ...
    ret = lseek64(c->fd, pos + offset, whence);
    return ret < 0 ? AVERROR(errno) : (ret - c->offset);
}
