...
TRACE_EVENT(mm_compaction_kcompactd_score_update,

	TP_PROTO(int nid, unsigned int score, unsigned int wmark_low),

	TP_ARGS(nid, score, wmark_low),

	TP_STRUCT__entry(
		__field(int, nid)
		__field(unsigned int, score)
		__field(unsigned int, wmark_low)
	),

	TP_fast_assign(
		__entry->nid = nid;
		__entry->score = score;
		__entry->wmark_low = wmark_low;
	),

	TP_printk("nid=%d score=%u wmark_low=%u", __entry->nid,
				__entry->score, __entry->wmark_low)
);
