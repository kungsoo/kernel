...
extern unsigned long vip_fault_around_bytes __read_mostly;
#endif
...
int rejudge_protect_page(struct page *page, struct address_space *mapping, gfp_t gfp_mask, void *shadow);
#endif
...
static inline void update_nr_protect_pages(struct page *page, int inc)
{
	struct inode *inode = NULL;

	if (!PageSwapBacked(page) && page->mapping && page->mapping->host && page->mapping->host->i_protect) {
		inode = page->mapping->host;
		atomic_add(inc, &inode->nr_protect_pages);
	}
}
#endif
