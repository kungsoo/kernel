...
#ifdef CONFIG_HW_VIP_DR
	/*
	 * Distinguish the foreground and background processes.
	 * In alloc_pages_slowpath, background processes are reclaimed to
	 * reduce congestion caused by foreground processes.
	 */
	rcu_read_lock();
	blkcg = task_blkcg(current);
	blkcg_type = blkcg->type;
	rcu_read_unlock();
	if (blkcg_type > BLK_THROTL_KBG && can_direct_reclaim && !(current->flags & PF_MEMALLOC) &&
		!(fatal_signal_pending(current) && !(gfp_mask & __GFP_NOFAIL))) {
#ifdef CONFIG_HW_RECLAIM_ACCT
