...
/*
 * Free swapcache aggressively, expect 0 or 1.
 */
int free_swapcache_aggressively;
#endif
/*
...
#endif
{
	...
	list_for_each_entry(shrinker, p_shrinker_list, list) {
		...
		if (freed != 0)
			atomic_long_add(freed, &shrinker->freed);
#endif
						  ...
						  struct scan_control *sc)
{
	 ...
	 * Return PAGEREF_RECLAIM_PURGEABLE if every walked vma is
	 * purgeable. However, if rmap lock contention occurs, vma check
	 * is skipped, so we should return PAGEREF_KEEP in front.
	 */
#ifdef CONFIG_RMAP_TRYLOCK
...
unsigned int move_active_pages_to_lru(struct lruvec *lruvec,
...
#endif
	nr_activate = move_active_pages_to_lru(lruvec, &l_active,
			&l_hold, lru);
...
#ifndef CONFIG_ADJUST_SWAPPINESS
#ifdef CONFIG_DIRECT_SWAPPINESS
	if (!current_is_kswapd())
		swappiness = direct_vm_swappiness;
#ifdef CONFIG_OPTIMIZE_MM_AQ
...
#endif
#else
	swappiness = adjust_swappiness(swappiness);
#endif
...
static bool shrink_node(pg_data_t *pgdat, struct scan_control *sc)
{
	...
	do {
		 ...
		 */
		if (global_reclaim(sc) || should_only_do_gss())
		...
		*/
		...
		if (!sc->force_deactivate) {
			...
			if (refaults != target_lruvec->refaults[0] ||
			    inactive_list_is_low(target_lruvec, false, sc,
						 false))
			...
			if (refaults != target_lruvec->refaults[1] ||
			    inactive_list_is_low(target_lruvec, true, sc,
						 false))
...
static void snapshot_refaults(struct mem_cgroup *target_memcg, pg_data_t *pgdat)
{
	...
	lruvec->refaults[1] =
		lruvec_page_state(lruvec, WORKINGSET_ACTIVATE_FILE);
}
#endif
...
retry:
	...
	if (global_reclaim(sc)){
		...
		if (blkcg->type > BLK_THROTL_KBG)
			count_vm_event(ALLOCSTALL_BG);
		else
			count_vm_event(ALLOCSTALL_FG);
		rcu_read_unlock();
#endif
	}
	do {
...
int try_to_free_pages_ex(int nr_pages, int mode)
{
	...
	struct scan_control sc = {
		...
		.reclaim_idx = MAX_NR_ZONES - 1,
#ifdef CONFIG_FREE_SWAPCACHE_AGGRESSIVELY
		.may_writepage = ((mode == RCC_MODE_ANON) &&
				free_swapcache_aggressively) ? 1 : 0,
#else
		.may_writepage = 0,
#endif
