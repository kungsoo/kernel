...
#define MP_TEST_DATA_DIR "/data/vendor/log/tp_data"
#define MP_TEST_LOG_FILEPATH MP_TEST_DATA_DIR "/GCORE_MP_Log"
#define KIT_MP_TEST_NAME "GCORE_MP_Log"
struct ts_rawdata_info *gc_info_test = NULL;
...
int gcore_parse_mp_test_dt(struct gcore_mp_data *mp_data)
{
	...
	mp_data->test_int_pin =
		of_property_read_bool(np, MP_TEST_OF_ITEM_TEST_INT_PIN);

	mp_data->test_chip_id =
		of_property_read_bool(np, MP_TEST_OF_ITEM_TEST_CHIP_ID);
	if (mp_data->test_chip_id)
	...
	if (mp_data->test_short) {
		...
		of_property_read_u32(np, MP_TEST_OF_TEST_SHORT_MIN,
			&mp_data->short_min);
	}

	mp_data->test_rawdata =
		of_property_read_bool(np, MP_TEST_OF_ITEM_TEST_RAWDATA);
	if (mp_data->test_rawdata) {
		of_property_read_u32_array(np, MP_TEST_OF_TEST_RAWDATA_RANGE,
			mp_data->rawdata_range, 2);
		of_property_read_u32(np, MP_TEST_OF_TEST_RAWDATA_MAX,
			&mp_data->rawdata_per);
...
int gcore_parse_mp_test_ini(struct gcore_mp_data *mp_data)
{
	...
	snprintf(mp_ini_name, sizeof(mp_ini_name), "ts/%s.ini",
		gdev_kit->factory_fw_name);
	GTP_DEBUG("mp ini name :%s", mp_ini_name);
	ret = request_firmware(&fw, mp_ini_name, dev);
	if (ret == 0) {
		GTP_DEBUG("firmware request(%s) success", mp_ini_name);
		...
		ret = sscanf(tmp, "rawdata_range=[%d %d]", &mp_data->rawdata_range_min,
			&mp_data->rawdata_range_max);
		if (ret < 0)
			GTP_ERROR("%s rawdata range error.\n", __func__);
		GTP_DEBUG("read range min:%d max:%d", mp_data->rawdata_range_min,
			mp_data->rawdata_range_max);
		...
		if (NULL == tmp) {
			GTP_DEBUG("empty_place set default:%d %d", empty_place[0],
				empty_place[1]);
		} else {
			int num1, num2;
		...
		GTP_ERROR("firmware request(%s) fail,ret=%d", mp_ini_name, ret);
	}
	return 0;
}
...
int gcore_mp_test_int_pin(struct gcore_mp_data *mp_data)
{
	...
	GTP_DEBUG("reg addr: 0xC0000098 read_buf: %x %x %x %x", read_buf[0],
	...
	for (i = 0; i < RAW_DATA_SIZE / 2; i++) {
		...
		if (!jump) {
			if (node_data < mp_data->open_min) {
				GTP_ERROR("i: %d node_data %d < open_min test fail!", i,
					node_data);
	...
	for (i = 0; i < RAW_DATA_SIZE / 2; i++) {
		...
		if (!jump) {
			if (node_data < mp_data->short_min) {
				GTP_ERROR("i: %d node_data %d < short_min test fail!", i,
					node_data);
...
int gcore_mp_test_item_rawdata(struct gcore_mp_data *mp_data)
{
	...
	for (j = 0; j < cols; j++) {
		for (i = 0; i < rows; i++) {
			if (i == rows - 1) {
				dec_value[i * cols + j] =
					abs(rndata[i * cols + j - cols] - rndata[i * cols + j]) *
					100 / rndata[i * cols + j];
			} else {
				dec_value[i * cols + j] =
					abs(rndata[i * cols + j + cols] - rndata[i * cols + j]) *
	...
	for (j = 0; j < cols; j++) {
		...
		for (i = 0; i < rows; i++)
			dec_value[i * cols + j] =
				abs(avg - rndata[i * cols + j]) * 100 / rndata[i * cols + j];
	}
#endif
	...
	for (i = 0; i < RAW_DATA_SIZE / 2; i++) {
		...
		if (!jump) {
			...
			if ((node_data > mp_data->rawdata_range_max) ||
				...
				GTP_ERROR("rawdata range %d data %d not in range test fail!", i,
					node_data);
			...
			if (node_data > mp_data->rawdata_per) {
				GTP_ERROR("dech %d %d n_data %d > rawdata_per test fail!", i,
					mp_data->rawdata_node_val[i], node_data);
			...
			if (node_data > mp_data->rawdata_per) {
				GTP_ERROR("decl %d %d n_data %d > rawdata_per test fail!", i,
					mp_data->rawdata_node_val[i], node_data);
			...
			if (node_data > mp_data->rawdata_per) {
				GTP_ERROR("avgh %d %d n_data %d > rawdata_per test fail!", i,
					mp_data->rawdata_node_val[i], node_data);
			...
			if (node_data > mp_data->rawdata_per) {
				GTP_ERROR("avgl %d %d n_data %d > rawdata_per test fail!", i,
					mp_data->rawdata_node_val[i], node_data);
	...
	for (i = 0; i < RAW_DATA_SIZE / 2; i++) {
		...
		if (!jump) {
			if (node_data > mp_data->peak_to_peak) {
				GTP_ERROR("i: %d node_data %d > peak_to_peak test fail!", i,
					node_data);
...
int gcore_alloc_mp_test_mem(struct gcore_mp_data *mp_data)
{
	...
	if (mp_data->test_rawdata) {
		...
		if (mp_data->rawdata_node_val == NULL) {
			mp_data->rawdata_node_val =
				(u16 *)kzalloc(RAW_DATA_SIZE, GFP_KERNEL);
	...
	int cols, const char *prefix, const char *suffix, char seperator)
{
	...
	for (c = 0; c < cols; c++) {
		if (!firstline) {
			count += sprintf(buf + count, "%4u%c ", data[c], seperator);
		} else {
	...
	int cols, const char *prefix, const char *suffix, char seperator)
{
	...
	for (c = 0; c < cols; c++)
		count +=
			scnprintf(buf + count, size - count, "%4d%c ", data[c], seperator);
	...
	int cols, const char *prefix, const char *suffix, char seperator)
{
	...
	for (c = 0; c < cols; c++)
		count +=
			scnprintf(buf + count, size - count, "%02x%c", data[c], seperator);
...
int dump_excel_head_row_to_buffer(char *buf, size_t size, int cols,
...
static loff_t pos;
int dump_mp_data_to_csv_file(const char *filepath, int flags, const u16 *data,
	int rows, int cols, enum MP_DATA_TYPE types)
{
	struct file *file;
	int r = 0;
	int ret = 0;
...
int dump_mp_info_to_csv_file(const char *filepath, int flags)
{
...
#elif defined(CONFIG_TOUCH_DRIVER_INTERFACE_SPI)
	scnprintf(speed, sizeof(speed), "%u",
		g_mp_data->gdev->bus_device->max_speed_hz);
#endif
	len = scnprintf(linebuf, sizeof(linebuf),
		"INFO:, \n"
		...
		"\n\n",
		TOUCH_DRIVER_RELEASE_VERISON, g_mp_data->mpbin_ver[1],
		g_mp_data->mpbin_ver[0], g_mp_data->mpbin_ver[3],
		g_mp_data->mpbin_ver[2], icname, intef, speed,
	...
	memcpy(g_mp_data->info_buf, linebuf, sizeof(linebuf));
...
int dump_demo_data_to_csv_file(const u8 *data, int rows, int cols)
{
	...
	for (i = 1; i <= 10; i++)
		GTP_DEBUG("data %d:%x %x %x %x %x %x", i, data[6 * i - 6],
			data[6 * i - 5], data[6 * i - 4], data[6 * i - 3], data[6 * i - 2],
			data[6 * i - 1]);


	if (IS_ERR(demolog_file)) {
		demolog_file =
			filp_open(demolog_path, O_WRONLY | O_CREAT | O_TRUNC, 0666);
		if (IS_ERR(demolog_file)) {
			GTP_ERROR("Open file %s failed %ld", demolog_path,
				PTR_ERR(demolog_file));
			return -EPERM;
		}
	}
	...
	for (r = 0; r < rows; r++) {
		len = dump_demo_data_row_to_buffer(linebuf, sizeof(linebuf), n_val,
			cols, NULL, "\n", ',');
		if (gcore_write_buff_to_file(demolog_file, linebuf, len)) {
			goto fail;
		}
...
void dump_mp_data_to_seq_file(struct seq_file *m, const u16 *data, int rows,
	int cols)
{
	int r = 0;
	...
	for (r = 0; r < rows; r++) {
		...
		len = dump_mp_data_row_to_buffer(linebuf, sizeof(linebuf), n_vals, cols,
			NULL, "\n", ',');
		seq_puts(m, linebuf);
...
void dump_diff_data_to_seq_file(struct seq_file *m, const s16 *data, int rows,
	int cols)
{
	int r = 0;
	...
	for (r = 0; r < rows; r++) {
		len = dump_diff_data_row_to_buffer(linebuf, sizeof(linebuf), n_vals,
			cols, NULL, "\n", ',');
		seq_puts(m, linebuf);
...
int gcore_save_mp_data_to_file(struct gcore_mp_data *mp_data)
{
	...
	scnprintf(mmitest_name, FILEPATH_MAX_LENGTH, "%s-%s-%s;", KIT_MP_TEST_NAME,
		rtctime, mp_data->all_test_result ? "FAIL" : "PASS");
	if (!IS_ERR_OR_NULL(gc_info_test)) {
		strncat(gc_info_test->result, mmitest_name, strlen(mmitest_name));
		gc_info_test->buff[0] = rows;
		gc_info_test->buff[1] = cols;
		gc_info_test = NULL;
	}
	GTP_DEBUG("dump mp data to csv file: %s row: %d col: %d", filepath, rows,
		cols);
	...
	if (mp_data->test_open) {
		ret = dump_mp_data_to_csv_file(filepath, O_RDWR | O_CREAT,
	...
	if (mp_data->test_rawdata) {
		...
		scnprintf(filepath, FILEPATH_MAX_LENGTH, "%s_%lu_%s.csv",
			...
			mp_data->all_test_result ? "FAIL" : "PASS");
		ret = dump_mp_data_to_csv_file(filepath, O_RDWR | O_CREAT | O_TRUNC,
...
int save_demo_data_to_file(int enable)
{
	...
	if (enable == 1) {
		...
		scnprintf(demolog_path, FILEPATH_MAX_LENGTH, "%s_%s.csv",
			"/sdcard/demodata", rtctime);
...
int gcore_mp_bin_update(void)
{
	...
	snprintf(mp_bin_name, sizeof(mp_bin_name), "ts/%s_mp.bin",
		gdev_kit->factory_fw_name);
	GTP_DEBUG("mp bin name :%s", mp_bin_name);
	...
	GTP_DEBUG("mp bin ver:%x %x %x %x", fw_buf[FW_VERSION_ADDR + 1],
		fw_buf[FW_VERSION_ADDR], fw_buf[FW_VERSION_ADDR + 3],
...
int gcore_kit_start_mp_test(struct ts_rawdata_info *info_top)
{
	struct gcore_mp_data *mp_data = g_mp_data;
	struct ts_kit_device_data *chip_data = g_mp_data->gdev->tskit_data;
	if (!IS_ERR_OR_NULL(info_top))
		gc_info_test = info_top;
	...
	chip_data->is_ic_rawdata_proc_printf = 1;
	if (gdev_kit->platformkit_data->chip_data->is_direct_proc_cmd == 0) {
		enable_irq(gdev_kit->touch_irq);
		GTP_DEBUG("Enable irq");
	}
	...
	if (mp_data->test_open || mp_data->test_short || mp_data->test_rawdata ||
		...
		if (gcore_mp_bin_update()) {
			...
			if (mp_data->test_rawdata) {
				mp_data->rawdata_test_result =
					gcore_mp_test_item_rawdata(mp_data);
...
static int gcore_seq_raw_show(struct seq_file *m, void *v)
{
	...
	while (gdev->data_ready == false) {
		if (wait_event_interruptible(gdev->usr_wait,
...
{
	rawdata_proc_entry = proc_create_data(RAWDATA_PROC_FILENAME, S_IRUGO, NULL,
		&gcore_rawdata_fops, NULL);
...
int gcore_mp_test_fn_init(struct gcore_dev *gdev)
{
	...
	mp_data->mp_proc_entry = proc_create_data(MP_TEST_PROC_FILENAME, S_IRUGO,
		NULL, &gcore_mp_fops, mp_data);
...
int mptest_hal_config_parse(struct gcore_mp_data *mp_data)
{
	...
	sscanf(mptest_ini_file_buff,
		"%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,", &mp_data->test_int_pin,
		&mp_data->test_chip_id, &chip_type, &mp_data->test_open,
		&mp_data->open_cb, &mp_data->open_min, &mp_data->test_short,
		&mp_data->short_cb, &mp_data->short_min, &mp_data->test_rawdata,
		&mp_data->rawdata_per, &mp_data->rawdata_range_min,
		&mp_data->rawdata_range_max, &mp_data->test_noise,
		&mp_data->peak_to_peak);
...
int mptest_hal_bin_update(void)
{
	...
	GTP_DEBUG("fw head buf:%x %x %x %x %x %x", mptest_bin_file_buff[0],
		mptest_bin_file_buff[1], mptest_bin_file_buff[2],
		mptest_bin_file_buff[3], mptest_bin_file_buff[4],
		mptest_bin_file_buff[5]);
...
int mptest_result_parse(char *string_buff, int string_pos, const u16 *data,
	int rows, int cols, enum MP_DATA_TYPE types)
{
	int r = 0;
	int ret = 0;
	...
	for (r = 0; r < rows; r++) {
		...
		if (ret != len) {
			GTP_ERROR("%d,%d, sprintf failed,ret:%d, len%d", types, r, ret,
				len);
			goto fail1;
		}
		string_pos_len += len;
	...
	case RAWDATA_DECH:
		len = sprintf(linebuf, "Max:, %4u, ,Min:, %4u, ,Threshold:, %4u%%, \n\n",
			g_mp_data->rawdata_dech_max, g_mp_data->rawdata_dech_min,
			g_mp_data->rawdata_per);
		break;
...
int mptest_result_to_hal(struct gcore_mp_data *mp_data)
{
	...
	len = scnprintf(linebuf, sizeof(linebuf),
		...
		"6-Noise Test Result:, %s, %s\n"
		"\n\n",
		g_mp_data->all_test_result, TOUCH_DRIVER_RELEASE_VERISON,
		g_mp_data->mpbin_ver[1], g_mp_data->mpbin_ver[0],
		g_mp_data->mpbin_ver[3], g_mp_data->mpbin_ver[2], icname, intef, speed,
		g_mp_data->all_test_result ? "FAIL" : "PASS",
		g_mp_data->chip_id_test_result ? "FAIL" : "PASS",
		g_mp_data->mp_update_result ? "FAIL" : "PASS",
		g_mp_data->open_test_result ? "FAIL" : "PASS",
