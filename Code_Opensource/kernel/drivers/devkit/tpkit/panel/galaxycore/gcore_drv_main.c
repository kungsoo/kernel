...
static ssize_t tperaseflash_store(struct file *file, const char __user *buffer,
...
static int create_tpd_proc_entry(void)
{
	...
	tpd_proc_entry = proc_create("Erase_flash", 0664, proc_gcorei2c_dir,
		&proc_ops_tperaseflash);
...
int gcore_get_gpio_info(struct gcore_dev *gdev, struct device_node *node)
{
	...
	gdev->irq_gpio = of_get_named_gpio_flags(node, "gcore,irq-gpio", 0,
	...
	gdev->rst_gpio = of_get_named_gpio_flags(node, "gcore,rst-gpio", 0,
		(enum of_gpio_flags *)&flag);
...
void gcore_get_dts_info(struct gcore_dev *gdev)
{
	...
	defined(CONFIG_TOUCH_DRIVER_RUN_ON_HISI_PLATFORM)
...
u8 Cal8bitsChecksum(u8 *array, u16 length)
{
	s32 s32Checksum = 0;
	u16 i;
	...
	if (coor_data[0] == 0xB1) {
		...
		if (!sum) {
			if (!gdev->ts_stat) {
				...
				queue_delayed_work(gdev->fwu_workqueue, &gdev->fwu_boot_work,
...
#if defined(CONFIG_ENABLE_PROXIMITY_TP_SCREEN_OFF) && \
	defined(CONFIG_MTK_PROXIMITY_TP_SCREEN_ON_ALSPS)
bool gcore_tpd_proximity_flag_off = 0;
bool gcore_get_proximity_flag_off(void)
{
	GTP_DEBUG("proximity_flag_off:%d", gcore_tpd_proximity_flag_off);

	return gcore_tpd_proximity_flag_off;
}
...
int gcore_probe_device(void)
{
	...
	int retries = 0;
	int ret = 0;
	if (gdev_kit->gc_ic_type == GC7272)
		chip_type = GCORE_CHIP_ID_GC7272;
	else
		GTP_ERROR("chip_type is error,chip_type=%04x", chip_type);
	...
	while (retries++ < 3) {
		...
		if (id == chip_type) {
			GTP_DEBUG("GC%04x chip id match success,chip_type=%04x", id,
				chip_type);
			ret = 0;
			break;
		} else {
...
void gcore_wdt_recovery_works(struct work_struct *work)
{
	...
	if (!gdev->ts_stat) {
		if (wdt_contin < 3) {
			GTP_ERROR("WDT timeout recovery ts:%d,wdtime:%d", gdev->ts_stat,
				wdt_contin);
...
static int gcore_charger_notify_call(struct notifier_block *nb,
	unsigned long event, void *data)
{
	...
	if ((strcmp(psy_usb->desc->name, "usb") == 0) ||
		...
		GTP_DEBUG("usb2");
		queue_delayed_work(gdev->gtp_workqueue, &gdev->charger_work,
...
static bool gcore_get_charger_ststus(void)
{
	...
	if (!strcmp(psy_usb->desc->name, "usb")) {
		ret = power_supply_get_property(psy_usb, POWER_SUPPLY_PROP_ONLINE,
			&prop_usb);
		if (ret < 0)
	...
	if (!strcmp(psy_ac->desc->name, "ac")) {
		ret = power_supply_get_property(psy_ac, POWER_SUPPLY_PROP_ONLINE,
			&prop_ac);
		if (ret < 0)
			GTP_ERROR("Couldn't get POWER_SUPPLY_PROP_ONLINE rc=%d", ret);
	}

	if ((prop_usb.intval == POWER_SUPPLY_STATUS_CHARGING) ||
		((prop_ac.intval == POWER_SUPPLY_STATUS_CHARGING)) ||
		(prop_usb.intval == POWER_SUPPLY_STATUS_FULL) ||
		((prop_ac.intval == POWER_SUPPLY_STATUS_FULL))) {
		status = true;
	} else {
		status = false;
	}
...
static void gcore_work_charger_detect_work(struct work_struct *work)
{
	...
	if (!gdev->ts_stat && (gdev->charger_mode != charger_mode_old)) {
...
s32 gcore_init(struct gcore_dev *gdev)
{
...
#ifdef CONFIG_BOOT_UPDATE_FIRMWARE_BY_ARRAY
	INIT_DELAYED_WORK(&gdev->fwu_boot_work,
		gcore_kit_request_firmware_update_boot_work);
#endif
#if GCORE_WDT_RECOVERY_ENABLE
	...
	INIT_WORK(&gdev->headset_work, gcore_extern_notify_event);
#endif

	INIT_DELAYED_WORK(&gdev->resume_notify_work,
		gcore_resume_event_notify_work);
...
{
	GTP_DEBUG("kit touch probe start");
	...
	if (gcore_active_panel) {
		if (drm_panel_notifier_register(gcore_active_panel,
			&gdev->drm_notifier) < 0)
...
static s32 gcore_i2c_probe(struct i2c_client *client,
...
static int gcore_i2c_detect(struct i2c_client *client,
...
#ifdef CONFIG_MTK_LEGACY_PLATFORM
	/* MTK spi.c if transfer > 1024 bytes,
	the lens must be a multiple of 1024
	 */
	if ((len > 1024) && (len % 1024)) {
		GTP_ERROR("mtk spi tansfer len must be a multiple of 1024!");
		return -EPERM;
...
s32 gcore_spi_probe(struct spi_device *slave)
{
	struct gcore_dev *touch_dev = gdev_kit;
