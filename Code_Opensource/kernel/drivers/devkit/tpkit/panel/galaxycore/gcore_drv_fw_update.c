...
#ifndef CONFIG_BOOT_UPDATE_FIRMWARE_BY_ARRAY
#include "gcore_drv_firmware.h"
#endif


#define FW_BIN_NAME "ts/gcore_tp_fw.bin"

#define STARTUP_UPDATE_DELAY_MS 1000
...
void gcore_reset(void)
{
	...
	int error;
	error = pinctrl_select_state(fn_data.gdev->pctrl,
		fn_data.gdev->pinctrl_state_reset_low);
	if (error < 0)
		GTP_ERROR("gcore_hw_reset: Set reset pin low state error:%d", error);
	msleep(10);
	error = pinctrl_select_state(fn_data.gdev->pctrl,
		fn_data.gdev->pinctrl_state_reset_high);
	if (error < 0)
		GTP_ERROR("gcore_hw_reset: Set reset pin high state error:%d", error);
	msleep(10);
#endif
}
...
s32 gcore_idm_write_page_reg(u8 page, u32 addr, u8 *buffer, s32 len)
{
	...
	GTP_DEBUG("reg ckucr0:%x %x %x %x", ckucr0[0], ckucr0[1], ckucr0[2],
		ckucr0[3]);
...
int gcore_fw_event_notify(enum fw_event_type event)
{
	...
	case FW_LOWPOWER_OFF:
		fw_event_cmd[10] = fw_event_cmd[10] & ~((0x00 | 0x01) << 6);
		notify_byte = fw_event_cmd[10];
		GTP_DEBUG("gcore fw event:turn off ic lowpower mode");
		break;

	case FW_LOWPOEWR_ON:
		fw_event_cmd[10] = fw_event_cmd[10] | ((0x00 | 0x01) << 6);
		notify_byte = fw_event_cmd[10];
		GTP_DEBUG("gcore fw event:turn on ic lowpower mode");
		break;
	...
	if (g_ret_update == 0 || gdev_fwu->ts_stat == TS_SUSPEND) {
		GTP_DEBUG("Only change cmd!");
		gdev_fwu->notify_state = false;
		GTP_DEBUG("fw event:[80B0 %x %x %x %x %x %x %x %x]", fw_event_cmd[3],
			fw_event_cmd[4], fw_event_cmd[5], fw_event_cmd[6], fw_event_cmd[7],
			fw_event_cmd[8], fw_event_cmd[9], fw_event_cmd[10]);
		return 0;
	} else {
		gdev_fwu->fw_event = event;
	}
	...
	if (gdev_fwu->fw_event == FW_TEL_HANDUP ||
		gdev_fwu->fw_event == FW_TEL_CALL ||
		gdev_fwu->fw_event == FW_HANDSFREE_OFF ||
		gdev_fwu->fw_event == FW_HANDSFREE_ON)
...
void gcore_modify_fw_event_cmd(enum fw_event_type event)
{
	...
	GTP_DEBUG("modify event cmd[9]=%d,cmd[10]=%d", fw_event_cmd[9],
		fw_event_cmd[10]);
}
...
void gcore_fw_event_notify_work(struct work_struct *work)
{
	...
	if (gdev_fwu->notify_state) {
		if (notify_contin < 4) {
			GTP_DEBUG("notify :%d ,retry notify times :%d", gdev_fwu->fw_event,
				notify_contin);
...
static u32 dead_zone_width = 0, edge_width = 0, corner_width = 0,
		   corner_height = 0;
static ssize_t gcore_fw_edge_protection_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d,%d,%d ", &event, &width, &height) == 3) {
		...
		GTP_DEBUG("fail event is:%d,width is:%d,height is:%d", event, width,
			height);
	}
	...
	case fw_dead_zone_event:
		...
		GTP_DEBUG("gcore fw edge event data:%d,%d,%d", cmd[1], cmd[cmd_len - 2],
			cmd[cmd_len - 1]);
		break;
	...
	case fw_corner_event:
		...
		GTP_DEBUG("gcore fw correr event data:%d,%d,%d", cmd[1],
			cmd[cmd_len - 2], cmd[cmd_len - 1]);
		break;
	...
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE,
		"dead_zone_width=%d edge_width=%d corner_width=%d edge_height=%d\n",
...
int gcore_read_fw_version(u8 *version, int length)
{
	...
	if (!(memcmp(gdev_fwu->fw_ver_in_bin, gdev_fwu->fw_ver_in_reg, 4))) {
		if (!memcmp(buf1, gdev_fwu->fw_ver_in_bin, 4) ||
			!memcmp(buf2, gdev_fwu->fw_ver_in_bin, 4)) {
			GTP_DEBUG("Enter IDM read version");
			ver_flag = true;
		} else {
			GTP_DEBUG("FW version number is not changed");
			memcpy(version, gdev_fwu->fw_ver_in_reg, 4);
			GTP_DEBUG("fw version:%x %x %x %x", version[0], version[1],
				version[2], version[3]);
			return 0;
		}
	}
	...
	GTP_DEBUG("fw version:%x %x %x %x", version[0], version[1], version[2],
		version[3]);
...
int gcore_dump_fw_to_file(void)
{
...
#ifdef CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM
	file =
		filp_open_block(DUMP_FW_FILE_NAME, O_WRONLY | O_CREAT | O_TRUNC, 0666);
#else
	...
	pos = 0;
#ifdef CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM
	ret = file->f_op->write(file, fw_buf, FW_SIZE, &pos);
#else
 ...
 */
#define GCORE_START_UPDATE 1
#define GCORE_THREE_UPDATE_FAIL 2
#define GCORE_FW_UPDATE_SUCCESS 3
#define GCORE_UNKNOW_RESULT 4
...
int gcore_auto_update_hostdownload(u8 *fw_buf)
{
	...
	case 0xB1:
		...
		ret = GCORE_START_UPDATE;
		break;

	case 0xB2:
		GTP_DEBUG("update pram fw retry 3 times fail!");
		fw_update_fn.wait_int = false;
		ret = GCORE_THREE_UPDATE_FAIL;
		break;
...
int gcore_update_hostdownload_trigger(u8 *fw_buf)
{
	...
	if (!wait_for_completion_interruptible_timeout(&fw_update_complete, 2 * HZ)) {
		ret = gcore_auto_update_hostdownload(gdev_fwu->firmware);
		if (ret == GCORE_START_UPDATE) {
			gcore_bus_read(&result, 1);
			GTP_DEBUG("read result :%x", &result);
			if (result == 0xB3)
				return 0;
		} else if (ret == GCORE_UNKNOW_RESULT) {
...
#endif
/* CONFIG_GCORE_AUTO_UPDATE_FW_HOSTDOWNLOAD */
...
int gcore_update_prepare(u8 *fw_buf)
{
	...
	update_info.flash_op_data =
		kzalloc(update_info.flash_op_length, GFP_KERNEL);
	...
	GTP_DEBUG("flash op data:%x %x %x %x %x %x", update_info.flash_op_data[0],
		update_info.flash_op_data[1], update_info.flash_op_data[2],
		update_info.flash_op_data[3], update_info.flash_op_data[4],
		update_info.flash_op_data[5]);
...
int gcore_update_judge(u8 *fw_buf)
{
	...
	fw_major_update =
		(fw_buf[FW_VERSION_ADDR + 1] << 8) | fw_buf[FW_VERSION_ADDR];
	fw_minor_update =
		(fw_buf[FW_VERSION_ADDR + 3] << 8) | fw_buf[FW_VERSION_ADDR + 2];
	fw_version_update = (fw_major_update << 16) | fw_minor_update;

	GTP_DEBUG("fw_version_update=%x fw_version=%x", fw_version_update,
		fw_version);
...
int gcore_burn_flash_op_fw(void)
{
	...
	while (times--) {
		...
		memcpy(update_info.flash_op_buf + 8, flash_op_p,
			FLASH_OP_TRANSFER_SIZE);

		ret = gcore_bus_write(update_info.flash_op_buf,
			FLASH_OP_TRANSFER_SIZE + 8);
		if (ret) {
			GTP_ERROR("flash op fw addr write error");
			return -EPERM;
		}
...
int gcore_flash_program_proc(void)
{
	...
	if ((update_info.fw_length - update_info.fw_burn_progress) >
		GC7XXX_MAX_TRANSFER_DATA) {
	...
	memcpy(&cmd[8], update_info.fw_data + update_info.fw_burn_progress,
		transfer_length);
...
int gcore_flashdownload_fspi_proc(void *fw_buf)
{
	...
	GTP_DEBUG("i2c hold timeout value:%x %x %x %x", syscr4[0], syscr4[1],
		syscr4[2], syscr4[3]);
	usleep_range(1000, 1100);
...
int gcore_flashdl_version_check(u8 *fw_buf)
{
	...
	if ((ver[0] != fw_buf[FW_VERSION_ADDR]) ||
		(ver[1] != fw_buf[FW_VERSION_ADDR + 1]) ||
...
#endif
/* CONFIG_GCORE_AUTO_UPDATE_FW_FLASHDOWNLOAD */
...
int gcore_tp_esd_fail = 0;
int gcore_tp_esd_fail_count = 0;

int gcore_esd_is_fail(void)
{
	GTP_DEBUG("tp esd flag:%d", gcore_tp_esd_fail);
	return gcore_tp_esd_fail;
}
EXPORT_SYMBOL(gcore_esd_is_fail);
...
void gcore_request_firmware_update_work(struct work_struct *work)
{
	...
	if (request_firmware(&fw, gdev_fwu->firmware_name,
		&gdev_kit->tskit_data->ts_platform_data->ts_dev->dev)) {
	...
	GTP_DEBUG("fw buf:%x %x %x %x %x %x", fw_update_buff[0], fw_update_buff[1],
		fw_update_buff[2], fw_update_buff[3], fw_update_buff[4],
		fw_update_buff[5]);
	...
	GTP_DEBUG("fw bin ver:%x %x %x %x", gdev_fwu->fw_ver_in_bin[1],
		gdev_fwu->fw_ver_in_bin[0], gdev_fwu->fw_ver_in_bin[3],
		gdev_fwu->fw_ver_in_bin[2]);
...
void gcore_extern_notify_event(struct work_struct *work)
{
	if (gdev_fwu->ts_stat == TS_SUSPEND) {
		switch (gdev_fwu->headset_state) {
		case 1:
		...
		case 0:
			...
			break;
		default:
			GTP_ERROR("parameter error: %d", gdev_fwu->headset_state);
			break;
		}
	} else {
		switch (gdev_fwu->headset_state) {
...
void gcore_get_headset_state(int headset_state)
{
	gdev_fwu->headset_state = headset_state;
	queue_work(gdev_fwu->gtp_workqueue, &gdev_fwu->headset_work);
}
EXPORT_SYMBOL(gcore_get_headset_state);
 ...
 */
static ssize_t fw_mode_set_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	u32 mode = 0;
 ...
 */
static ssize_t fw_reg_rw_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	u32 addr = 0;
	u32 value = 0;
	...
	if (!strncmp(buf, "-w", 2)) {
		...
		if (!strncmp(buf, "adr=", 4) && (sscanf(buf, "adr=%x", &addr) == 1)) {
			...
			GTP_DEBUG("%02x %02x %02x %02x", r_buff[0], r_buff[1], r_buff[2],
				r_buff[3]);
		} else {
			GTP_ERROR("Wrong parameters");
			return -EPERM;
...
static ssize_t fw_update_flash_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%s\n", gdev_fwu->firmware_name);
}

static ssize_t fw_update_flash_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	int enabled = 0;
...
static ssize_t ret_update_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	if (g_ret_update) {
...
static ssize_t hostdl_idm_set_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	int enabled = 0;
...
static ssize_t sensitivity_set_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	int enabled = 0;
...
static ssize_t rawdata_xy_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int row, col;
...
static ssize_t selftest_show(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	int ret = -1;
	...
	char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "FW Ver:%x.%x.%x.%x\n", read_data[1],
		read_data[0], read_data[3], read_data[2]);
}
...
static ssize_t tp_function_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	u8 *func_buf[5] = {"off", "off", "off", "off", "off"};
	...
	return scnprintf(buf, PAGE_SIZE,
		"TP "
		"ESD:%s\nGESTURE_WAKEUP:%s\nPROXIMITY_TP_Function:%s\nMP_TEST:%s\nTP "
		"log print:%s\n",
		func_buf[0], func_buf[1], func_buf[2], func_buf[3], func_buf[4]);
}

static ssize_t notify_event_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	int enabled = 0;
...
static ssize_t set_rst_gpio_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (strncmp(buf, "RST", 4)) {
		...
		error = pinctrl_select_state(fn_data.gdev->pctrl,
			fn_data.gdev->pinctrl_state_reset_low);
		if (error < 0)
			GTP_ERROR("gcore_hw_reset: Set reset pin low state error:%d",
				error);
		msleep(10);
		error = pinctrl_select_state(fn_data.gdev->pctrl,
			fn_data.gdev->pinctrl_state_reset_high);
		if (error < 0)
			GTP_ERROR("gcore_hw_reset: Set reset pin high state error:%d",
				error);
		msleep(10);
#endif
	} else {
...
static ssize_t fw_update_force_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	int enabled = 0;
...
static ssize_t clear_point_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	int enabled = 0;
...
#if GCORE_MP_TEST_ON
static ssize_t debug_val_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int enabled = 0;
	...
	struct device_attribute *attr, char *buf)
{
	...
	error = snprintf(buf, 32, "%d -> %d\n", gcore_report_log_cfg,
		!gcore_report_log_cfg);
	gcore_report_log_cfg = !gcore_report_log_cfg;
...
static DEVICE_ATTR(page_reg, S_IRUSR | S_IWUSR, page_reg_rw_show,
	page_reg_rw_store);
...
static DEVICE_ATTR(fw_update, 0600, fw_update_flash_show,
	fw_update_flash_store);
...
#ifdef CONFIG_GCORE_AUTO_UPDATE_FW_FLASHDOWNLOAD
static DEVICE_ATTR(update_force, 0600, fw_update_force_show,
	fw_update_force_store);
#endif

static DEVICE_ATTR(fw_edge, 0600, gcore_fw_edge_protection_show,
	gcore_fw_edge_protection_store);
