...
static ssize_t gcore_app_read(struct file *file, char __user *buffer,
	size_t count, loff_t *ppos);
static long gcore_app_ioctl(struct file *filp, unsigned int cmd,
	unsigned long arg);
...
ssize_t gcore_app_read(struct file *file, char __user *buffer, size_t count,
	loff_t *ppos)
{
	int ret = 0;
...
ssize_t gcore_app_write(struct file *file, const char __user *buffer,
	size_t count, loff_t *ppos)
{
	return count;
}
...
long gcore_app_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case IOC_APP_READ_FW_VERSION:
		...
		GTP_DEBUG("app read data %x %x %x %x", read_data[0], read_data[1],
			read_data[2], read_data[3]);
		ret = snprintf(version, sizeof(version), "%d.%d.%d.%d", read_data[1],
	...
	case IOC_APP_UPDATE_FW:
		...
		GTP_DEBUG("fw_buf: %x %x %x %x", gdev->fw_ver_in_bin[0],
			gdev->fw_ver_in_bin[1], gdev->fw_ver_in_bin[2],
			gdev->fw_ver_in_bin[3]);
	...
	case IOC_APP_READ_REG:
		ret = copy_from_user(&msg_reg, (struct reg_msg __user *)arg,
			sizeof(msg_reg));
		if (ret) {
		...
		gcore_idm_read_page_reg(msg_reg.page, msg_reg.addr, msg_reg.buffer,
			msg_reg.length);
		usleep_range(1000, 2100);
		ENTER_IDM_CPU_RESET();
		usleep_range(1000, 2100);
		...
		GTP_DEBUG("read msg buffer %x %x", msg_reg.buffer[0],
			msg_reg.buffer[1]);
		ret = copy_to_user((struct reg_msg __user *)arg, &msg_reg,
			sizeof(msg_reg));
		if (ret)
			return -EPERM;
		break;
	...
	case IOC_APP_WRITE_REG:
		...
		gcore_enter_idm_mode();
		usleep_range(1000, 2100);
		gcore_idm_write_page_reg(msg_reg.page, msg_reg.addr, msg_reg.buffer,
			msg_reg.length);
		usleep_range(1000, 2100);
		ENTER_IDM_CPU_RESET();
		usleep_range(1000, 2100);
	...
	case IOC_APP_SWITCH_MODE:
		...
		ret = copy_from_user(&msg_mode, (struct mode_inf __user *)arg,
			sizeof(msg_mode));
		if (ret) {
			GTP_ERROR("WRITE REG:copy from user fail");
			return -EPERM;
		...
		GTP_DEBUG("get data form app mode:%d,len:%d", msg_mode.mode,
			msg_mode.packet_len);
		if (gcore_fw_mode_set_proc2(msg_mode.mode)) {
			GTP_ERROR("fw mode set demo rawdata proc fail");
			return -EPERM;
		}
		break;
	...
	case IOC_APP_GET_RAWDATA_RES:
		...
		snprintf(rawdata_res, sizeof(rawdata_res), "%d*%d", g_rawdata_row,
			g_rawdata_col);
	...
	case IOC_GET_FW_ALL_STATUS:
		...
		snprintf(fw_data, sizeof(fw_data), "%d,%x", get_fw_init_count(),
			status);
#else
	...
	case IOC_SET_FW_DEBUG_MODE:
		...
		GTP_DEBUG("set debug mode:%d,len:%d, %d, flag:%d", msg_mode.mode,
			gdev->fw_packet_len, msg_mode.packet_len, status);
		if (gcore_fw_mode_set_debug(msg_mode.mode, status)) {
...
{
	gcore_proc_entry =
		proc_create(GCORE_PROC_FILE, 0666, NULL, &gcore_app_fops);
	...
	gcore_mp_entry =
		proc_create(GCORE_MP_FILE, 0666, NULL, &gcore_selftest_fops);
...
static ssize_t gcore_tool_read(struct file *file, char __user *buffer,
	size_t count, loff_t *ppos);
static ssize_t gcore_tool_write(struct file *file, const char __user *buffer,
	size_t count, loff_t *ppos);
static long gcore_tool_ioctl(struct file *filp, unsigned int cmd,
	unsigned long arg);
...
ssize_t gcore_tool_read(struct file *file, char __user *buffer, size_t count,
	loff_t *ppos)
{
	struct gcore_dev *gdev = file->private_data;
	int ret = 0;
...
ssize_t gcore_tool_write(struct file *file, const char __user *buffer,
	size_t count, loff_t *ppos)
{
	int ret = 0;
...
int gcore_tool_node_init(void)
{
	...
	tool_info.gcore_dev = device_create(tool_info.gcore_class, NULL,
		tool_info.devno, NULL, "gcore");
...
int tpd_enable_ps(int enable)
{
	if (enable) {
		...
		GTP_DEBUG("TEL turn on PROXIMITY_TP_SCREEN_OFF,Ps_Enable:%s",
			gdev_intf->PS_Enale ? "true" : "false");
	} else {
		...
		GTP_DEBUG("TEL turn off PROXIMITY_TP_SCREEN_OFF,Ps_Enable:%s",
			gdev_intf->PS_Enale ? "true" : "false");
	}
	return 0;
}
...
int gcore_sys_node_init(void)
{
	...
	touchscreen_class_dev = device_create(touchscreen_class, NULL,
		MKDEV(MAJOR(tool_info.devno), 1), NULL, "ts_gcore");
...
static ssize_t tpfwupgrade_store(struct file *file, const char __user *buffer,
...
static int create_proc_gcorei2C_entry(void)
{
	...
	tpd_proc_entry = proc_create("FW_updata", 0664, gdev_intf->tpd_proc_dir,
		&proc_ops_tpfwupgrade);
