...
typedef enum {
	...
	HX_INSPECT_NOISE_FAIL,    /* Sensor noise test error */
...
static uint32_t hx_get_rawdata_func(int raw[], uint32_t datalen,  uint8_t checktype)
{
	...
	for (i = 0; i < datalen; i++) {
		...
		if (checktype == HX_WT_NOISE) {
			raw[i] = ((int8_t)tmp_rawdata[(i * 2) + 1] << 8) + tmp_rawdata[(i * 2)];
		} else {
...
static uint32_t mpTestFunc(uint8_t checktype, uint32_t datalen)
{
	...
	case HX_WT_NOISE:
		...
		if (noise_count > palm_num) {
			TS_LOG_ERR("%s: noise test noise_count > 70 FAIL\n", __func__);
			ret = HX_INSPECT_NOISE_FAIL;
			goto ERR_EXIT;
		}
...
static int hx_micro_open_test(int step)
{
	int retval = NO_ERR;
	TS_LOG_INFO("Step=%d,[MP_OPEN_TEST]\n", step);
	retval = mpTestFunc(HIMAX_INSPECTION_MICRO_OPEN, (HX_ZF_TX_NUM*HX_ZF_RX_NUM) + HX_ZF_TX_NUM + HX_ZF_RX_NUM);
	if (retval == 0)
		TS_LOG_INFO("%s: End --> PASS\n", __func__);
	else
		TS_LOG_INFO("%s: End --> FAIL\n", __func__);
...
int hx_zf_factory_start(struct himax_ts_data *ts, struct ts_rawdata_info *info_top)
{
	...
	if (IC_ZF_TYPE == HX_83108A_SERIES_PWON) {
		...
		hx_result_status[test6] = hx_micro_open_test(test6);
		hx_result_status[test5] = hx_short_test(test5);
		hx_result_status[test3] = hx_iir_test(test3);
	}
	//=============Show test result===================
	for (step = 1 ; step < MAX_TEST; step++) {
		if (hx_result_status[step] == 0) {
	...
	do {
		hx_reset_device();
		fw_update_status = hx_zf_fw_resume_update(himax_firmware_name);
		if (fw_update_status) {
