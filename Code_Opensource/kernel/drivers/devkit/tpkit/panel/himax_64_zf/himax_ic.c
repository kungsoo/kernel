...
static int g_hx_delta_coord_roi = 0;
static int g_hx_delta_coord = 0;
static int g_hx_delta = 0;
...
static int hx_horizon_switch(struct ts_horizon_info *info);
...
static int himax_roi_switch(struct ts_roi_info *info)
{
	...
	case TS_ACTION_READ:
		TS_LOG_INFO("TS_ACTION_READ Enter");
		hx_reg_assign(FW_ADDR_ROI_SWITCH, tmp_addr);
	...
	case TS_ACTION_WRITE:
		TS_LOG_INFO("TS_ACTION_WRITE Enter");
		if (!!info->roi_switch) {
			hx_addr_reg_assign(FW_ADDR_ROI_SWITCH, FW_ADDR_ROI_SWITCH_ON, tmp_addr, tmp_data);
			g_himax_zf_ts_data->roiflag = HX_ROI_ENABLE;
		} else {
			hx_addr_reg_assign(FW_ADDR_ROI_SWITCH, FW_ADDR_ROI_SWITCH_OFF, tmp_addr, tmp_data);
...
static bool hx_ic_package_check(void)
{
	...
	for (i = 0; i < retry; i++) {
		...
		if ((tmp_data[DATA_3] == HX_83108A_ID_PART_1) &&
			...
			((tmp_data[DATA_1] == HX_83108A_ID_PART_3) || (tmp_data[DATA_1] == HX_83108B_ID_PART_3))) {
			IC_ZF_TYPE = HX_83108A_SERIES_PWON;
			ret_data = IC_PACK_CHECK_SUCC;
			if (tmp_data[DATA_1] == HX_83108A_ID_PART_3)
				TS_LOG_INFO("Hx IC package 83108A series\n");
			else if (tmp_data[DATA_1] == HX_83108B_ID_PART_3)
				TS_LOG_INFO("Hx IC package 83108B series\n");
		} else {
...
static void ESD_HW_REST(void)
{
	...
	if (ret < 0)
		TS_LOG_INFO("ESD_HX TP: Wait LCD ESD - Reset failed\n");
}
...
static bool header_format_check(uint8_t *buf)
{
	return (buf[0] == 0xA3) && (buf[1] == 0x3A) &&
		(buf[106] != 0) && (buf[107] != 0);
}

static void hx_get_rawdata_coord_phalangeal_joint_form_event(int mul_num, int self_num,
	uint8_t *hx_delta_coord_roi)
{
	int i = 0;
	for (i = 0; i < (mul_num + self_num); i++) {
		if (i < mul_num) {
			mutual_data[i] = ((int8_t)hx_delta_coord_roi[i * 2 + 4 + 1]) * 256 +
				hx_delta_coord_roi[i * 2 + 4];
		} else { /* self */
			self_data[i-mul_num] = ((int8_t)hx_delta_coord_roi[i * 2 + 4 + 1]) * 256 +
				hx_delta_coord_roi[i * 2 + 4];
		}
	}
}

static int hx_start_get_rawdata_coord_phalangeal_joint_form_event(uint8_t *hx_delta_coord_roi)
{
	int mul_num = 0;
	int self_num = 0;
	int retval = NO_ERR;
	...
	/* header format check */
	if (hx_delta_coord_roi[0] == 0x5A && hx_delta_coord_roi[1] == 0xA5) {
		hx_get_rawdata_coord_phalangeal_joint_form_event(mul_num, self_num, hx_delta_coord_roi);
	} else {
	...
	if (g_process_full == 0x0c) {
		memcpy(&g_rb_frame.rawdata[update_idx].self[0], self_data, self_num * sizeof(int16_t));
		memcpy(&g_rb_frame.rawdata[update_idx].mutual[0], mutual_data, mul_num * sizeof(int16_t));
	} else if (g_process_full == 0x0F) {
		memcpy(&g_rb_frame.rawdata[update_idx].self[0], &hx_delta_coord_roi[4 + 2 * mul_num], 2 * self_num * sizeof(uint8_t));
		memcpy(&g_rb_frame.rawdata[update_idx].mutual[0], &hx_delta_coord_roi[4], 2 * mul_num * sizeof(uint8_t));
	}
	TS_LOG_INFO("%s: update_idx = %d, p_update = %d\n", __func__, update_idx, g_rb_frame.p_update);
...
static int hx_start_get_roi_data_from_event(uint8_t *buf, uint8_t *hx_delta_coord_roi, int diag_cmd)
{
	int i;
	int check_sum_cal;
	int retval = NO_ERR;

	if ((diag_cmd >= 1) && (diag_cmd <= 3) && (!g_process_type))
		hx_start_get_rawdata_coord_phalangeal_joint_form_event(hx_delta_coord_roi);

	if (!header_format_check(buf)) {
		TS_LOG_ERR("confirm the firmware format, header format is wrong!\n");
		retval = HX_ERROR;
		return retval;
	}

	for (i = 0, check_sum_cal = 0; i < (164 - 56); i = i + 2)
...
{
	int i = 0;
	uint32_t hx_eb_num = 0;
	uint32_t hx_ec_num = 0;
	uint32_t hx_ed_num = 0;
	uint32_t hx_zero_num = 0;
	...
	for (i = 0; i < hx_touch_info_size; i++) {
		if (buf[i] == ESD_EVENT_ALL_ZERO)
			hx_zero_num++;
		else if (buf[i] == ESD_EVENT_ALL_ED)
			hx_ed_num++;
		else if (buf[i] == ESD_EVENT_ALL_EB)
			hx_eb_num++;
		else if (buf[i] == ESD_EVENT_ALL_EC)
			hx_ec_num++;
	}

	if (hx_zero_num == hx_touch_info_size)
		TS_LOG_INFO("[HX TP MSG]: ESD event checked - ALL 0x00.\n");
	else if (hx_ed_num == hx_touch_info_size)
		TS_LOG_INFO("[HX TP MSG]: ESD event checked - ALL 0xED.\n");
	else if (hx_eb_num == hx_touch_info_size)
		TS_LOG_INFO("[HX TP MSG]: ESD event checked - ALL 0xEB.\n");
	else if (hx_ec_num == hx_touch_info_size)
		TS_LOG_INFO("[HX TP MSG]: ESD event checked - ALL 0xEC.\n");
	else
		TS_LOG_INFO("[HX TP MSG]  eventstack read 56byte data is different");

	return ESD_ALL_ZERO_BAK_VALUE;
}
#endif
...
static int himax_checksum_cal(int hx_touch_info_size, struct himax_touching_data hx_touching, uint8_t *buf)
{
	...
	for (hx_touching.loop_i = 0; hx_touching.loop_i < m; hx_touching.loop_i++) {
		...
		if (buf[hx_touching.loop_i] == 0x00)
			zero_cnt++;
	}

	if (zero_cnt == m) {
		TS_LOG_INFO("[HX TP MSG] all zero checksum ");
		checksum = HX_ERR;
	}
	...
	struct himax_touching_data hx_touching, uint8_t *buf)
{
	...
	if (hx_point_num != 0) {
		...
		for (hx_touching.loop_i = 0; hx_touching.loop_i < g_himax_zf_ts_data->nFinger_support; hx_touching.loop_i++) {
			...
			if (hx_touching.x >= 0 && hx_touching.x <= g_himax_zf_ts_data->pdata->abs_x_max &&
				hx_touching.y >= 0 && hx_touching.y <= g_himax_zf_ts_data->pdata->abs_y_max) {
		...
		for (hx_touching.loop_i = 0; hx_touching.loop_i < g_himax_zf_ts_data->nFinger_support; hx_touching.loop_i++) {
				if (((g_himax_zf_ts_data->pre_finger_mask >> hx_touching.loop_i) & 1) == 1) {
					...
					g_himax_zf_ts_data->pre_finger_data[hx_touching.loop_i][0] = 0;
					g_himax_zf_ts_data->pre_finger_data[hx_touching.loop_i][1] = 0;
					g_himax_zf_ts_data->pre_finger_data[hx_touching.loop_i][2] = 0;
				}
			}
...
static int hx_read_event_stack(uint8_t *buf, int length)
{
	uint8_t cmd[4] = {0};
	...
	himax_zf_bus_read(ADDR_READ_EVENT_STACK, buf, length, length, DEFAULT_RETRY_CNT);
...
void hx_log_touch_data(struct himax_ts_data *ts, uint8_t *buf)
{
	int i = 0;
	int print_size = 0;
	int diag_cmd = 0;
	...
	if (g_himax_zf_ts_data->roiflag && (diag_cmd == 0)) {
		print_size = 164;
	} else if (!g_himax_zf_ts_data->roiflag) {
		if ((diag_cmd >= 1) && (diag_cmd <= 3) && (!g_process_type)) {
				if (g_process_full == 0x0c || g_process_full == 0x0F)
					print_size = g_hx_delta_coord;
		}
	}  else if (g_himax_zf_ts_data->roiflag) {
		if ((diag_cmd >= 1) && (diag_cmd <= 3) && (!g_process_type)) {
				if (g_process_full == 0x0c || g_process_full == 0x0F)
					print_size = g_hx_delta_coord_roi;
		}
	} else {
		print_size = 128;
	}
	...
	for (i = 0; i < print_size; i += 8) {
		if ((i + 7) >= print_size) {
			TS_LOG_INFO(HX_DL_COORD_FORMAT, i, buf[i], i + 1, buf[i + 1],
				i + 2, buf[i + 2], i + 3, buf[i + 3]);
			break;
		}
		TS_LOG_INFO(HX_DL_COORD_FORMAT, i, buf[i], i + 1, buf[i + 1],
			i + 2, buf[i + 2], i + 3, buf[i + 3]);
		TS_LOG_INFO(HX_DL_COORD_FORMAT, i + 4, buf[i + 4], i + 5,
			buf[i + 5], i + 6, buf[i + 6], i + 7, buf[i + 7]);
	}
}

void hx_log_touch_event(struct himax_ts_data *ts, struct ts_fingers *info)
{
	int i;
	bool stylus_valid;

	/* finger log info */
	if (hx_real_point_num > 0) {
		for (i = 0; i < ts->nFinger_support; i++) {
			if (info->fingers[i].x >= 0 && info->fingers[i].x <= ts->pdata->abs_x_max
				&& info->fingers[i].y >= 0 && info->fingers[i].y <= ts->pdata->abs_y_max) {
				TS_LOG_INFO(PRT_LOG, i + 1, info->fingers[i].x, info->fingers[i].y, info->fingers[i].pressure);
			}
		}
	} else if (hx_real_point_num == 0) {
		TS_LOG_INFO("All Finger leave\n");
	} else {
		TS_LOG_INFO("%s : Finger wrong input!\n", __func__);
	}
}

void hx_ts_dbg_func(struct himax_ts_data *ts, uint8_t *buf, struct ts_fingers *info)
{
	if (ts->debug_log_level & BIT(0))
		hx_log_touch_data(ts, buf);

	if (ts->debug_log_level & BIT(1))
		hx_log_touch_event(ts, info);
}
...
static int hx_irq_bottom_half(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	...
	int fw_upgrade_err = RESULT_ERR;
	static int iCount;
	unsigned char check_sum_cal = 0;
	unsigned char check_event_state = 0;
	...
	uint8_t buf_roi[HX_RECEIVE_ROI_BUF_MAX_SIZE] = {0};
	...
	TS_LOG_INFO("%s:g_hx_delta_coord_roi = %d, g_hx_delta_coord = %d, g_hx_delta = %d\n",
		__func__, g_hx_delta_coord_roi, g_hx_delta_coord, g_hx_delta);

	memset(g_himax_zf_ts_data->hx_delta_coord_roi, 0x00, g_hx_delta_coord_roi);
	memset(g_himax_zf_ts_data->hx_delta_coord, 0x00, g_hx_delta_coord);
	...
	if (atomic_read(&g_himax_zf_ts_data->suspend_mode) && g_himax_zf_ts_data->tskit_himax_data->ts_platform_data->chip_data->easy_wakeup_info.sleep_mode) {
		...
		TS_LOG_INFO("%s: SMWP open Enter:\n", __func__);
		retval = hx_read_event_stack(buf, HX_RECEIVE_BUF_MAX_SIZE);
		if (retval < 0)
	...
	diag_cmd = hx_zf_getDiagCommand();
	if (!g_himax_zf_ts_data->roiflag) {
		if (diag_cmd && (!g_process_type)) {
			if (g_process_full == 0x0c || g_process_full == 0x0F) {
				TS_LOG_INFO("%s: 1 roi close diag = %d, stack read length = %d\n",
					__func__, diag_cmd, g_hx_delta_coord);
				/*diad cmd not 0, need to read 1256+56 */
				retval = hx_read_event_stack(g_himax_zf_ts_data->hx_delta_coord, g_hx_delta_coord);
				memcpy(buf, &g_himax_zf_ts_data->hx_delta_coord[g_hx_delta], hx_touch_info_size);
			} else {
				TS_LOG_INFO("%s: 2 roi close diag = %d, stack read length = %d\n", __func__,
					diag_cmd, HX_RECEIVE_BUF_MAX_SIZE);
				/* diad cmd not 0, need to read 128 */
				retval = hx_read_event_stack(buf, HX_RECEIVE_BUF_MAX_SIZE);
			}
		} else {
			retval = hx_read_event_stack(buf, hx_touch_info_size);/* 56byte */
			TS_LOG_INFO("%s: roi close diag = %d, stack read length = %d\n",
				__func__, diag_cmd, hx_touch_info_size);
		}
	} else {
		if (diag_cmd && (!g_process_type)) {

			if (g_process_full == 0x0c || g_process_full == 0x0F) {
				TS_LOG_INFO("%s: 3 roi open diag = %d, stack read length = %d\n",
					__func__, diag_cmd, g_hx_delta_coord_roi);
				/* diad cmd not 0, ROI data need to read 1420 byte */
				retval = hx_read_event_stack(g_himax_zf_ts_data->hx_delta_coord_roi, g_hx_delta_coord_roi);
				memcpy(buf, &g_himax_zf_ts_data->hx_delta_coord_roi[g_hx_delta], hx_touch_info_size);

				if ((g_himax_zf_ts_data->hx_delta_coord_roi[g_hx_delta + 56] == 0xA3) &&
					(g_himax_zf_ts_data->hx_delta_coord_roi[g_hx_delta + 56 + 1] == 0x3A)) {
					memcpy(g_himax_zf_ts_data->hx_rawdata_buf_roi,
						&g_himax_zf_ts_data->hx_delta_coord_roi[g_hx_delta_coord],
						HX_RECEIVE_ROI_BUF_MAX_SIZE - HX_TOUCH_SIZE);
				}
			} else {
				TS_LOG_INFO("%s: 4 roi open diag = %d, stack read length = %d\n",
					__func__, diag_cmd, HX_RECEIVE_BUF_MAX_SIZE);
			}
		} else {
			TS_LOG_INFO("%s: 5 roi open diag = %d, stack read length = %d\n", __func__,
				diag_cmd, HX_RECEIVE_ROI_BUF_MAX_SIZE);
			/* diad cmd not 0, ROI data need to read 164 byte */
			retval = hx_read_event_stack(g_himax_zf_ts_data->hx_coord_roi, 164);
			memcpy(buf, &g_himax_zf_ts_data->hx_coord_roi[0], hx_touch_info_size);

			if ((g_himax_zf_ts_data->hx_coord_roi[56] == 0xA3) &&
				(g_himax_zf_ts_data->hx_coord_roi[56 + 1] == 0x3A))
				memcpy(g_himax_zf_ts_data->hx_rawdata_buf_roi,
					&g_himax_zf_ts_data->hx_coord_roi[56], 164 - 56);
		}
	}
	...
	if (retval < 0) {
		...
		if (check_sum_cal != 0x00) {
			...
			check_event_state = hx_check_report_data_for_esd(hx_touch_info_size, buf);
		} else {
...
#ifdef HX_ESD_WORKAROUND
		diag_cmd = hx_zf_getDiagCommand();
		if (check_event_state != 0 && ESD_RESET_ACTIVATE == 0 && hw_reset_activate == 0
			&& diag_cmd == 0) { // ESD Check
			himax_zf_read_fw_status();
			if (check_event_state == ESD_ALL_ZERO_BAK_VALUE) {
				g_zero_event_count++;
				TS_LOG_INFO("ALL Zero event is %d times.\n", g_zero_event_count);

				if (g_zero_event_count == 1) {
					for (hx_touching.loop_i = 0;
						hx_touching.loop_i < g_himax_zf_ts_data->nFinger_support;
						hx_touching.loop_i++) {
						if (((g_himax_zf_ts_data->pre_finger_mask >> hx_touching.loop_i) & 1) == 1) {
							info->fingers[hx_touching.loop_i].status = TS_FINGER_PRESS;
							info->fingers[hx_touching.loop_i].x = g_himax_zf_ts_data->pre_finger_data[hx_touching.loop_i][0];
							info->fingers[hx_touching.loop_i].y = g_himax_zf_ts_data->pre_finger_data[hx_touching.loop_i][1];
							info->fingers[hx_touching.loop_i].major = 255;
							info->fingers[hx_touching.loop_i].minor = 255;

							if (g_himax_zf_ts_data->pre_finger_data[hx_touching.loop_i][0] == 0 &&
								g_himax_zf_ts_data->pre_finger_data[hx_touching.loop_i][1] == 0)
								info->fingers[hx_touching.loop_i].pressure = 0;
							else
								info->fingers[hx_touching.loop_i].pressure = 10;

							TS_LOG_INFO("info->fingers[%d].x = %d\n", hx_touching.loop_i, info->fingers[hx_touching.loop_i].x);
							TS_LOG_INFO("info->fingers[%d].y = %d\n", hx_touching.loop_i, info->fingers[hx_touching.loop_i].y);
						}
					}
				}

				if ((g_zero_event_count == ALL_ZERO_2_TIMES) || (g_zero_event_count == ALL_ZERO_3_TIMES)) {
					hx_reset_device();
					fw_upgrade_err = hx_zf_fw_resume_update(himax_firmware_name); /* request_firmware name */
					hx_resend_command();
				}

				if ((g_zero_event_count == ALL_ZERO_4_TIMES) || (fw_upgrade_err == UPDATE_FAIL)) {
					ESD_RESET_ACTIVATE = 1;
					TS_LOG_INFO("[HX TP MSG] FW upgrade error, wait lcd recovery !\n");
					ESD_HW_REST();
				}
			}
	...
	if (!g_himax_zf_ts_data->roiflag) {
		...
		if ((diag_cmd >= 1) && (diag_cmd <= 3) && (!g_process_type)) {
			if (g_process_full == 0x0c || g_process_full == 0x0F) {
				if (hx_start_get_rawdata_coord_phalangeal_joint_form_event(g_himax_zf_ts_data->hx_delta_coord) == HX_ERROR)
		...
		if ((diag_cmd >= 1) && (diag_cmd <= 3) && (!g_process_type)) {
			if (g_process_full == 0x0c || g_process_full == 0x0F) {
				if (hx_start_get_roi_data_from_event(g_himax_zf_ts_data->hx_rawdata_buf_roi, g_himax_zf_ts_data->hx_delta_coord_roi, diag_cmd) == HX_ERROR)
					goto bypass_checksum_failed_packet;
			}

		} else if (diag_cmd == 0) {
			if (hx_start_get_roi_data_from_event(g_himax_zf_ts_data->hx_rawdata_buf_roi, NULL, diag_cmd) == HX_ERROR)
				goto bypass_checksum_failed_packet;
		}
	}
...
bypass_checksum_failed_packet:
	...
	if (g_himax_zf_ts_data->roiflag && (diag_cmd == 0)) {
		memcpy(buf_roi, buf, 56);
		memcpy(&buf_roi[56], g_himax_zf_ts_data->hx_rawdata_buf_roi, 108);
		hx_ts_dbg_func(g_himax_zf_ts_data, buf_roi, info);
	}  else if (g_himax_zf_ts_data->roiflag) {
		if ((diag_cmd >= 1) && (diag_cmd <= 3) && (!g_process_type)) {
				if (g_process_full == 0x0c || g_process_full == 0x0F)
					hx_ts_dbg_func(g_himax_zf_ts_data, g_himax_zf_ts_data->hx_delta_coord_roi, info);
		}
	} else if (!g_himax_zf_ts_data->roiflag) {
		if ((diag_cmd >= 1) && (diag_cmd <= 3) && (!g_process_type)) {
			if (g_process_full == 0x0c || g_process_full == 0x0F)
				hx_ts_dbg_func(g_himax_zf_ts_data, g_himax_zf_ts_data->hx_delta_coord, info);
			else
				hx_ts_dbg_func(g_himax_zf_ts_data, buf, info);
		} else if (diag_cmd == 0) {
			hx_ts_dbg_func(g_himax_zf_ts_data, buf, info);
		}
	}
...
static int hx_parse_dts(struct device_node *device, struct ts_kit_device_data *chip_data)
{
	...
	retval = of_property_read_u32(device, "download_fw_inrecovery",
		&chip_data->download_fw_inrecovery);
	if (retval)
		TS_LOG_INFO("Not define download_fw_inrecovery in Dts!\n");

	retval = of_property_read_u32(device, "download_fw_incharger", &read_val);
	if (retval) {
		TS_LOG_INFO("Not define download_fw_incharger in Dts!\n");
	} else {
		chip_data->download_fw_incharger = (u8)read_val;
		TS_LOG_INFO("download_fw_incharger = %d\n", chip_data->download_fw_incharger);
	}
	...
	retval = of_property_read_u32(device, "horizon_change_supported", &read_val);
	if (retval) {
		read_val = NOT_SUPPORT;
		TS_LOG_INFO("NOT support to parse the roi dts!\n");
	}
	chip_data->ts_platform_data->feature_info.horizon_info.horizon_supported = (u8)read_val;
	TS_LOG_INFO("HXTP,horizon_supported = %d\n",
		chip_data->ts_platform_data->feature_info.horizon_info.horizon_supported);
...
static int hx_chip_detect(struct ts_kit_platform_data *platform_data)
{
	...
	tskit_project_id_interception(platform_data, himax_zf_project_id, TS_PROJECT_ID_LEN);
...
static int hx_init_chip(void)
{
	...
	g_hx_delta_coord_roi = (2 + HX_ZF_RX_NUM * HX_ZF_TX_NUM + HX_ZF_RX_NUM + HX_ZF_TX_NUM) * 2 + 56 + 108;
	g_hx_delta_coord = (2 + HX_ZF_RX_NUM * HX_ZF_TX_NUM + HX_ZF_RX_NUM + HX_ZF_TX_NUM) * 2 + 56;
	g_hx_delta = (2 + HX_ZF_RX_NUM * HX_ZF_TX_NUM + HX_ZF_RX_NUM + HX_ZF_TX_NUM) * 2;

	TS_LOG_INFO("%s:g_hx_delta_coord_roi = %d, g_hx_delta_coord = %d, g_hx_delta = %d\n", __func__,
		g_hx_delta_coord_roi, g_hx_delta_coord, g_hx_delta);
	...
	for (i = 0; i < HX_RB_FRAME_SIZE; i++) {
		g_rb_frame.rawdata[i].mutual = kzalloc(sizeof(uint16_t) *
			(HX_ZF_RX_NUM * HX_ZF_TX_NUM), GFP_KERNEL);
		g_rb_frame.rawdata[i].self = kzalloc(sizeof(uint16_t) *
			(HX_ZF_RX_NUM + HX_ZF_TX_NUM), GFP_KERNEL);
	}
#endif

	g_himax_zf_ts_data->hx_delta_coord = kzalloc(sizeof(uint8_t) *
		g_hx_delta_coord, GFP_KERNEL);
	if (g_himax_zf_ts_data->hx_delta_coord == NULL) {
		TS_LOG_ERR("Failed to alloc mem for hx_delta_coord\n");
		err =  -ENOMEM;
		goto mem_alloc_fail_delta_coord;
	}

	g_himax_zf_ts_data->hx_delta_coord_roi = kzalloc(sizeof(uint8_t) *
		g_hx_delta_coord_roi, GFP_KERNEL);
	if (g_himax_zf_ts_data->hx_delta_coord_roi == NULL) {
		TS_LOG_ERR("Failed to alloc mem for hx_delta_coord_roi\n");
		err =  -ENOMEM;
		goto mem_alloc_fail_delta_coord_roi;
	}

	g_himax_zf_ts_data->info_data = kzalloc(sizeof(uint8_t) *
		(MUTUL_NUM_HX83108A + SELF_NUM_HX83108A) * 2, GFP_KERNEL);
	if (g_himax_zf_ts_data->info_data == NULL) {
		TS_LOG_ERR("Failed to alloc mem for info_data\n");
		err =  -ENOMEM;
		goto mem_alloc_fail_info;
	}

	g_himax_zf_ts_data->hx_rawdata_buf_roi = kzalloc(sizeof(uint8_t) *
		(HX_RECEIVE_ROI_BUF_MAX_SIZE - HX_TOUCH_SIZE), GFP_KERNEL);
	if (g_himax_zf_ts_data->hx_rawdata_buf_roi == NULL) {
		TS_LOG_ERR("Failed to alloc hx_rawdata_buf_roi mem for hx_coord_roi\n");
		err =  -ENOMEM;
		goto mem_alloc_fail_rawdata_buf;
	}
	g_himax_zf_ts_data->hx_coord_roi = kzalloc(sizeof(uint8_t) *
		(HX_RECEIVE_ROI_BUF_MAX_SIZE), GFP_KERNEL);

	if (g_himax_zf_ts_data->hx_coord_roi == NULL) {
		TS_LOG_ERR("Failed to alloc mem for hx_coord_roi\n");
		err =  -ENOMEM;
		goto mem_alloc_fail_hx_coord_roi;
	}
...
mem_alloc_fail_hx_coord_roi:
	kfree(g_himax_zf_ts_data->hx_rawdata_buf_roi);
	g_himax_zf_ts_data->hx_rawdata_buf_roi = NULL;

mem_alloc_fail_rawdata_buf:
	kfree(g_himax_zf_ts_data->info_data);
	g_himax_zf_ts_data->info_data = NULL;

mem_alloc_fail_info:
	kfree(g_himax_zf_ts_data->hx_delta_coord_roi);
	g_himax_zf_ts_data->hx_delta_coord_roi = NULL;

mem_alloc_fail_delta_coord_roi:
	kfree(g_himax_zf_ts_data->hx_delta_coord);
	g_himax_zf_ts_data->hx_delta_coord = NULL;

mem_alloc_fail_delta_coord:
#ifdef HX_TP_SYS_DIAG
...
void hx_resend_command(void) {
	...
	TS_LOG_INFO("%s: horizon_supported = %d\n",
		__func__, info->horizon_info.horizon_supported);
	retval = hx_horizon_switch(&(info->horizon_info));
	if (retval < 0)
		TS_LOG_ERR("%s:Failed set horizon switch:%d, err:%d\n",
			__func__, info->horizon_info.horizon_switch, retval);
#endif
...
static int hx_after_resume(void *feature_info)
{
	...
	do {
		hx_reset_device();
		retval = hx_zf_fw_resume_update(himax_firmware_name);
		if (retval) {
			TS_LOG_INFO("%s: hx_zf_fw_resume_update ok!\n", __func__);
			break;
		}
		fw_update_retry++;
		TS_LOG_INFO("%s: fw_update_retry = %d\n", __func__, fw_update_retry);
	} while (fw_update_retry < FW_RETRY_TIMES);
	hx_resend_command();
	hx_zf_diag_node_set(0, 0);
		...
		int reload_length)
{
	...
	do {
		...
		if ((tmp_data[DATA_0] & HEX_NUM1) != HEX_NUM1) {
...
void hx_zf_get_dsram_data(uint8_t *info_data)
{
	...
	hx_addr_reg_assign(ADDR_DSRAM_START, DATA_DSRAM_START, tmp_addr, tmp_data);
	...
	for (i = 0; i < (total_read_times); i++) {
		if (total_size_temp >= max_spi_size) {
			...
			hx_zf_register_read(tmp_addr, total_size_temp % max_spi_size,
				&temp_info_data[i * max_spi_size]);
		}
	...
	if (zf_dsram_flag == false)	{
		...
		hx_addr_reg_assign(ADDR_DSRAM_START, DATA_INIT, tmp_addr, tmp_data);
		hx_zf_write_burst(tmp_addr, tmp_data);
	} else {
		TS_LOG_DEBUG("Continue to SRAM!\n");
		hx_addr_reg_assign(ADDR_DSRAM_START, DATA_RETURN_SRAM_EVENT, tmp_addr, tmp_data);
		hx_zf_write_burst(tmp_addr, tmp_data);
	}
	/* 5. Data Checksum Check */
	for (i = 2; i < total_size; i = i+2) {/* 2:PASSWORD NOT included */
		check_sum_cal += (temp_info_data[i + DATA_1] * SHIFT_NUM + temp_info_data[i]);
	}
	if (check_sum_cal % 0x10000 != 0) {
		...
		if (g_himax_zf_ts_data->debug_log_level & BIT(DATA_5)) {
			TS_LOG_DEBUG("%s skip checksum fail\n",
				__func__);
			memcpy(info_data, &temp_info_data[DATA_4], mutual_data_size * sizeof(uint8_t));
		}
		return;
	} else {
		memcpy(info_data, &temp_info_data[DATA_4], mutual_data_size * sizeof(uint8_t));
...
static int hx_horizon_switch(struct ts_horizon_info *info)
{
	...
	if (!info->horizon_supported) {
		TS_LOG_INFO("%s: horizon change is not supported, horizon_supported = %d",
			__func__, info->horizon_supported);
		return retval;
	}

	TS_LOG_INFO("%s: write_horizon_switch=%d\n", __func__, info->horizon_switch);
	if (!info->horizon_switch) {
		TS_LOG_INFO("%s: Vertical screen status 0xA55AA55A\n", __func__);
		hx_addr_reg_assign(ADDR_HORIZON_SWITCH_HIMAX, VERTICAL_SCREEN_STATUS,
			tmp_addr, tmp_data);
		himax_zf_register_write(tmp_addr, FOUR_BYTE_CMD, tmp_data);
	} else if (info->horizon_switch == HORIZON_ENABLE) {
		TS_LOG_INFO("%s: Horizontal screen status 0xA33AA33A\n", __func__);
		hx_addr_reg_assign(ADDR_HORIZON_SWITCH_HIMAX, HORIZONTAL_SCREEN_STATUS,
			tmp_addr, tmp_data);
		himax_zf_register_write(tmp_addr, FOUR_BYTE_CMD, tmp_data);
	} else {
		TS_LOG_INFO("%s: afferent screen status error!\n");
	}
	return retval;
}
...
struct ts_device_ops ts_kit_himax_zf_ops = {
	...
	.chip_horizon_switch = hx_horizon_switch,
#ifdef ROI
...
static int __init hx_module_init(void)
{
	...
	g_himax_zf_ts_data->tmp_buffer = kzalloc(sizeof(uint8_t) * MAX_DIAG_BUF_SIZE, GFP_KERNEL);

	if (g_himax_zf_ts_data->tmp_buffer == NULL) {
		TS_LOG_ERR("Failed to alloc mem for tmp_buffer\n");
		err =  -ENOMEM;
		goto mem_alloc_fail_tmp_buffer;
	}
	...
	return err;
out:
	kfree(g_himax_zf_ts_data->tmp_buffer);
	g_himax_zf_ts_data->tmp_buffer = NULL;

mem_alloc_fail_tmp_buffer:
