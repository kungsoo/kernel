...
static ssize_t tp_tddi_lowpower_process(struct device *dev,
	unsigned int status, size_t count, struct thp_core_data *cd)
{
	int ret;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cd->aod_support_on_tddi)
		return tp_tddi_lowpower_process(dev, status, count, cd);
