...
/* define HUAWEI_CHARGER_FB here to enable charger notify callback */
...
#define MAX_INVALID_ID_NUM 10
#define TS_PROJECT_ID_LEN 10
...
#define RESULT_INDEX_0 0
#define RESULT_INDEX_1 1
#define RESULT_INDEX_2 2
#define NUM_TWO 2
...
/* For NV STRUCTURE 0.5 interface */
...
#define I2C_MSG_NUM_ONE 1
#define I2C_MSG_NUM_TWO 2
...
#define DOUBLE_CHIP_INFO_LENGTH 32
...
#define TS_MT_MODE 2
#define TS_MT_MODE_ON (g_ts_kit_log_cfg == TS_MT_MODE)
...
#define TUI_DEVICE_NAME_LENGTH 11
#define FW_UPDATE_RESULT_LENGTH 8
#define ADV_WIDTH_LENGTH 4
#define FIRMWARE_NAME_PARTS 4
...
enum ts_pm_type {
	...
	TS_I2C_TO_AP = LCDKIT_TS_I2C_TO_AP,
	TS_I2C_TO_MCU = LCDKIT_TS_I2C_TO_MCU,
};
...
struct ts_tui_data {
	char device_name[TUI_DEVICE_NAME_LENGTH];
};
#endif
...
struct fw_param {
	char fw_name[MAX_STR_LEN * FIRMWARE_NAME_PARTS];
};
...
struct ts_chip_info_param {
	int status;
	u8 chip_name[DOUBLE_CHIP_INFO_LENGTH];
	u8 ic_vendor[DOUBLE_CHIP_INFO_LENGTH];
	u8 fw_vendor[DOUBLE_CHIP_INFO_LENGTH];
...
struct ts_dsm_info {
	char fw_update_result[FW_UPDATE_RESULT_LENGTH];
...
struct pirated_panel_info {
	u32 invalid_pid_count;
	u8 invalid_pid[MAX_INVALID_ID_NUM][TS_PROJECT_ID_LEN];
};
...
struct anti_false_touch_param {
	...
	/*
	 * if x > drv_stop_width and
	 * the same finger x < drv_stop_width, report
	 */
...
struct ts_kit_device_data {
	...
	unsigned char adv_width[ADV_WIDTH_LENGTH];
	...
	u32 download_fw_inrecovery; // 0 -- not support, 1 -- support
...
struct ts_kit_platform_data {
	...
	struct pirated_panel_info panel_info;
...
void ts_kit_anti_false_touch_param_achieve(
	struct ts_kit_device_data *chip_data);
