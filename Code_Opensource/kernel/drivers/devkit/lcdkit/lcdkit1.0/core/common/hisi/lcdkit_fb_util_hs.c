 ...
 * lcdkit_fb_util_hs.c
 *
 * lcdkit fb util hisi function for lcd driver
 *
...
static int g_alpm_setting_ret = LCDKIT_FAIL;
...
ssize_t effect_available_show(struct device* dev,
	struct device_attribute* attr, char* buf)
{
	int value;  // bit0:hiace; bit1:csc; bit2:bitextend; bit3:dither; bit4:arsr1p; bit5:sbl; bit6:acm; bit7:igm; bit8:xcc; bit9:gmp; bit10:gamma
	struct fb_info* fbi = NULL;
...
{
	if (!dev) {
		HISI_FB_ERR("alpm setting show NULL dev Pointer!\n");
		return -1;
	}

	if (!buf) {
		HISI_FB_ERR("alpm setting show NULL buf Pointer!\n");
		return -1;
	}

	return (ssize_t)snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "%d\n", g_alpm_setting_ret);
}
	...
	struct device_attribute* attr, const char* buf, size_t count)
{
	...
	g_alpm_setting_ret = LCDKIT_FAIL;
	if (!dev) {
	...
	if (lcdkit_info.panel_infos.watch_aod && !hisifd->aod_function) {
		LCDKIT_WARNING("aod function is 0 when aod cmd\n");
		if (cmd != AOD_EXIT_CMD) {
			LCDKIT_WARNING("aod function here set to 1\n");
			hisifd->aod_function = 1;
		}
	}
	if (!lcdkit_info.panel_infos.watch_aod) {
		down(&hisifd->blank_sem);
	} else {
		mutex_lock(&lcdkit_info.panel_infos.switch_lock);
		if (lcdkit_info.panel_infos.watch_aod_hybrid && cmd != ALPM_HYBRID_EXIT_CMD) {
			LCDKIT_INFO("[AOD]current is aod hybrid, do nothing!\n");
...
ssize_t display_idle_mode_show(struct device* dev,
	struct device_attribute* attr, char* buf)
{
	ssize_t ret;
	struct fb_info *fbi = NULL;
	struct hisi_fb_data_type *hisifd = NULL;
	if (!dev) {
		HISI_FB_ERR("display idle mode show NULL dev Pointer!\n");
		return -1;
	}

	if (!buf) {
		HISI_FB_ERR("display idle mode show NULL buf Pointer!\n");
		return -1;
	}

	fbi = dev_get_drvdata(dev);
	if (!fbi) {
		HISI_FB_ERR("display idle mode show NULL fbi Pointer!\n");
		return -1;
	}

	hisifd = (struct hisi_fb_data_type *)fbi->par;
	if (!hisifd) {
		HISI_FB_ERR("display idle mode show NULL hisifd Pointer!\n");
		return -1;
	}
	ret = snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, \
		"display_idle_mode = %d \n", hisifd->display_idle_mode);

	return ret;
}

ssize_t display_idle_mode_store(struct device* dev,
	struct device_attribute* attr, const char* buf)
{
	struct fb_info *fbi = NULL;
	struct hisi_fb_data_type *hisifd = NULL;
	...
	if (!dev) {
		HISI_FB_ERR("display idle mode store NULL dev Pointer!\n");
		return -1;
	}

	if (!buf) {
		HISI_FB_ERR("adisplay idle mode store NULL buf Pointer!\n");
		return -1;
	}

	fbi = dev_get_drvdata(dev);
	if (!fbi) {
		HISI_FB_ERR("display idle mode store NULL fbi Pointer!\n");
		return -1;
	}

	hisifd = (struct hisi_fb_data_type *)fbi->par;
	if (!hisifd) {
		HISI_FB_ERR("display idle mode store NULL hisifd Pointer!\n");
		return -1;
	}
	if (strlen(buf) >= MAX_BUF) {
		HISI_FB_ERR("buf overflow!\n");
	...
	ret = sscanf_s(buf, "%u", &hisifd->display_idle_mode);
	if (!ret) {
		HISI_FB_ERR("sscanf return invaild:%d\n", ret);
		return -1;
	}
	LCDKIT_INFO("display_idle_mode = %d !\n", hisifd->display_idle_mode);
	return ret;
}
