 ...
 * lcdkit_fb_util.c
 *
 * lcdkit fb util function for lcd driver
 *
 ...
 * input: read or write function
 * @pdata: this void point is used to converte to fb data struct.
 * output: get the result of this function
...
static ssize_t lcdkit_alpm_setting_show(struct device* dev,
		struct device_attribute* attr, char* buf)
{
	ssize_t ret = 0;
	struct lcdkit_panel_data* lcdkit_info;
	...
	if (lcdkit_info->panel_infos.aod_support) {
		ret = alpm_setting_show(dev, attr, buf);
		if (ret < 0)
			LCDKIT_ERR("alpm_setting_show return fail. \n");
	}
	return ret;
}
...
static ssize_t lcdkit_idle_mode_show(struct device* dev,
		struct device_attribute* attr, char* buf)
{
	ssize_t ret = 0;
	struct lcdkit_panel_data* lcdkit_info;
	...
	if (lcdkit_info->panel_infos.aod_support) {
		ret = display_idle_mode_show(dev, attr, buf);
		if (ret < 0)
			LCDKIT_ERR("display_idle_mode_show return fail. \n");
	}
	return ret;
}

static ssize_t lcdkit_idle_mode_store(struct device* dev,
	struct device_attribute* attr, const char* buf, size_t count)
{
	ssize_t ret;
	struct lcdkit_panel_data* lcdkit_info;
	...
	if (lcdkit_info->panel_infos.aod_support) {
		ret = display_idle_mode_store(dev, attr, buf);
		if (!ret) {
			LCDKIT_ERR("display_idle_mode_store return fail. \n");
			return -EINVAL;
		}
	}
	return count;
...
static DEVICE_ATTR(alpm_setting, 0644, lcdkit_alpm_setting_show, lcdkit_alpm_setting_store);
...
static DEVICE_ATTR(display_idle_mode, 0644, lcdkit_idle_mode_show, lcdkit_idle_mode_store);
