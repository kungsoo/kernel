...
bool g_watch_panel_status = false;
struct timer_list backlight_second_timer;
static unsigned int panel_version;
static bool backlight_off_flag = FALSE;
static bool backlight_init_status = TRUE;
...
static ssize_t tddi_aod_light_set(struct platform_device *pdev, uint32_t bl_level);

unsigned int lcdkit_get_tp_driver_type(void)
{
	return lcdkit_info.panel_infos.lcd_support_tp_driver_type;
}
...
{
	u32 delta_time_backlight_to_panel_on;
	u32 delay_margin;
	struct timeval tv;
	...
	delta_time_backlight_to_panel_on = (tv.tv_sec -
		lcdkit_info.panel_infos.dis_on_cmds_record_tv.tv_sec)*1000000
		+ tv.tv_usec - lcdkit_info.panel_infos.dis_on_cmds_record_tv.tv_usec;
	delta_time_backlight_to_panel_on /= 1000;
	if (delta_time_backlight_to_panel_on >=
...
static void lcdkit_bias_init(struct platform_device* pdev)
{
	...
	if (lcdkit_is_lcd_panel()) {
		if (lcdkit_bias_is_regulator_ctrl_power()) {
			...
			ret = vcc_cmds_tx(pdev, lcdkit_scharger_bias_get_cmds,
				ARRAY_SIZE(lcdkit_scharger_bias_get_cmds));
			if (ret != 0)
				LCDKIT_ERR("LCD scharger vcc get failed!\n");
			/* init bias/vsp/vsn */
			lcdkit_power_init(lcdkit_scharger_bias_set_cmds,
...
static ssize_t lcdkit_vcc_init(struct platform_device* pdev)
{
	...
	if (lcdkit_iovcc_is_regulator_ctrl_power()) {
		lcdkit_power_init(lcdkit_io_vcc_init_cmds,
			ARRAY_SIZE(lcdkit_io_vcc_init_cmds));
		/* init lcdio vcc */
		ret = vcc_cmds_tx(pdev, lcdkit_io_vcc_init_cmds,
			ARRAY_SIZE(lcdkit_io_vcc_init_cmds));
	...
	if (lcdkit_vdd_is_regulator_ctrl_power()) {
		lcdkit_power_init(lcdkit_vdd_init_cmds,
			ARRAY_SIZE(lcdkit_vdd_init_cmds));
		/* init lcdvdd vcc */
		ret = vcc_cmds_tx(pdev, lcdkit_vdd_init_cmds,
			ARRAY_SIZE(lcdkit_vdd_init_cmds));
	...
	if (HYBRID == g_tskit_ic_type || AMOLED == g_tskit_ic_type) {
		...
		if(lcdkit_vci_is_regulator_ctrl_power()) {
			ret = vcc_cmds_tx(pdev, lcdkit_vci_init_cmds,
				ARRAY_SIZE(lcdkit_vci_init_cmds));
			if (ret != 0) {
				LCDKIT_ERR("LCD vcc init failed!\n");
				return ret;
			}
...
static ssize_t lcdkit_pinctrl_init(struct platform_device* pdev)
{
	ssize_t ret ;
	// lcd pinctrl init
	ret = pinctrl_cmds_tx(pdev, lcdkit_pinctrl_init_cmds,
		ARRAY_SIZE(lcdkit_pinctrl_init_cmds));
	...
	if(tmp != NULL)
	{
		if(tmp->ic_type == BACKLIGHT_BIAS_IC)
		{
			...
			LCDKIT_INFO("backlight_bias_ic power ctrl mode is %d  bl gpio ctrl mode is %d!\n",
				lcdkit_info.panel_infos.lcd_backlight_power_ctrl_mode,
				lcdkit_info.panel_infos.lcd_bl_gpio_ctrl_mode);
...
static void lcdkit_backlight_ic_power_off(void)
{
	...
	if (tmp != NULL)
	{
		LCDKIT_INFO("backlight_ic_power_off  power ctrl mode is %d bl gpio ctrl mode is %d bl suspend disable is %d!\n",
			lcdkit_info.panel_infos.lcd_backlight_power_ctrl_mode,
			lcdkit_info.panel_infos.lcd_bl_gpio_ctrl_mode,
			lcdkit_info.panel_infos.lcd_suspend_bl_disable);
		lcdkit_backlight_ic_disable_device();
		if (lcdkit_info.panel_infos.lcd_bl_gpio_ctrl_mode)
		{
			if (lcdkit_info.panel_infos.lcd_suspend_bl_disable)
			{
				gpio_cmds_tx(lcdkit_bl_disable_cmds,
					ARRAY_SIZE(lcdkit_bl_disable_cmds));
				gpio_cmds_tx(lcdkit_bl_free_cmds,
					ARRAY_SIZE(lcdkit_bl_free_cmds));
			}
		}
		if (lcdkit_info.panel_infos.lcd_backlight_power_ctrl_mode)
		{
			gpio_cmds_tx(lcdkit_bl_power_disable_cmds,
				ARRAY_SIZE(lcdkit_bl_power_disable_cmds));
			gpio_cmds_tx(lcdkit_bl_power_free_cmds,
				ARRAY_SIZE(lcdkit_bl_power_free_cmds));
		}
	}
}
...
static void sharp_panel_read(void)
{
	...
	read_ret = mipi_dsi_lread_reg(read_value, read_reg, back_len,
		hisifd->mipi_dsi0_base);
	if (read_ret) {
	...
	if (read_data == 0x22) {
		strncpy(lcdkit_panel_name, "SHARP_2LANE_NT36870_VN1_5P88_1440P_SV3",
			strlen("SHARP_2LANE_NT36870_VN1_5P88_1440P_SV3"));
	} else {
		if (strlen(lcdkit_info.panel_infos.panel_name) <
			LCDKIT_MAX_PANEL_NAME_LEN) {
			strncpy(lcdkit_panel_name,lcdkit_info.panel_infos.panel_name,
				strlen(lcdkit_info.panel_infos.panel_name));
		} else {
			strncpy(lcdkit_panel_name,lcdkit_info.panel_infos.panel_name,
				(LCDKIT_MAX_PANEL_NAME_LEN - 1));
		}
	}
	return;
}
...
static int lcdkit_vcc_and_tp_power_on(struct platform_device* pdev)
{
	...
	else if (pinfo->lcd_init_step == LCD_INIT_MIPI_LP_SEND_SEQUENCE)
	{
		if (g_tskit_ic_type && lcdkit_info.panel_infos.ts_resume_ctrl_mode ==
			LCDKIT_TS_RESUME_BEFORE_LCD_RST) {
		...
		panel_name = lcdkit_info.panel_infos.panel_model ? \
			lcdkit_info.panel_infos.panel_model :
...
int lcdkit_set_lcd_reset_low(void)
{
	struct platform_device *pdev = NULL;
	struct hisi_fb_data_type *hisifd = NULL;

	if (!lcdkit_info.panel_infos.ts_control_lcd_reset_support)
		return LCDKIT_OK;
	...
	if (hisifd->panel_power_on)
		gpio_cmds_tx(lcdkit_gpio_reset_low_cmds,
			ARRAY_SIZE(lcdkit_gpio_reset_low_cmds));
	LCDKIT_INFO("lcdkit_set_lcd_reset_low is success\n");
	return LCDKIT_OK;
}
...
static void notify_ts_resume(void)
{
	int data_for_ts_resume;
	LCDKIT_INFO("call ts resume default\n");
	if (lcdkit_info.panel_infos.tp_resume_no_sync) {
...
static void lcdkit_on_preload(struct hisi_fb_data_type* hisifd)
{
	/* check status register if need do display_on_cmds */
	char read0a[] = { 0x0a };
	char read0e[] = {0x0e};
	uint32_t read_value = 0xff;
	uint32_t read_value0e = 0xff;
	struct lcdkit_dsi_cmd_desc check_reg_dsi_cmd = {
		DTYPE_DCS_READ, 1, 0, 0, 10, LCDKIT_WAIT_TYPE_US, 1, read0a
	};
	struct lcdkit_dsi_panel_cmds lcd_check_reg = {
		NULL, 0, &check_reg_dsi_cmd, 1, 0, 0
	};
	struct lcdkit_dsi_cmd_desc check_reg_dsi_cmd2 = {
		DTYPE_DCS_READ, 1, 0, 0, 10, LCDKIT_WAIT_TYPE_US, 1, read0e
	};
	struct lcdkit_dsi_panel_cmds lcd_check_reg2 = {
		NULL, 0, &check_reg_dsi_cmd2, 1, 0, 0
	};

	lcdkit_dsi_rx(hisifd, &read_value, 1, &lcd_check_reg);
	lcdkit_dsi_rx(hisifd, &read_value0e, 1, &lcd_check_reg2);
	LCDKIT_INFO("read_reg_value, 0a:0x%x, 0e:0x%x\n", read_value, read_value0e);

	if (read_value != 0x9c || read_value0e != 0x80) {
		LCDKIT_INFO("need lcdkit on cmd\n");
		lcdkit_info.lcdkit_on_cmd(hisifd,
			&lcdkit_info.panel_infos.display_on_cmds);
	} else {
		LCDKIT_INFO("do not need lcdkit on cmd\n");
	}
}

static int tddi_aod_lcdkit_on(struct platform_device* pdev)
{
	struct hisi_fb_data_type* hisifd = NULL;
	struct hisi_panel_info* pinfo = NULL;
	ssize_t ret = 0;
	...
	if (pinfo->lcd_init_step == LCD_INIT_POWER_ON) {
		...
		mdelay(15);
		LCDKIT_INFO("reset high low high! \n");
		gpio_cmds_tx(lcdkit_gpio_reset_normal_cmds, ARRAY_SIZE(lcdkit_gpio_reset_normal_cmds));
		if (is_lcdkit_initcode_enable()) {
			lcdkit_dsi_tx(hisifd, &lcdkit_info.panel_infos.g_panel_cmds);
		} else {
			LCDKIT_INFO("lcdkit_on_cmd! \n");
			lcdkit_info.lcdkit_on_cmd(hisifd, &lcdkit_info.panel_infos.display_on_cmds);
		}
		if (lcdkit_check_reg_on(hisifd))
			LCDKIT_ERR("0A register detect error!!!\n");
		pinfo->lcd_init_step = LCD_INIT_MIPI_HS_SEND_SEQUENCE;
	}
	else if (pinfo->lcd_init_step == LCD_INIT_MIPI_HS_SEND_SEQUENCE) {
		lcdkit_notifier_call_chain(LCDKIT_TS_AFTER_RESUME, &data_for_ts_after_resume);
		lcdkit_info.panel_infos.alpm_start_time = ktime_get();
	}
	else {
		LCDKIT_ERR("lcd_init_step failed!\n");
		ret = -EINVAL;
	}
	return 0;
}
...
static int lcdkit_on(struct platform_device* pdev)
{
	...
	if (lcdkit_info.panel_infos.tddi_aod && lcdkit_info.panel_infos.panel_power_state != POWER_OFF) {
		LCDKIT_INFO("It is AOD power on state, lcdkit_on bypass! \n");
		ret = tddi_aod_lcdkit_on(pdev);
		return ret;
	}

	g_backlight_count = BACKLIGHT_PRINT_TIMES;
	if (hisifd->aod_function && !lcdkit_info.panel_infos.mipi_aod_esd_switch && g_watch_panel_status) {
		LCDKIT_INFO("It is in AOD mode and should bypass lcdkit_on! \n");
		if (lcdkit_info.panel_infos.watch_aod && i2c_switch_ap() && !lcdkit_info.panel_infos.watch_aod_send_frame) {
			data_for_ts_resume = LCDKIT_NO_SYNC_TIMEOUT;
			lcdkit_notifier_call_chain(LCDKIT_TS_RESUME_DEVICE, &data_for_ts_resume);
			lcdkit_notifier_call_chain(LCDKIT_TS_AFTER_RESUME, &data_for_ts_after_resume);
		}
	...
	pinfo = &(hisifd->panel_info);
	if (lcdkit_panel_is_power_on(hisifd) == 1)
	...
	if (lcdkit_is_lcd_panel()) {
		...
		if (is_lcdkit_vsp_vsn_enable() &&
			lcdkit_bias_is_regulator_ctrl_power()) {
			lcdkit_debug_set_vsp_vsn(lcdkit_scharger_bias_set_cmds,
				ARRAY_SIZE(lcdkit_scharger_bias_set_cmds));
	...
	if (pinfo->lcd_init_step == LCD_INIT_POWER_ON) {
		...
		if (CHECKED_ERROR == lcdkit_btb_check())
			LCDKIT_ERR("lcdkit btb checked failed!");
		...
		if ((!lcdkit_is_enter_sleep_mode()) && (!g_lcd_prox_enable)) {
			...
			if (lcdkit_info.panel_infos.iovcc_before_vci == 1) {
				...
				if (lcdkit_iovcc_is_regulator_ctrl_power()) {
					...
					gpio_cmds_tx(lcdkit_iovcc_request_cmds,
						ARRAY_SIZE(lcdkit_iovcc_request_cmds));
					gpio_cmds_tx(lcdkit_iovcc_enable_cmds,
						ARRAY_SIZE(lcdkit_iovcc_enable_cmds));
				}
				...
				if (HYBRID == g_tskit_ic_type || AMOLED == g_tskit_ic_type) {
					if(lcdkit_vci_is_regulator_ctrl_power()) {
						...
						gpio_cmds_tx(lcdkit_vci_request_cmds,
							ARRAY_SIZE(lcdkit_vci_request_cmds));
						gpio_cmds_tx(lcdkit_vci_enable_cmds,
							ARRAY_SIZE(lcdkit_vci_enable_cmds));
					}
				}
			...
			if (lcdkit_vbat_is_gpio_ctrl_power()) {
				gpio_cmds_tx(lcdkit_vbat_request_cmds,
					ARRAY_SIZE(lcdkit_vbat_request_cmds));
			...
			if (lcdkit_bias_is_gpio_ctrl_power()) {
				...
				if (lcdkit_info.panel_infos.vsp_on_is_need_reset) {
					gpio_cmds_tx(&lcdkit_bias_request_cmds[0], 1);
					gpio_cmds_tx(&lcdkit_bias_enable_cmds[0], 1);

					if (lcdkit_info.panel_infos.reset_pull_high_flag == 1) {
						// reset just pull high
					...
					gpio_cmds_tx(&lcdkit_bias_request_cmds[1], 1);
					gpio_cmds_tx(&lcdkit_bias_enable_cmds[1], 1);

					if (lcdkit_info.panel_infos.reset_low_after_vsn_on)
						gpio_cmds_tx(lcdkit_gpio_reset_low_cmds, \
							ARRAY_SIZE(lcdkit_gpio_reset_low_cmds));
				} else {
					gpio_cmds_tx(lcdkit_bias_request_cmds, \
			...
			if (lcdkit_info.panel_infos.idle2_lcd_reset_low) {
				if (g_tskit_ic_type &&
					lcdkit_info.panel_infos.idle2_lcd_reset_low ==
					LCDKIT_TS_RESUME_BEFORE_LCD_RST) {
				...
				// Enter FT8201 gesture wake-up mode
				if (lcdkit_info.panel_infos.tp_gesture_sequence_flag) {
					LCDKIT_INFO("Enter FT8201 gesture wake-up mode\n");
					// tp reset low
					gpio_cmds_tx(lcdkit_tp_rst_gesture_low_cmds, \
						ARRAY_SIZE(lcdkit_tp_rst_gesture_low_cmds));
					// vsp vsn reset low
					gpio_cmds_tx(lcdkit_bias_disable_cmds, \
						ARRAY_SIZE(lcdkit_bias_disable_cmds));
					// lcd rest low
					gpio_cmds_tx(lcdkit_lcd_rst_gesture_low_cmds, \
						ARRAY_SIZE(lcdkit_lcd_rst_gesture_low_cmds));
					// vsp vsn reset high
					gpio_cmds_tx(lcdkit_bias_enable_cmds, \
						ARRAY_SIZE(lcdkit_bias_enable_cmds));
					// tp lcd reset high
					gpio_cmds_tx(lcdkit_tp_lcd_rst_gesture_high_cmds, \
						ARRAY_SIZE(lcdkit_tp_lcd_rst_gesture_high_cmds));
					// tp lcd reset HLH
					gpio_cmds_tx(lcdkit_tp_lcd_rst_gesture_hlh_cmds, \
						ARRAY_SIZE(lcdkit_tp_lcd_rst_gesture_hlh_cmds));
				} else {
					gpio_cmds_tx(lcdkit_gpio_reset_high_cmds, \
						ARRAY_SIZE(lcdkit_gpio_reset_high_cmds));
				}
		...
		if (pinfo->bl_ic_ctrl_mode == COMMON_IC_MODE
			&& !lcdkit_info.panel_infos.bl_poweron_after_dsi_cmd)
...
lp_sequence_restart:
		...
		if (lcdkit_info.panel_infos.second_reset) {
			if (lcdkit_info.panel_infos.reset_pull_high_flag == 1) {
				...
				LCDKIT_INFO("prox_not_need_lcd_reset = %d\n",
					lcdkit_info.panel_infos.prox_not_need_lcd_reset);
				if (!(g_lcd_prox_enable &&
					lcdkit_info.panel_infos.prox_not_need_lcd_reset))
					gpio_cmds_tx(lcdkit_gpio_reset_normal_cmds,
						ARRAY_SIZE(lcdkit_gpio_reset_normal_cmds));

				if (g_lcd_prox_enable &&
					lcdkit_info.panel_infos.delay_af_reset_prox) {
					LCDKIT_INFO("prox need lcd reset delay\n");
					mdelay(lcdkit_info.panel_infos.delay_af_reset_prox);
				}
			}
		}
		...
		if (g_tskit_ic_type && !lcdkit_info.panel_infos.ts_resume_ctrl_mode) {
			if (lcdkit_info.panel_infos.mipi_switch_support) {
				if (i2c_switch_ap())
					notify_ts_resume();
			} else {
				notify_ts_resume();
		...
		if (runmode_is_factory()) {
			if (!strncmp(lcdkit_info.panel_infos.panel_name,
				"JDI_NT35696 5.1' CMD TFT 1920 x 1080",
				strlen(lcdkit_info.panel_infos.panel_name))) {
			...
			if(lcdkit_info.panel_infos.dynamic_gamma_support) {
				if ((!strncmp(lcdkit_info.panel_infos.panel_name,
					"JDI_2LANE_NT36860 5.88' CMD TFT 1440 x 2560",
					strlen(lcdkit_info.panel_infos.panel_name)))||
					(!strncmp(lcdkit_info.panel_infos.panel_name,
					"JDI_2LANE_NT36860 CUT2 5.88' CMD TFT 1440 x 2560",
					strlen(lcdkit_info.panel_infos.panel_name)))||
					(!strncmp(lcdkit_info.panel_infos.panel_name,
					"JDI_NT36860 5.88' CMD TFT 1440 x 2560",
					strlen(lcdkit_info.panel_infos.panel_name)))) {
		...
		if (is_lcdkit_initcode_enable()) {
			...
			if (lcdkit_info.panel_infos.mipi_switch_support) {
				mipi_dsi_ulps_cfg(hisifd, 0); // enter ulps
				mdelay(1);
				mipi_to_ap(hisifd);
				mdelay(1);
				...
				lcdkit_on_preload(hisifd);
				/* skip base frame when lcd on */
				hisifd->base_frame_mode = 1;
			} else {
				lcdkit_info.lcdkit_on_cmd(hisifd,
					&lcdkit_info.panel_infos.display_on_cmds);
			}
		...
		if (lcdkit_info.panel_infos.dsi_upt_snd_cmd_support) {
			ret = find_cmd_by_mipi_clk(hisifd, pinfo->mipi.dsi_bit_clk_upt,
				&snd_mipi_clk_cmds);
			if (ret == 0)
		...
		if (lcdkit_info.panel_infos.display_effect_on_support) {
			...
			lcdkit_dsi_tx(hisifd,
				&lcdkit_info.panel_infos.display_effect_on_cmds);
		}
		...
		if (g_tskit_ic_type && !lcdkit_info.panel_infos.tprst_before_lcdrst &&
			!lcdkit_info.panel_infos.tpfw_early_lcdinit) {
			if (lcdkit_info.panel_infos.mipi_switch_support) {
				if (get_i2c_switch_value())
					lcdkit_notifier_call_chain(LCDKIT_TS_AFTER_RESUME,
						&data_for_ts_after_resume);
			} else {
				lcdkit_notifier_call_chain(LCDKIT_TS_AFTER_RESUME, &data_for_ts_after_resume);
		...
		lcdkit_info.panel_infos.mipi_aod_esd_switch = 0;
	} else {
...
static void lcdkit_switch_to_mcu(struct hisi_fb_data_type *hisifd)
{
	if (!lcdkit_info.panel_infos.mipi_switch_support)
		return;
	if (get_watch_recovery_state())
		return;
	mipi_dsi_ulps_cfg(hisifd, 0); // enter ulps
	mdelay(1);
	mipi_to_mcu(hisifd);
...
static int tddi_aod_lcdkit_off(struct platform_device* pdev)
{
	struct hisi_fb_data_type* hisifd = NULL;
	struct hisi_panel_info* pinfo = NULL;
	ssize_t ret = 0;
	int notify_tp_early_suspend = LCDKIT_SHORT_SYNC_TIMEOUT;
	int notify_tp_suspend = LCDKIT_NO_SYNC_TIMEOUT;
	...
	if (pinfo->lcd_uninit_step == LCD_UNINIT_MIPI_HS_SEND_SEQUENCE) {
		tddi_aod_light_set(pdev, lcdkit_info.panel_infos.tddi_aod_no_light);
		if (hisifd->display_idle_mode && lcdkit_info.panel_infos.tddi_aod)
			lcdkit_notifier_call_chain(LCDKIT_TS_TDDI_AOD_OFF, NULL);
		lcdkit_notifier_call_chain(LCDKIT_TS_EARLY_SUSPEND, NULL);
		msleep(lcdkit_info.panel_infos.delay_af_tp_before_suspend);
		LCDKIT_INFO("lcdkit_off_cmd! \n");
		...
		lcdkit_notifier_call_chain(LCDKIT_TS_BEFORE_SUSPEND, &notify_tp_early_suspend);
		lcdkit_notifier_call_chain(LCDKIT_TS_SUSPEND_DEVICE, &notify_tp_suspend);
		pinfo->lcd_uninit_step = LCD_UNINIT_POWER_OFF;
	}
	else if (pinfo->lcd_uninit_step == LCD_UNINIT_POWER_OFF) {
		msleep(5);
		LCDKIT_INFO("It is AOD mode, skip reset bias iovcc setting! \n");
	}
	else {
		LCDKIT_ERR("lcd_uninit_step failed!\n");
		ret = -EINVAL;
	}
	return ret;
}
...
static int lcdkit_off(struct platform_device* pdev)
{
	...
	if (hisifd->display_idle_mode && lcdkit_info.panel_infos.tddi_aod) {
		LCDKIT_INFO("It is in tddi AOD mode, lcdkit_off bypass! \n");
		ret = tddi_aod_lcdkit_off(pdev);
		return ret;
	}

	/* reset base_frame_mode to original */
	hisifd->base_frame_mode = 0;
	/* mipi_aod_esd_switch default 0 */
	if (hisifd->aod_function && !lcdkit_info.panel_infos.mipi_aod_esd_switch) {
	...
	if (pinfo->lcd_uninit_step == LCD_UNINIT_MIPI_HS_SEND_SEQUENCE) {
		...
		lcdkit_info.lcdkit_off_cmd(hisifd,
		...
		if (!lcdkit_info.panel_infos.rst_after_vbat_flag) {
			if (g_tskit_ic_type &&
				!g_lcdkit_pri_info.power_off_simult_support &&
				!lcdkit_info.panel_infos.tp_before_lcdsleep) {
				/* notify early suspend */
				lcdkit_notifier_call_chain(LCDKIT_TS_BEFORE_SUSPEND,
					&data_for_notify_early_suspend);
				/* notify suspend */
	...
	if ((!lcdkit_get_proxmity_enable()) && (!lcdkit_is_enter_sleep_mode())) {
		...
		if (!lcdkit_info.panel_infos.reset_shutdown_later) {
			if (!lcdkit_info.panel_infos.power_off_not_need_reset) {
				// lcd reset gpio low
				gpio_cmds_tx(lcdkit_gpio_reset_low_cmds, \
					ARRAY_SIZE(lcdkit_gpio_reset_low_cmds));
				msleep(lcdkit_info.panel_infos.delay_af_rst_off);
		...
		if (g_lcdkit_pri_info.power_off_simult_support) {
			if (!lcdkit_info.panel_infos.power_iovcc_not_need_low) {
				if (lcdkit_iovcc_is_regulator_ctrl_power()) {
					vcc_cmds_tx(pdev, lcdkit_io_vcc_disable_cmds,
						ARRAY_SIZE(lcdkit_io_vcc_disable_cmds));
				} else if (lcdkit_iovcc_is_gpio_ctrl_power()) {
		...
		if (lcdkit_info.panel_infos.reset_shutdown_later) {
			...
			if ((g_tskit_ic_type &&
				g_lcdkit_pri_info.power_off_simult_support) ||
				(g_tskit_ic_type && lcdkit_info.panel_infos.rst_after_vbat_flag)) {
				/* notify early suspend */
				lcdkit_notifier_call_chain(LCDKIT_TS_BEFORE_SUSPEND,
					&data_for_notify_early_suspend);
				/* notify suspend */
		...
		if (HYBRID ==  g_tskit_ic_type || AMOLED == g_tskit_ic_type ) {
			if (lcdkit_vci_is_regulator_ctrl_power()) {
				...
				gpio_cmds_tx(lcdkit_vci_free_cmds,
					ARRAY_SIZE(lcdkit_vci_free_cmds));
			}
		}

		msleep(lcdkit_info.panel_infos.delay_af_vci_off);
		if (pinfo->bl_ic_ctrl_mode == COMMON_IC_MODE &&
				!lcdkit_info.panel_infos.bl_disable_before_lcd_suspend)
			lcdkit_backlight_ic_power_off();
	} else
	{
		if (g_tskit_ic_type && lcdkit_info.panel_infos.tp_after_lcd_reset)
...
static int lcdkit_set_backlight(struct platform_device* pdev, uint32_t bl_level)
{
	...
	if (hisifd->display_idle_mode && lcdkit_info.panel_infos.tddi_aod) {
		LCDKIT_INFO("It is in tddi AOD mode, lcdkit_set_backlight bypass!\n");
		return 0;
	}

	if (hisifd->aod_mode && hisifd->aod_function && !lcdkit_info.panel_infos.mipi_aod_esd_switch && g_watch_panel_status) {
		LCDKIT_INFO("It is in AOD mode and should bypass lcdkit_set_backlight!\n");
		return 0;
	}
	if (lcdkit_info.panel_infos.watch_aod_hybrid) {
		LCDKIT_INFO("It is in hybrid AOD mode and should bypass lcdkit_set_backlight!\n");
		return 0;
	}
	g_watch_panel_status = true;
	if (lcdkit_info.panel_infos.bl_is_shield_backlight == true) {
	...
	if (g_backlight_count) {
		...
		g_backlight_count = (g_backlight_count > 0) ?
			(g_backlight_count - 1) : 0;
	}
	...
	if (jank_last_bl_level == 0 && bl_level != 0) {
		...
		if ((backlight_init_status == FALSE) &&
			lcdkit_info.panel_infos.bl_poweron_after_dsi_cmd)
			lcdkit_backlight_ic_power_on();
	...
	if (hisifd->panel_info.bl_set_type & BL_SET_BY_PWM) {
		...
		if((pinfo->rgbw_support) && (hisifd->backlight.bl_level_old == 0) &&
			(bl_level!=0) &&
			(lcdkit_info.panel_infos.rgbw_backlight_cmds.cmds) &&
			(lcdkit_info.panel_infos.rgbw_backlight_cmds.cmd_cnt>0)) {
			bl_level = (bl_level < hisifd->panel_info.bl_max) ? bl_level :
				hisifd->panel_info.bl_max;
			reg_value_61h = bl_level * 4095 / hisifd->panel_info.bl_max;
			lcdkit_info.panel_infos.rgbw_backlight_cmds.cmds[0].payload[1] =
				(reg_value_61h >> 8) & 0x0f;
			lcdkit_info.panel_infos.rgbw_backlight_cmds.cmds[0].payload[2] =
				reg_value_61h & 0xff;
			LCDKIT_PANEL_CMD_REQUEST();
			hisifb_activate_vsync(hisifd);
		...
		(hisifd->panel_info.bl_max > 0)) {
		bl_level = (bl_level < hisifd->panel_info.bl_max) ?
			bl_level : hisifd->panel_info.bl_max;
		hisifb_display_effect_fine_tune_backlight(hisifd, (int)bl_level,
			(int *)&bl_level);
		if (lcdkit_info.panel_infos.backlight_cmds.cmds[0].dlen == 2) {
			lcdkit_info.panel_infos.backlight_cmds.cmds[0].payload[1] =
				bl_level  * 255 / hisifd->panel_info.bl_max;
		} else {
			if (hisifd->panel_info.bl_otm) {
				lcdkit_info.panel_infos.backlight_cmds.cmds[0].payload[1] =
					(bl_level >> 2) & 0xff;  // high 8bit
				lcdkit_info.panel_infos.backlight_cmds.cmds[0].payload[2] =
...
static int lcdkit_enter_hbm_fp(struct hisi_fb_data_type *hisifd)
{
	...
	if(is_mipi_video_panel(hisifd)) {
		if (lcdkit_info.panel_infos.panel_highspeed_support)
			set_reg(hisifd->dss_base + DSS_MIPI_DSI0_OFFSET +
				MIPIDSI_VID_MODE_CFG_OFFSET, 0x0, 1, 15);
	}
	...
	if(is_mipi_video_panel(hisifd)) {
		if (lcdkit_info.panel_infos.panel_highspeed_support)
			set_reg(hisifd->dss_base + DSS_MIPI_DSI0_OFFSET +
				MIPIDSI_VID_MODE_CFG_OFFSET, 0x1, 1, 15);
	}
	return ret;
}
...
static int lcdkit_esd_check(struct platform_device* pdev)
{
	...
	if (g_lcdkit_pri_info.platform_esd_support) {
		temp = inp32(hisifd->mipi_dsi0_base +
			g_lcdkit_pri_info.platform_esd_reg);
	...
	if (g_lcdkit_pri_info.esd_support) {
		...
		for (i = 0; i < g_lcdkit_pri_info.esd_cmds.cmd_cnt; i++) {
			...
			ret = mipi_dsi_lread_reg(&read_value, &dsi_cmd,
				lcdkit_dsi_cmd->dlen, hisifd->mipi_dsi0_base);
			if (!ret) {
				for (j = 0; j < lcdkit_dsi_cmd->dlen; j++) {
					...
					case 0:
						if (tmp_value &
							g_lcdkit_pri_info.esd_value.buf[count++])
					...
					default:
						LCDKIT_ERR("[ESD] error esd-expect-value-type = %d, line = %d\n",
							hisifd->panel_info.esd_expect_value_type,
							__LINE__);
						break;
					}
					if (esd_exception_flags) {
						...
						LCDKIT_ERR("esd abnormal read reg:0x%x = 0x%x\n",
							g_lcdkit_pri_info.esd_cmds.cmds[i].payload[0],
							read_value);
						LCDKIT_PANEL_CMD_RELEASE();
#ifdef CONFIG_HUAWEI_THP
						if (lcdkit_info.panel_infos.send_tp_esd_event) {
							ret = thp_send_esd_event(GPIO_CHECK_TIMES);
							if (ret)
...
static int lcdkit_set_fastboot(struct platform_device* pdev)
{
	...
	if(lcdkit_info.panel_infos.get_blmaxnit_type == GET_BLMAXNIT_FROM_DDIC){
		...
		lcdkit_dsi_rx(hisifd, &lcdkit_brightness_ddic_info, 1,
			&lcdkit_info.panel_infos.bl_maxnit_cmds);
...
static int lcdkit_set_display_region(struct platform_device* pdev,
...
static void aod_close_te(struct hisi_fb_data_type *hisifd)
{
	char reg[2] = { 0x34, 0x00 };  // cmd len is 2
	struct dsi_cmd_desc lcd_reg_cmds[] = {
		{ DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_US, sizeof(reg), reg },
	};
	LCDKIT_PANEL_CMD_REQUEST();
	mipi_dsi_cmds_tx(lcd_reg_cmds, \
		ARRAY_SIZE(lcd_reg_cmds), hisifd->mipi_dsi0_base);
	LCDKIT_PANEL_CMD_RELEASE();
}

static void aod_open_te(struct hisi_fb_data_type *hisifd)
{
	char reg[2] = { 0x35, 0x00 };  // cmd len is 2
	struct dsi_cmd_desc lcd_reg_cmds[] = {
		{ DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_US, sizeof(reg), reg },
	};
	LCDKIT_PANEL_CMD_REQUEST();
	...
	read_watch_reg_value(hisifd);
}

static void lcdkit_aod_on(struct platform_device *pdev,
	struct lcdkit_dsi_panel_cmds* cmds, uint32_t bl_level)
{
	struct hisi_fb_data_type *hisifd = NULL;
	...
	LCDKIT_INFO("[AOD]enter ALPM low brightness mode and set cmd display_cmd(0x29)!\n");
	LCDKIT_PANEL_CMD_REQUEST();
	hisifd->panel_info.esd_enable = 0;
	hisifd->aod_mode = 0;
	lcdkit_dsi_tx(hisifd, &lcdkit_info.panel_infos.panel_enter_aod_cmds);
	lcdkit_dsi_tx(hisifd, cmds);
	LCDKIT_PANEL_CMD_RELEASE();
	lcdkit_set_backlight(pdev, bl_level);
	aod_close_te(hisifd);
	hisifd->aod_mode = 1; // enter aod mode
	lcdkit_info.panel_infos.mipi_aod_esd_switch = 0;
	...
	if (hisi_fb_blank_sub(FB_BLANK_POWERDOWN, hisifd->fbi))
		LCDKIT_ERR("panel down error!\n");
}

static void exit_watch_aod(struct hisi_fb_data_type *hisifd)
{
	int data_for_ts_resume = LCDKIT_SHORT_SYNC_TIMEOUT;
	int data_for_ts_after_resume = LCDKIT_NO_SYNC_TIMEOUT;
	char playload[1] = { 0x29 }; // display on cmd
	struct lcdkit_dsi_cmd_desc cmds = { DTYPE_DCS_WRITE, 0, 0, 0, 100, 0, 1, playload };
	struct lcdkit_dsi_panel_cmds display_on_cmd = { NULL, 0, &cmds, 1, 0, 0 };
	...
	LCDKIT_PANEL_CMD_RELEASE();
	hisifd->aod_mode = 0; // exit aod mode
#if defined(CONFIG_DPU_FB_AP_AOD)
	hisifd->ap_aod_skip_frame_flag = 0;
#endif
...
static ssize_t lcdkit_watch_aod_alpm(struct platform_device *pdev, const char *buf, size_t count)
{
	struct hisi_fb_data_type *hisifd = NULL;
	int ret;
	unsigned int cmd = 0;
	int bl_level;
	char playload2[1] = { 0x29 }; //display on cmd
	...
	switch (cmd) {
	case AOD_EXIT_CMD:
		exit_watch_aod(hisifd);
		break;
	case AOD_WATCH_NORMAL_BRIGHTNESS:
		bl_level = lcdkit_info.panel_infos.watch_aod_normal_brightness;
		lcdkit_aod_on(pdev, &display_on_cmd, bl_level);
		break;

	case AOD_SEND_FRAME_CMD:
	...
	case AOD_WATCH_LOW_BRIGHTNESS:
		bl_level = lcdkit_info.panel_infos.watch_aod_low_brightness;
		lcdkit_aod_on(pdev, &display_on_cmd, bl_level);
		break;
	case ALPM_HYBRID_CMD:
#if defined(CONFIG_DPU_FB_AP_AOD)
		hisifd->ap_aod_skip_frame_flag = 1;
#endif
		lcdkit_info.panel_infos.watch_aod_hybrid = 1;
		LCDKIT_INFO("[AOD] here to wait play end\n");
		/* wait for two frame */
		mdelay(34);
		...
		if (hisifd->panel_power_on == true) {
			hisifb_activate_vsync(hisifd);
			mipi_dsi_ulps_cfg(hisifd, 0); // enter ulps
			LCDKIT_INFO("[AOD] here to enter ulps\n");
		} else {
			LCDKIT_INFO("[AOD] here already enter ulps(power down)\n");
		}
		up(&hisifd->blank_sem);
		mdelay(1);
		mipi_to_mcu(hisifd);
		mdelay(1);
		lcdkit_info.panel_infos.mipi_aod_esd_switch = 0;
		hisi_fb_blank_sub(FB_BLANK_POWERDOWN, hisifd->fbi);
		break;
	case ALPM_HYBRID_EXIT_CMD:
		hisi_fb_blank_sub(FB_BLANK_POWERDOWN, hisifd->fbi);
		lcdkit_info.panel_infos.mipi_aod_esd_switch = 1;
		hisi_fb_blank_sub(FB_BLANK_UNBLANK, hisifd->fbi);
		mipi_to_ap(hisifd);
		read_watch_reg_value(hisifd);
#if defined(CONFIG_DPU_FB_AP_AOD)
		hisifd->ap_aod_skip_frame_flag = 0;
#endif
		lcdkit_info.panel_infos.watch_aod_hybrid = 0;
		/* here to check power status again */
		if (!hisifd->panel_power_on)
			hisi_fb_blank_sub(FB_BLANK_UNBLANK, hisifd->fbi);
		exit_watch_aod(hisifd);
		break;
	case AOD_WATCH_ULTRA_LOW_BRIGHTNESS:
		bl_level = lcdkit_info.panel_infos.watch_aod_ultralow_brightness;
		lcdkit_aod_on(pdev, &display_on_cmd, bl_level);
		break;
	default:
		LCDKIT_INFO("[AOD] AOD cmd %d break\n", cmd);
		break;
	}
	return count;
}
...
static ssize_t tddi_aod_light_set(struct platform_device *pdev, uint32_t bl_level)
{
	...
	LCDKIT_INFO("Tddi AOD hisi_blpwm_set_backlight bl_level = %d \n", bl_level);
	lcdkit_backlight_enable_gpio_operation(pinfo->bl_ic_ctrl_mode, bl_level);
	ret = hisi_blpwm_set_backlight(hisifd, bl_level);
	return ret;
}

static void lcd_kit_delay_mod_aod_light(unsigned int mode)
{
	ktime_t cur_timestamp;
	unsigned int diff_ms = 0;

	if (mode == ALPM_NO_LIGHT || !lcdkit_info.panel_infos.doze_time_delay) {
		LCDKIT_INFO("aod light not need delay\n");
		return;
	}

	cur_timestamp = ktime_get();
	diff_ms = (ktime_to_us(cur_timestamp) - \
		ktime_to_us(lcdkit_info.panel_infos.alpm_start_time)) / 1000;
	if (diff_ms < lcdkit_info.panel_infos.doze_time_delay) {
		LCDKIT_INFO("aod light delay %ums\n", lcdkit_info.panel_infos.doze_time_delay - diff_ms);
		msleep(lcdkit_info.panel_infos.doze_time_delay - diff_ms);
	}
}

static ssize_t lcdkit_tddi_aod_alpm(struct platform_device *pdev, const char *buf, size_t count)
{
	int ret;
	unsigned int cmd = 0;
	struct hisi_fb_data_type *hisifd = NULL;
	...
	ret = sscanf_s(buf, "%u", &cmd);
	if (!ret) {
		LCDKIT_ERR("sscanf return invaild:%d\n", ret);
		return -EINVAL;
	}
	LCDKIT_INFO("Tddi AOD cmd is %d \n", cmd);
	lcd_kit_delay_mod_aod_light(cmd);
	switch (cmd) {
	case ALPM_ON_HIGH_LIGHT:
		tddi_aod_light_set(pdev, lcdkit_info.panel_infos.tddi_aod_high_light);
		break;
	case ALPM_ON_MIDDLE_LIGHT:
		tddi_aod_light_set(pdev, lcdkit_info.panel_infos.tddi_aod_middle_light);
		break;
	case ALPM_ON_LOW_LIGHT:
		tddi_aod_light_set(pdev, lcdkit_info.panel_infos.tddi_aod_low_light);
		break;
	case ALPM_NO_LIGHT:
		tddi_aod_light_set(pdev, lcdkit_info.panel_infos.tddi_aod_no_light);
		break;
	default:
		LCDKIT_INFO("[AOD] AOD cmd invaild:%d\n", cmd);
		break;
	}
	return count;
}
...
static ssize_t lcdkit_aod_alpm_setting_store(struct platform_device *pdev, const char *buf, size_t count)
{
	...
	if (lcdkit_info.panel_infos.watch_aod)
		return lcdkit_watch_aod_alpm(pdev, buf, count);

	if (lcdkit_info.panel_infos.tddi_aod)
		return lcdkit_tddi_aod_alpm(pdev, buf, count);
	...
	struct lcdkit_dsi_cmd_desc cmds =
		{DTYPE_GEN_LWRITE, 0, 0, 0, 10, 0, 3, playload};
	struct lcdkit_dsi_panel_cmds backlight_cmds = {NULL, 0, &cmds, 1, 0, 0};

	char playload1[1] = {0x28};
	struct lcdkit_dsi_cmd_desc cmds1 =
		{DTYPE_DCS_WRITE, 0, 0, 0, 100, 0, 1, playload1};
	struct lcdkit_dsi_panel_cmds display_off_cmd = {NULL, 0, &cmds1, 1, 0, 0};

	char playload2[1] = {0x29};
	struct lcdkit_dsi_cmd_desc cmds2 =
		{DTYPE_DCS_WRITE, 0, 0, 0, 100, 0, 1, playload2};
	struct lcdkit_dsi_panel_cmds display_on_cmd = {NULL, 0, &cmds2, 1, 0, 0};
	...
	case ALPM_ON_10NIT_CMD:
		...
		lcdkit_dsi_tx(hisifd,
			&lcdkit_info.panel_infos.panel_aod_low_brightness_cmds);
		lcdkit_dsi_tx(hisifd, &display_on_cmd);
		break;
...
void read_ddic_reg_parse(uint32_t input[],uint8_t inputlen, uint8_t output[], uint8_t start_position, uint8_t length)
{
	...
	if ((NULL == input) || (NULL == output) ||
		(start_position + length > inputlen)) {
		LCDKIT_ERR("NULL Pointer!\n");
	...
	for (i = 0; i < length; i++, start_position++) {
			...
			case 0:
				if ((input[start_position / 4] & 0xFF) == 0)
					output[i] = (uint8_t)((input[start_position / 4] &
						0xFF) + '0');
				else
			...
			case 1:
				if (((input[start_position / 4] >> 8) & 0xFF) == 0)
					output[i] = (uint8_t)(((input[start_position / 4] >> 8) &
						0xFF) + '0');
				else
					output[i] = (uint8_t)((input[start_position / 4] >> 8) &
						0xFF);
...
void read_td4336_project_id(struct hisi_fb_data_type* hisifd, uint8_t g_project_id[], uint8_t start_position, uint8_t length)
{
	...
	if (hisifd == NULL || g_project_id == NULL) {
		LCDKIT_ERR("NULL Pointer!\n");
		return ;
	}
	...
	{
		{DTYPE_GEN_READ1, 0, 100, WAIT_TYPE_US, sizeof(project_id_reg),
			project_id_reg},
	};
	...
	LCDKIT_PANEL_CMD_REQUEST();
	ret = mipi_dsi_lread_reg(g_read_value_td4336, project_id_cmd,
		READ_REG_TD4336_NUM, mipi_dsi0_base);
	if(ret < 0)
		HISI_FB_ERR("Read project_id_cmd fail\n");
	read_ddic_reg_parse(g_read_value_td4336, READ_REG_TD4336_NUM,
		g_project_id, start_position, length);
	LCDKIT_PANEL_CMD_RELEASE();
	return;
}
...
void read_himax83112_project_id(struct hisi_fb_data_type* hisifd, uint8_t g_project_id[])
{
	...
	char project_addr[PROJECT_ID_LENGTH] =
		{0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8a, 0x8b};
	char project_addr_112a[PROJECT_ID_LENGTH] =
	...
	{
		{DTYPE_DCS_LWRITE, 0, 0, WAIT_TYPE_US, sizeof(otp_poweron_offset1),
			otp_poweron_offset1},
		{DTYPE_DCS_LWRITE, 0, 0, WAIT_TYPE_US, sizeof(otp_poweron1),
			otp_poweron1},
		{DTYPE_DCS_LWRITE, 0, 1, WAIT_TYPE_MS, sizeof(otp_poweron_offset2),
			otp_poweron_offset2},
	};
	struct dsi_cmd_desc otp_poweron2_cmds[] =
	{
		{DTYPE_DCS_LWRITE, 0, 0, WAIT_TYPE_US, sizeof(otp_poweron_offset1),
			otp_poweron_offset1},
		{DTYPE_DCS_LWRITE, 0, 0, WAIT_TYPE_US, sizeof(otp_poweron2),
			otp_poweron2},
		{DTYPE_DCS_LWRITE, 0, 1, WAIT_TYPE_MS, sizeof(otp_poweron_offset2),
			otp_poweron_offset2},
	};
	struct dsi_cmd_desc otp_poweroff_cmds[] =
	{
		{DTYPE_DCS_LWRITE, 0, 0, WAIT_TYPE_US, sizeof(otp_poweron_offset1),
			otp_poweron_offset1},
		{DTYPE_DCS_LWRITE, 0, 0, WAIT_TYPE_US, sizeof(otp_poweroff),
			otp_poweroff},
		{DTYPE_DCS_LWRITE, 0, 0, WAIT_TYPE_US, sizeof(otp_poweron_offset2),
			otp_poweron_offset2},
	};

	struct dsi_cmd_desc playload1_enter_cmds[] =
	{
		{DTYPE_DCS_LWRITE, 0, 1, WAIT_TYPE_US, sizeof(playload1_enter1),
			playload1_enter1},
		{DTYPE_DCS_LWRITE, 0, 0, WAIT_TYPE_US, sizeof(playload1_enter2),
			playload1_enter2},
	};
	struct dsi_cmd_desc project_id_cmd[] =
	{
		{DTYPE_GEN_READ, 0, 100, WAIT_TYPE_US, sizeof(project_id_reg),
			project_id_reg},
	};
	...
	mipi_dsi_cmds_tx(otp_poweron1_cmds, ARRAY_SIZE(otp_poweron1_cmds),
		mipi_dsi0_base);
	mipi_dsi_cmds_tx(otp_poweron2_cmds, ARRAY_SIZE(otp_poweron2_cmds),
		mipi_dsi0_base);
	...
	for (i = 0; i<PROJECT_ID_LENGTH; i++) {
		...
		mipi_dsi_cmds_tx(playload1_enter_cmds, ARRAY_SIZE(playload1_enter_cmds),
			mipi_dsi0_base);
	...
	mipi_dsi_cmds_tx(otp_poweroff_cmds, ARRAY_SIZE(otp_poweroff_cmds),
		mipi_dsi0_base);
	LCDKIT_PANEL_CMD_RELEASE();
	return;
}
...
static void lcdkit_check_project_id(void)
{
	...
	if ((CORRECT_PROJECT_ID_LEN_1 == len) || (CORRECT_PROJECT_ID_LEN_2 == len)) {
		for (pos=0; pos < len; pos++) {
			if (((g_project_id[pos] >= 'A') && (g_project_id[pos] <= 'Z')) ||
				(( g_project_id[pos] >= 'a') && (g_project_id[pos] <= 'z'))) {
				letter_num++;
				continue;
			} else if((g_project_id[pos] >= '0') &&
				(g_project_id[pos] <= '9')) {
				digit_num++;
				continue;
			}
		...
		if ((valid_flag == TRUE) && (letter_num > 0) && (letter_num < len) &&
			(digit_num > 0) && (digit_num < len)) {
			LCDKIT_DEBUG("read correct project_id = %s\n", g_project_id);
			return;
		}
	}

	if (lcdkit_info.panel_infos.panel_default_project_id == NULL) {
	...
	if (default_len < LCD_DDIC_INFO_LEN) {
		...
		strncpy(g_project_id,lcdkit_info.panel_infos.panel_default_project_id,
			default_len);
...
static void lcdkit_read_project_id(void)
{
	...
	case Hx83112C:
		...
		break;
	case TD4336:
		read_td4336_project_id(hisifd, g_project_id,
			PROJECT_ID_START_POSITION_TD4336, PROJECT_ID_LENGTH);
		break;
	// this case is for eml_read_reg_flag == 0 or == 4
	default:
		lcdkit_dsi_tx(hisifd,
			&lcdkit_info.panel_infos.host_project_id_enter_cmds);
		ret = hostprocessing_read_ddic(g_project_id,
			&lcdkit_info.panel_infos.host_project_id_cmds,
			hisifd, LCD_DDIC_INFO_LEN);
		lcdkit_dsi_tx(hisifd,
			&lcdkit_info.panel_infos.host_project_id_exit_cmds);
		break;
	}
...
void lcdkit_read_ddic_reg(unsigned char *out, const struct lcdkit_dsi_panel_cmds *cmds,
	struct hisi_fb_data_type *hisifd, int max_out_size)
{
	...
	for (i = 0; i < cmds->cmd_cnt; i++) {
		if (lcdkit_cmd_is_write(cm)) {
			lcdkit_single_cmd_tx(cm, hisifd);
			cm++;
			continue;
		}
...
static ssize_t lcdkit_color_param_get_func(struct hisi_fb_data_type* hisifd)
{
	...
	if (lcdkit_info.panel_infos.lcd_brightness_color_uniform_support) {
		...
		hisifd->de_info.lcd_color_oeminfo.id_flag =
			lcdkit_info.panel_infos.lcd_color_oeminfo.id_flag;
		hisifd->de_info.lcd_color_oeminfo.tc_flag =
			lcdkit_info.panel_infos.lcd_color_oeminfo.tc_flag;

		hisifd->de_info.lcd_color_oeminfo.panel_id.modulesn =
			lcdkit_info.panel_infos.lcd_color_oeminfo.panel_id.modulesn;
		hisifd->de_info.lcd_color_oeminfo.panel_id.equipid =
			lcdkit_info.panel_infos.lcd_color_oeminfo.panel_id.equipid;
		hisifd->de_info.lcd_color_oeminfo.panel_id.modulemanufactdate =
			lcdkit_info.panel_infos.lcd_color_oeminfo.panel_id.modulemanufactdate;
		hisifd->de_info.lcd_color_oeminfo.panel_id.vendorid =
			lcdkit_info.panel_infos.lcd_color_oeminfo.panel_id.vendorid;

		hisifd->de_info.lcd_color_oeminfo.color_params.c_lmt[0] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_params.c_lmt[0];
		hisifd->de_info.lcd_color_oeminfo.color_params.c_lmt[1] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_params.c_lmt[1];
		hisifd->de_info.lcd_color_oeminfo.color_params.c_lmt[2] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_params.c_lmt[2];
		hisifd->de_info.lcd_color_oeminfo.color_params.mxcc_matrix[0][0] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_params.mxcc_matrix[0][0];
		hisifd->de_info.lcd_color_oeminfo.color_params.mxcc_matrix[0][1] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_params.mxcc_matrix[0][1];
		hisifd->de_info.lcd_color_oeminfo.color_params.mxcc_matrix[0][2] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_params.mxcc_matrix[0][2];
		hisifd->de_info.lcd_color_oeminfo.color_params.mxcc_matrix[1][0] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_params.mxcc_matrix[1][0];
		hisifd->de_info.lcd_color_oeminfo.color_params.mxcc_matrix[1][1] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_params.mxcc_matrix[1][1];
		hisifd->de_info.lcd_color_oeminfo.color_params.mxcc_matrix[1][2] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_params.mxcc_matrix[1][2];
		hisifd->de_info.lcd_color_oeminfo.color_params.mxcc_matrix[2][0] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_params.mxcc_matrix[2][0];
		hisifd->de_info.lcd_color_oeminfo.color_params.mxcc_matrix[2][1] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_params.mxcc_matrix[2][1];
		hisifd->de_info.lcd_color_oeminfo.color_params.mxcc_matrix[2][2] =
		...
		hisifd->de_info.lcd_color_oeminfo.color_params.white_decay_luminace =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_params.white_decay_luminace;

		hisifd->de_info.lcd_color_oeminfo.color_mdata.chroma_coordinates[0][0] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_mdata.chroma_coordinates[0][0];
		hisifd->de_info.lcd_color_oeminfo.color_mdata.chroma_coordinates[0][1] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_mdata.chroma_coordinates[0][1];
		hisifd->de_info.lcd_color_oeminfo.color_mdata.chroma_coordinates[1][0] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_mdata.chroma_coordinates[1][0];
		hisifd->de_info.lcd_color_oeminfo.color_mdata.chroma_coordinates[1][1] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_mdata.chroma_coordinates[1][1];
		hisifd->de_info.lcd_color_oeminfo.color_mdata.chroma_coordinates[2][0] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_mdata.chroma_coordinates[2][0];
		hisifd->de_info.lcd_color_oeminfo.color_mdata.chroma_coordinates[2][1] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_mdata.chroma_coordinates[2][1];
		hisifd->de_info.lcd_color_oeminfo.color_mdata.chroma_coordinates[3][0] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_mdata.chroma_coordinates[3][0];
		hisifd->de_info.lcd_color_oeminfo.color_mdata.chroma_coordinates[3][1] =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_mdata.chroma_coordinates[3][1];
		hisifd->de_info.lcd_color_oeminfo.color_mdata.white_luminance =
			lcdkit_info.panel_infos.lcd_color_oeminfo.color_mdata.white_luminance;
	} else {
...
static ssize_t lcdkit_cabc_store(struct platform_device *pdev, const char *buf, size_t count)
{
	...
	if (plcdkit_info->lcdkit_cabc_mode_store &&
		plcdkit_info->panel_infos.cabc_switch_support) {
		LCDKIT_PANEL_CMD_REQUEST();
		if (lcdkit_check_mipi_fifo_empty(hisifd->mipi_dsi0_base))
...
void hisifb_get_calicolordata_from_share_mem(struct lcdbrightnesscoloroeminfo *pinfo)
{
	...
	memcpy((void*)pinfo, (void*)vir_addr,
...
static int __init lcdkit_probe(struct platform_device* pdev)
{
	...
	if (hisi_fb_device_probe_defer(lcdkit_info.panel_infos.lcd_disp_type,
		lcdkit_info.panel_infos.bl_type)) {
	...
	if (lcdkit_info.panel_infos.mipi_switch_support) {
		mipi_switch_init(pdev);
		mutex_init(&lcdkit_info.panel_infos.switch_lock);
	}
	...
	/* effect init */
	lcdkit_effect_get_data(lcdkit_panel_init(lcdkit_info.panel_infos.product_id,
		lcdkit_info.panel_infos.lcd_compatible), pinfo);
...
#if defined(CONFIG_HISI_FB_970)
	if(lcdkit_info.panel_infos.lcd_brightness_color_uniform_support) {
		hisifb_get_calicolordata_from_share_mem(
			&lcdkit_info.panel_infos.lcd_color_oeminfo);
	}
#endif
	/* vsp/vsn bl init */
		...
		if ((!strncmp(lcdkit_info.panel_infos.panel_name,
			...
			switch(panel_version){
				case SAMSUNG_8FC1_PANEL_VERSION_SV1:
					pinfo->mask_delay.delay_time_before_fp =
						lcdkit_info.panel_infos.mask_delay_time_before_fp_sv1;
					pinfo->mask_delay.delay_time_after_fp =
...
static int __init lcdkit_disp_init(void)
{
	...
	OF_PROPERTY_READ_U32_RETURN(np, "product_id",
		&lcdkit_info.panel_infos.product_id);
	LCDKIT_INFO("lcdkit_info.panel_infos.product_id = %d",
		lcdkit_info.panel_infos.product_id);

	lcdkit_info.panel_infos.lcd_compatible =
		(char*)of_get_property(np, "lcd_panel_type", NULL);
	isbulcked = false;
	if (!lcdkit_info.panel_infos.lcd_compatible) {
		LCDKIT_ERR("Is not normal lcd and return\n");
		return ret;
	} else {
		if(!strncmp(lcdkit_info.panel_infos.lcd_compatible,
			"auo_otm1901a_5p2_1080p_video_default",
			strlen("auo_otm1901a_5p2_1080p_video_default"))) {
			LCDKIT_INFO("the panel is not buckled! \n");
			return ret;
		}
		isbulcked = true;
		len = strlen(lcdkit_info.panel_infos.lcd_compatible);
		memset( (char *)lcdkit_driver.driver.of_match_table->compatible, 0,
			LCDKIT_PANEL_COMP_LENGTH);
		strncpy( (char *)lcdkit_driver.driver.of_match_table->compatible,
			lcdkit_info.panel_infos.lcd_compatible,
			len > (LCDKIT_PANEL_COMP_LENGTH - 1) ?
			(LCDKIT_PANEL_COMP_LENGTH - 1) : len);
	}
