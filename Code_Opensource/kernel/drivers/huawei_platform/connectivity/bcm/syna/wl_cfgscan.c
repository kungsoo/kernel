...
s32 wl_inform_single_bss(struct bcm_cfg80211 *cfg, wl_bss_info_t *bi, bool update_ssid)
{
	...
	if ((bi->SSID_len == 7) && !memcmp(bi->SSID, "DIRECT-", 7)) {
		uint32 tmp_band = 0;
		uint32 tmp_channel = notif_bss_info->channel;
		if ((tmp_channel < 0) || ((tmp_channel > 14) && (tmp_channel < 36)) ||
				(tmp_channel > 165)) {
			WL_ERR(("%s: got invalid channel %d, chanspec=%x\n", __func__,
					notif_bss_info->channel, bi->chanspec));
			notif_bss_info->channel = 1;
		}
		if (notif_bss_info->channel > 14) {
			tmp_band = WL_CHANSPEC_BAND_5G;
		}else {
			tmp_band = WL_CHANSPEC_BAND_2G;
		}
		if(tmp_band != notif_bss_info->band) {
			WL_ERR(("%s: got invalid band %x vs %x, chanspec=%x\n", __func__,
					notif_bss_info->band, tmp_band, bi->chanspec));
			notif_bss_info->band = tmp_band;
		}
	}
	...
	WL_DBG(("SSID : \"%s\", rssi %d, channel %d, capability : 0x04%x, bssid %pM"
			"mgmt_type %d frame_len %d chanspec=%x\n", tmp_buf,
			notif_bss_info->rssi, notif_bss_info->channel,
			mgmt->u.beacon.capab_info, &bi->BSSID, mgmt_type,
			notif_bss_info->frame_len, bi->chanspec));
				...
				if (!bcmp(&bi->BSSID, &bss->BSSID, ETHER_ADDR_LEN) &&
					...
					*/
					...
					WL_DBG(("%s("MACDBG"), i=%d prev: RSSI %d"
						" flags 0x%x, new: RSSI %d flags 0x%x, chanspec=%x\n",
						bss->SSID, MAC2STRDBG(bi->BSSID.octet), i,
						bss->RSSI, bss->flags, bi->RSSI, bi->flags, bi->chanspec));
	...
	struct cfg80211_ssid *this_ssid)
{
	 ...
	 */
#ifndef WL_HOSTAPD_CFG /* Allow scan on AP interface when configuring from hostad */
...
#endif /* WL_CFG80211_P2P_DEV_IF */
{
	...
	if ((FALSE == g_dhd_escan_on) && wl_get_drv_status_all(cfg, CONNECTED)) {
		WL_ERR(("wl_cfg80211_scan: g_dhd_escan_on set return busy\n"));
		return -EBUSY;
	}
...
exit:
	...
	} else if (err == BCME_NOTUP) {
		WL_ERR(("Convert BCME_NOTUP to linux error code EINVAL (err=%d cookie:%llu)\n", err, *cookie));
		err = -EINVAL;
		wl_flush_fw_log_buffer(ndev, FW_LOGSET_MASK_ALL);
	} else {
