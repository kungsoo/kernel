...
#else
int dhd_get_monitor_data(dhd_pub_t *dhdp,void *pktbuf,struct sk_buff *skb,void *pktdata,uint pktlen)
{
...
bool dhd_allow_stop = TRUE;
module_param(dhd_allow_stop, bool, 0644);
...
_dhd_set_mac_address(dhd_info_t *dhd, int ifidx, uint8 *addr)
{
	...
	/* avoid recovery after set random mac */
	/* dhd_allow_stop = TRUE; */
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 11, 0)) && (RHEL_RELEASE_CODE < \
...
static int dhd_reopen_thread(void *data)
{
	...
	struct net_device *dev = dhd->iflist[0]->net;
...
dhd_monitor_enabled(dhd_pub_t *dhd, int ifidx)
{
	return (dhd->info->monitor_type > 0 ) && (dhd->info->monitor_type != DHD_MONITOR_TYPE_MGMT );
}
#endif // WL_MON_OWN_PKT
...
#ifndef WL_CFG80211_MONITOR
#ifndef WL_MON_OWN_PKT
	if (FW_SUPPORTED((&dhd->pub), monitor) && (dhd->pub.op_mode != DHD_FLAG_MFG_MODE)) {
#ifdef DHD_PCIE_RUNTIMEPM
		/* Disable RuntimePM in monitor mode */
		DHD_DISABLE_RUNTIME_PM(&dhd->pub);
		DHD_ERROR(("%s : disable runtime PM in monitor mode\n", __FUNCTION__));
...
#ifndef WL_CFG80211_MONITOR
	if (FW_SUPPORTED((&dhd->pub), monitor) && (dhd->pub.up) && (dhd->pub.op_mode != DHD_FLAG_MFG_MODE)) {
#ifdef DHD_PCIE_RUNTIMEPM
		/* Enable RuntimePM */
		DHD_ENABLE_RUNTIME_PM(&dhd->pub);
		DHD_ERROR(("%s : enabled runtime PM\n", __FUNCTION__));
...
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	if (dev_close_flag) {
		DHD_ERROR(("%s: dhd is closed. skip it.\n", __func__));
		return -ENODEV;
	}
...
dhd_stop(struct net_device *net)
{
	...
	if (IS_CFG80211_STATIC_IF_ACTIVE(cfg) ||
		...
		0) {
		uint wl_down = 1;
		ret = dhd_wl_ioctl_cmd(&dhd->pub, WLC_DOWN, (char *)&wl_down, sizeof(wl_down), TRUE, 0);
...
#endif /* APF */
#ifdef READ_CONFIG_FROM_FILE
	dhd_conf_dependency_dft(&dhd->pub);
#endif /* READ_CONFIG_FROM_FILE */
...
#ifdef CUSTOMER_HW4
	dhd_read_macaddr_from_file(&dhd->pub, (struct ether_addr *)net->dev_addr);
	net->dev_addr[0] |= 0x02;
#endif /* CUSTOMER_HW4 */
...
dhd_legacy_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	*/
	...
	uint32 enable_memuse = 1;
#endif /* DHD_BUS_MEM_ACCESS */
#if  defined(DISABLE_1024QAM_VHT)
	uint32 vht_features = 3; /* disable vht 1024 QAM support */
#else defined(SUPPORT_2G_VHT) || defined(SUPPORT_5G_1024QAM_VHT)
...
#endif /* BCMSUP_4WAY_HANDSHAKE */
#if defined(SUPPORT_2G_VHT) || defined(SUPPORT_5G_1024QAM_VHT) || defined(DISABLE_1024QAM_VHT)
...
#endif /* SUPPORT_2G_VHT || SUPPORT_5G_1024QAM_VHT  || DISABLE_1024QAM_VHT */
#ifdef DISABLE_11N_PROPRIETARY_RATES
...
dhd_os_get_img_fwreq(const struct firmware **fw, char *file_path)
{
	...
	if (ret < 0) {
		DHD_ERROR(("%s: request_firmware_direct err: %d\n", __FUNCTION__, ret));
		if (-ENOENT == ret) {
...
#ifdef WL_CFG80211
	if (dhd->iflist[ifidx] && dhd->iflist[ifidx]->net) {
		DHD_UP_LOCK(&dhd->pub.up_lock, flags);
		if (dhd->pub.up) {
...
static void dhd_hang_process(struct work_struct *work_data)
{
	...
	if (dev) {
#ifdef DHD_REOPEN
		PROC_START(dhd_reopen_thread, dhd, &dhd->thr_reopen_ctl, 0, "FW_reload");
#else
		dhd_reopen_dev(dev);
#endif /* DHD_REOPEN */
	}
...
int dhd_reopen_dev(struct net_device *dev)
{
	...
	if(dhd->pub.op_mode & DHD_FLAG_HOSTAP_MODE) {
		...
		DHD_ERROR(("dhd_reopen_dev enter\n"));
		dhd_stop(dev);
		dhd_open(dev);
	}
	return 0;
...
#endif /* BCM_ROUTER_DHD || DHD_DEBUG || DHD_COREDUMP */

#if defined(DHD_DEBUG) || defined(DHD_COREDUMP)
static void
...
void dhd_set_cpucore(dhd_pub_t *dhd, int set)
{
	...
	DHD_ERROR(("%s, set %d, DPC_CPUCORE %d, RXF_CPUCORE %d, num_cpus %d\n",
		__FUNCTION__, set, DPC_CPUCORE, RXF_CPUCORE, num_cpus));

	if (DPC_CPUCORE && (DPC_CPUCORE < num_cpus)) {
		do {
			if (set == TRUE) {
	...
	if (RXF_CPUCORE && (RXF_CPUCORE < num_cpus)) {
		do {
			if (set == TRUE) {
	...
	DHD_ERROR(("%s: set(%d) cpucore success!\n", __FUNCTION__, set));
...
void dhd_init_custom_dts(struct device_node * node)
{
	int ret = 0;
	const char *local_fw_path = NULL;
	const char *local_nv_path = NULL;
	const char *local_clm_path = NULL;

	DHD_INFO(("init navram and firmware path from dts\n"));
	if (node == NULL) {
		DHD_INFO(("%s root node is NULL\n", __func__));
		return;
	}

	ret = of_property_read_string(node, "wifi_nvram_name", &local_nv_path);
	if (ret == 0 && local_nv_path != NULL) {
		(void)memset_s(nvram_path, sizeof(nvram_path), 0, sizeof(nvram_path));
		(void)memcpy_s(nvram_path, sizeof(nvram_path), local_nv_path, strlen(local_nv_path));
	}
	ret = of_property_read_string(node, "wifi_fw_name", &local_fw_path);
	if (ret == 0 && local_fw_path != NULL) {
		(void)memset_s(firmware_path, sizeof(firmware_path), 0, sizeof(firmware_path));
		(void)memcpy_s(firmware_path, sizeof(firmware_path), local_fw_path, strlen(local_fw_path));
	}
	ret = of_property_read_string(node, "wifi_clm_name", &local_clm_path);
	if (ret == 0 && local_clm_path != NULL) {
		(void)memset_s(clm_path, sizeof(clm_path), 0, sizeof(clm_path));
		(void)memcpy_s(clm_path, sizeof(clm_path), local_clm_path, strlen(local_clm_path));
	}
}
#endif
