...
#define GOODIX_DEVICE_ID_LEN         2
#define GOODIX_DETECT_LEN            2
#define I2C_DETECT_LONG              1
#define I2C_DETECT_SHORT             2
...
static void read_ps_chip_info(void)
{
	...
	else if (!strncmp(chip_info, "proximity,ps_s009_001",
		sizeof("proximity,ps_s009_001")))
		dev_info->chip_type = PS_CHIP_HX32062SE;
	else if (!strncmp(chip_info, "proximity,ps_s008_001",
		sizeof("proximity,ps_s008_001")))
		dev_info->chip_type = PS_CHIP_SIP3510;
	else
		return;
...
static int i2c_detect_sensors(char *device_name, int i2c_address, int i2c_bus_num, int register_add,
			      u32 *wia, int len)
{
	int i;
	int ret;
...
static int i2c_detect_sensors_short(char *device_name, int i2c_address, int i2c_bus_num,
		 int register_add, u32 *wia, int len)
{
	int i;
	int ret;
	uint32_t device_id = 0;
	uint8_t detect_device_id[GOODIX_DEVICE_ID_LEN] = { 0 };
	ret = mcu_i2c_rw((uint8_t)i2c_bus_num, (uint8_t)i2c_address,
		 (uint8_t *)&register_add, GOODIX_DETECT_LEN,
		 &detect_device_id, GOODIX_DEVICE_ID_LEN);
	if (ret) {
	 hwlog_err("%s:detect_i2c_device:send i2c read cmd to mcu fail,ret=%d\n", device_name, ret);
	 return -1;
	}
	ret = memcpy_s(&device_id, sizeof(device_id), detect_device_id, GOODIX_DEVICE_ID_LEN);
	if (ret != EOK) {
	   hwlog_err("%s:memcpy_s fail,ret %d\n", __func__, ret);
	   return -1;
	}
	...
	hwlog_info("%s:i2c short detect fail,chip_value:0x%x,len:%d\n", device_name, device_id, len);
	return -1;
}
...
static int detect_i2c_device(struct device_node *dn, char *device_name)
{
	...
	if (!prop->value)
		return -ENODATA;
	len = prop->length / 4; /* 4: to word */
	hwlog_info("chip id prop len %d, origin len %d", len, prop->length);
	...
	hwlog_info("%s:i2c %d slave address 0x%x chip_id_reg 0x%x chipid value 0x%x 0x%x 0x%x 0x%x\n",
		device_name, i2c_bus_num, i2c_address, register_add, wia[0], wia[1], wia[2], wia[3]);

	if (of_property_read_u8(dn, "i2c_detect_long_type", &i2c_detect_long_type) == 0)
		hwlog_info("%s: success read i2c_detect_long_type:0x%x\n",
		 device_name, i2c_detect_long_type);
	else
	    hwlog_info("%s:failed read i2c_detect_long_type:0x%x\n", device_name, i2c_detect_long_type);
	if (i2c_detect_long_type == I2C_DETECT_SHORT) {
	ret = i2c_detect_sensors_short(device_name, i2c_address, i2c_bus_num, register_add, wia, len);
	    if (ret)
	      return -1;
	    else
	      return 0;
	}
	if (i2c_detect_long_type == I2C_DETECT_LONG) {
	  ret = i2c_detect_sensors(device_name, i2c_address, i2c_bus_num, register_add, wia, len);
		if (ret)
			return -1;
		else
			return 0;
