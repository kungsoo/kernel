...
static void show_als_debug_hx32062se(uint32_t id, short *dbg, uint32_t num)
{
	uint32_t k;
	hx32062se_als_para_table *table = NULL;
	table = als_get_hx32062se_table_by_id(id);
	if (!table)
		return;
}
static void show_als_debug_sip3510(uint32_t id, short *dbg, uint32_t num)
{
	uint32_t k;
	sip3510_als_para_table *table = NULL;
	table = als_get_sip3510_table_by_id(id);
	if (!table)
		return;
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (dev_info->chip_type == ALS_CHIP_ROHM_RGB) /* bh745_para */
		...
		show_als_debug_sip3510(id, dbg, num);
	else if (dev_info->chip_type == ALS_CHIP_HX32062SE)
		show_als_debug_hx32062se(id, dbg, num);
...
static ssize_t store_als_debug_hx32062se(struct als_platform_data *pf_data,
	uint32_t id, const short *dbg, uint32_t num, size_t size)
{
	hx32062se_als_para_table *table = NULL;
...
static ssize_t store_als_debug_sip3510(struct als_platform_data *pf_data,
	uint32_t id, const short *dbg, uint32_t num, size_t size)
{
	sip3510_als_para_table *table = NULL;
	...
	uint32_t id, const char *buf, uint32_t num, size_t size)
{
	 ...
	 * para[9]: g_middle_als_data: lower 15 bits
	 * para[10]: g_middle_als_data: upper 15 bits
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	case ALS_CHIP_SIP3510:
		return store_als_debug_sip3510(pf_data, id, dbg, num, size);
	case ALS_CHIP_HX32062SE:
		return store_als_debug_hx32062se(pf_data, id, dbg, num, size);
