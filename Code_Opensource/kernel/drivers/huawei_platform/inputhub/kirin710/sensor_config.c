...
#define BU27006_MAX_THRESHOLD_NUM    25
#define BU27006_MIN_THRESHOLD_NUM    26
...
int write_calibrate_data_to_nv(int nv_number, int nv_size, const char *nv_name,
	const char *temp)
{
	int ret;
...
int send_cap_prox_calibrate_data_to_mcu(void)
{
	...
	if (!strncmp(sensor_chip_info[CAP_PROX], "huawei,cypress_sar_psoc4000",
		...
		hwlog_info("a96t3x6:offset[0]=%d, offset[1]=%d, diff[0]=%d, diff[1]=%d, length:%ld %ld\n",
		...
		hwlog_info("aw9610:offset=%d, diff=%d length:%ld %ld\n",
...
static void reset_als_calibrate_data(void)
{
	...
	if (t == ALS_CHIP_ROHM_RGB || t == ALS_CHIP_AVAGO_RGB ||
		...
		t == ALS_CHIP_SY3133 || t == ALS_CHIP_TMD2755 || t == ALS_CHIP_HX32062SE ||
		t == ALS_CHIP_SIP3510 || t == ALS_CHIP_STK33562 || t == ALS_CHIP_MN78911)
...
static void set_hx32062se_als_extend_parameters(struct als_platform_data *pf_data,
	struct als_device_info *dev_info)
{
	int als_para_table = 0;
	unsigned int i;
	hx32062se_als_para_table *hx32062se_tab = als_get_hx32062se_first_table();
	uint32_t table_size = als_get_hx32062se_table_count();
	for (i = 0; i < table_size; i++) {
		if ((hx32062se_tab[i].phone_type == als_data.als_phone_type) &&
			(hx32062se_tab[i].phone_version == als_data.als_phone_version) &&
			(hx32062se_tab[i].tp_manufacture == get_tplcd_manufacture_curr())) {
			als_para_table = i;
			break;
		}
	}
	dev_info->table_id = als_para_table;
	if (memcpy_s(als_data.als_extend_data, sizeof(als_data.als_extend_data),
		hx32062se_tab[als_para_table].als_para,
		(sizeof(hx32062se_tab[als_para_table].als_para) >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE) ?
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE :
		sizeof(hx32062se_tab[als_para_table].als_para)) != EOK)
		return;
	hwlog_err("hx32062se:als_para_table=%d, phone_type=%d, phone_version=%d\n",
		als_para_table, als_data.als_phone_type, als_data.als_phone_version);
}

static void set_sip3510_als_extend_parameters(struct als_platform_data *pf_data,
	struct als_device_info *dev_info)
{
	int als_para_table = 0;
	unsigned int i;
	sip3510_als_para_table *sip3510_tab = als_get_sip3510_first_table();
	uint32_t table_size = als_get_sip3510_table_count();
	for (i = 0; i < table_size; i++) {
		if ((sip3510_tab[i].phone_type == als_data.als_phone_type) &&
			(sip3510_tab[i].phone_version == als_data.als_phone_version) &&
			(sip3510_tab[i].tp_manufacture == get_tplcd_manufacture_curr())) {
			als_para_table = i;
			break;
		}
	}
	dev_info->table_id = als_para_table;
	if (memcpy_s(als_data.als_extend_data, sizeof(als_data.als_extend_data),
		sip3510_tab[als_para_table].als_para,
		(sizeof(sip3510_tab[als_para_table].als_para) >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE) ?
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE :
		sizeof(sip3510_tab[als_para_table].als_para)) != EOK)
		return;
	hwlog_err("sip3510:als_para_table=%d, phone_type=%d, phone_version=%d\n",
		als_para_table, als_data.als_phone_type, als_data.als_phone_version);
}
...
static void set_hx32062se_ps_parameters(struct ps_platform_data *ps_data,
	struct ps_device_info *dev_info)
{
	int ps_para_table = 0;
	unsigned int i;
	hx32062se_ps_para_table *hx32062se_tab = ps_get_hx32062se_first_table();
	uint32_t table_size = ps_get_hx32062se_table_count();
	if (ps_data == NULL)
	    return;
	for (i = 0; i < table_size; i++) {
		if ((hx32062se_tab[i].phone_type == ps_data->ps_phone_type) &&
			(hx32062se_tab[i].phone_version == ps_data->ps_phone_version) &&
			(hx32062se_tab[i].tp_manufacture == get_tplcd_manufacture_curr())) {
			ps_para_table = i;
			break;
		}
	...
	if (memcpy_s(ps_data->ps_extend_data, sizeof(ps_data->ps_extend_data),
		hx32062se_tab[ps_para_table].ps_para,
		(sizeof(hx32062se_tab[ps_para_table].ps_para) >
		SENSOR_PLATFORM_EXTEND_DATA_SIZE) ?
		SENSOR_PLATFORM_EXTEND_DATA_SIZE :
		sizeof(hx32062se_tab[ps_para_table].ps_para)) != EOK)
		return;
	hwlog_err("hx32062se:ps_para_table=%d, phone_type=%d, phone_version=%d\n",
		ps_para_table, ps_data->ps_phone_type, ps_data->ps_phone_version);
}
static void set_sip3510_ps_parameters(struct ps_platform_data *ps_data,
	struct ps_device_info *dev_info)
{
	int ps_para_table = 0;
	unsigned int i;
	sip3510_ps_para_table *sip3510_tab = ps_get_sip3510_first_table();
	uint32_t table_size = ps_get_sip3510_table_count();
	if (ps_data == NULL)
	    return;
	for (i = 0; i < table_size; i++) {
		if ((sip3510_tab[i].phone_type == ps_data->ps_phone_type) &&
			(sip3510_tab[i].phone_version == ps_data->ps_phone_version) &&
			(sip3510_tab[i].tp_manufacture == get_tplcd_manufacture_curr())) {
			ps_para_table = i;
			break;
		}
	...
	if (memcpy_s(ps_data->ps_extend_data, sizeof(ps_data->ps_extend_data),
		sip3510_tab[ps_para_table].ps_para,
		(sizeof(sip3510_tab[ps_para_table].ps_para) >
		SENSOR_PLATFORM_EXTEND_DATA_SIZE) ?
		SENSOR_PLATFORM_EXTEND_DATA_SIZE :
		sizeof(sip3510_tab[ps_para_table].ps_para)) != EOK)
		return;
	hwlog_err("sip3510:ps_para_table=%d, phone_type=%d, phone_version=%d\n",
		ps_para_table, ps_data->ps_phone_type, ps_data->ps_phone_version);
}
	...
	struct als_device_info *dev_info)
{
	...
	int als_para_table = 0;
	int i;
	bu27006_als_para_table *bu27006_tab = als_get_bu27006_first_table();
	uint32_t table_size = als_get_bu27006_table_count();

	for (i = 0; i < table_size; i++) {
		if ((bu27006_tab[i].phone_type == als_data.als_phone_type) &&
			(bu27006_tab[i].phone_version == als_data.als_phone_version) &&
			(bu27006_tab[i].tp_lcd_manufacture == tplcd_manufacture ||
			bu27006_tab[i].tp_lcd_manufacture == DEFAULT_TPLCD) &&
			(bu27006_tab[i].tp_color == phone_color)) {
			als_para_table = i;
			break;
		}
	}
	dev_info->table_id = als_para_table;
	if (memcpy_s(als_data.als_extend_data, sizeof(als_data.als_extend_data),
		bu27006_tab[als_para_table].bu27006_para,
		(sizeof(bu27006_tab[als_para_table].bu27006_para) >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE) ?
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE :
		sizeof(bu27006_tab[als_para_table].bu27006_para)) != EOK)
		return;

	dev_info->min_thres = bu27006_tab[als_para_table].bu27006_para[BU27006_MIN_THRESHOLD_NUM];
	dev_info->max_thres = bu27006_tab[als_para_table].bu27006_para[BU27006_MAX_THRESHOLD_NUM];

	hwlog_info("als_para_tabel=%d,bu27006 phone_color=0x%x,tplcd_manufacture=%d,phone_type=%d,phone_ver=%d\n",
		als_para_table, phone_color, tplcd_manufacture,
		als_data.als_phone_type, als_data.als_phone_version);
}
	...
	struct als_device_info *dev_info)
{
	...
	uint32_t table_size = als_get_tcs3707_table_count();
	...
	dev_info->table_id = als_para_table;
	if (memcpy_s(als_data.als_extend_data, sizeof(als_data.als_extend_data),
		tcs3707_tab[als_para_table].tcs3707_para,
		(sizeof(tcs3707_tab[als_para_table].tcs3707_para) >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE) ?
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE :
		sizeof(tcs3707_tab[als_para_table].tcs3707_para)) != EOK)
		return;

	dev_info->min_thres = tcs3707_tab[als_para_table].tcs3707_para[TCS3707_MIN_THRESHOLD_NUM];
	dev_info->max_thres = tcs3707_tab[als_para_table].tcs3707_para[TCS3707_MAX_THRESHOLD_NUM];
	...
	struct device_node *dn)
{
	if (dev_info->chip_type == ALS_CHIP_ROHM_RGB) {
		...
		mutex_lock(&mutex_set_para);
		set_hx32062se_als_extend_parameters(pf_data, dev_info);
		mutex_unlock(&mutex_set_para);
	} else if (dev_info->chip_type == ALS_CHIP_SIP3510) {
		mutex_lock(&mutex_set_para);
		set_sip3510_als_extend_parameters(pf_data, dev_info);
		mutex_unlock(&mutex_set_para);
...
void select_ps_para(struct sensor_detect_manager *sm)
{
	...
	if (dev_info->chip_type == PS_CHIP_HX32062SE)
		set_hx32062se_ps_parameters(ps_data, dev_info);
	if (dev_info->chip_type == PS_CHIP_SIP3510)
		set_sip3510_ps_parameters(ps_data, dev_info);
