...
#define GDINIC_REGS_NEED_INITIATED_NUM   16
...
struct gh621x_sar_data {
	uint16_t threshold_to_ap;
	uint16_t phone_type;
	uint16_t threshold_to_modem[CAP_MODEM_THRESHOLE_LEN];
	uint16_t init_reg_val[GDINIC_REGS_NEED_INITIATED_NUM];
	uint8_t ph;
	uint32_t calibrate_thred[CAP_CALIBRATE_THRESHOLE_LEN];
};
...
union sar_data {
	...
	struct gh621x_sar_data gh621x_data;
};
