...
t_unlock:
	...
	if (m)
		...
		printk("The top%u processes that allocate most memory:\n", top_n);
#else
	seq_printf(m, "%u\n", top_n);
#endif
	list_for_each_entry_safe(temp_info, dump_info, &dump_class, node_task) {
		...
		if (count++ < top_n) {
			...
			if (m) {
				...
				printk("pid: %d, comm: %s, size:%dKB\n",
					temp_info->pid, temp_info->task_name, temp_info->task_size);
			}
#else
			seq_printf(m, "pid:%d ", temp_info->pid);
			seq_printf(m, "comm:%s ", temp_info->task_name);
...
#ifdef CONFIG_OPTIMIZE_MM_AQ_HM4
void topn_process_mem_show(void)
{
