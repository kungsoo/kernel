...
static int bq2560x_device_check(void)
{
	...
	*/
	...
	case SGM41513H_VENDOR_ID:
		...
		return CHARGE_IC_GOOD;
	case SGM41513D_VENDOR_ID:
		hwlog_info("sgm41513D is good\n");
		g_sgm41513d_exist = true;
		return CHARGE_IC_GOOD;
...
static int bq2560x_5v_chip_init(struct bq2560x_device_info *di)
{
	...
	if (g_sgm41513_exist || g_sgm41513d_exist)
...
static int sgm41513_convert_to_iterm_reg_val(int value)
{
	u8 reg = 0;

	/*
	 * for sgm41513, The method of setting the terminal current(reg02)
	 ...
	 */
	if (value < 5)
		hwlog_info("sgm41513 iterm too low");
	else if (value < 20)
		reg = value / 5 - 1;
	else if (value <= 60)
		reg = (value / 10) + 1;
...
static int bq2560x_reused_set_iterm(int value)
{
	...
	if (g_sgm41513d_exist)
		val = sgm41513_convert_to_iterm_reg_val(value);
	else
