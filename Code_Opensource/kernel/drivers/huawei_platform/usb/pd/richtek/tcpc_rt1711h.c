		...
		struct tcpc_device *tcpc_dev, bool en, int pull)
{
	...
	if (chip->usb_esd_case == 1) {
		/*
		* RICHETEK: Disable auto idle mode during lpm
		* CK_300K from 320K, SHIPPING off, AUTOIDLE disable:enable, TIMEOUT = 32ms
		*/
		rv = rt1711_i2c_write8(tcpc_dev, RT1711H_REG_IDLE_CTRL,
			RT1711H_REG_IDLE_SET(0, 1, en ? 0 : 1, 2));
		if (rv < 0)
			pr_err("disable auto idle mode during lpm fail\n");
	}
...
static int rt1711_tcpcdev_init(struct rt1711_chip *chip, struct device *dev)
{
	...
	of_property_read_u32(np, "usb_esd_case",
		&chip->usb_esd_case);
