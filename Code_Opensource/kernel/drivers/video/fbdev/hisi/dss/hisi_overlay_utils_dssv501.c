	...
	for (i = 0; i < pov_h_block->layer_nums; i++) {
		...
		if ((layer->need_cap & CAP_AFBCD) || (layer->need_cap & CAP_HFBCD)) {
			...
			if ((pov_req->ovl_idx == DSS_OVL0) || (pov_req->ovl_idx == DSS_OVL1))
				aif1_bw->axi_sel = (dpufd->use_mmbuf_cnt % 2) ? AXI_CHN0 : AXI_CHN1;
			else
				aif1_bw->axi_sel = (dpufd->use_mmbuf_cnt % 2) ? AXI_CHN1 : AXI_CHN0;
	...
	dss_aif_t *s_aif)
{
	...
	//set_reg(dpufd_primary->dss_base + DSS_VBIF0_AIF + AIF_CH_CTL_CMD, 0x1, 1, 0);
}
	...
	char __iomem *scl_lut_base, int coef_lut_idx)
{
	...
	ret = hisi_dss_arsr1p_write_coefs(dpufd, enable_cmdlist, scl_lut_base + ARSR_POST_COEFF_H_Y0,
		(const int **)COEF_Y_SCL_UP_TAP6, ARSR2P_PHASE_NUM, ARSR2P_TAP6);
	if (ret < 0) {
		DPU_FB_ERR("Error to write H_Y0_COEF coefficients.\n");
	}
	ret = hisi_dss_arsr1p_write_coefs(dpufd, enable_cmdlist, scl_lut_base + ARSR_POST_COEFF_V_Y0,
		(const int **)COEF_Y_SCL_UP_TAP6, ARSR2P_PHASE_NUM, ARSR2P_TAP6);
	if (ret < 0) {
		DPU_FB_ERR("Error to write V_Y0_COEF coefficients.\n");
	}

	ret = hisi_dss_arsr1p_write_coefs(dpufd, enable_cmdlist, scl_lut_base + ARSR_POST_COEFF_H_UV0,
		(const int **)COEF_AUV_SCL_UP_TAP4, ARSR2P_PHASE_NUM, ARSR2P_TAP4);
	if (ret < 0) {
		DPU_FB_ERR("Error to write H_UV0_COEF coefficients.\n");
	}

	ret = hisi_dss_arsr1p_write_coefs(dpufd, enable_cmdlist, scl_lut_base + ARSR_POST_COEFF_V_UV0,
		(const int **)COEF_AUV_SCL_UP_TAP4, ARSR2P_PHASE_NUM, ARSR2P_TAP4);
	if (ret < 0) {
	...
	char __iomem * arsr2p_base, dss_arsr2p_t *s_arsr2p)
{
	...
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_LEVEL, s_arsr2p->arsr2p_effect.sharp_level, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_GAIN_LOW, s_arsr2p->arsr2p_effect.sharp_gain_low, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_GAIN_MID, s_arsr2p->arsr2p_effect.sharp_gain_mid, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_GAIN_HIGH, s_arsr2p->arsr2p_effect.sharp_gain_high, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_GAINCTRLSLOPH_MF, s_arsr2p->arsr2p_effect.sharp_gainctrl_sloph_mf, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_GAINCTRLSLOPL_MF, s_arsr2p->arsr2p_effect.sharp_gainctrl_slopl_mf, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_GAINCTRLSLOPH_HF, s_arsr2p->arsr2p_effect.sharp_gainctrl_sloph_hf, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_GAINCTRLSLOPL_HF, s_arsr2p->arsr2p_effect.sharp_gainctrl_slopl_hf, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_MF_LMT, s_arsr2p->arsr2p_effect.sharp_mf_lmt, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_GAIN_MF, s_arsr2p->arsr2p_effect.sharp_gain_mf, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_MF_B, s_arsr2p->arsr2p_effect.sharp_mf_b, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_HF_LMT, s_arsr2p->arsr2p_effect.sharp_hf_lmt, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_GAIN_HF, s_arsr2p->arsr2p_effect.sharp_gain_hf, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_HF_B, s_arsr2p->arsr2p_effect.sharp_hf_b, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_LF_CTRL, s_arsr2p->arsr2p_effect.sharp_lf_ctrl, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_LF_VAR, s_arsr2p->arsr2p_effect.sharp_lf_var, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_LF_CTRL_SLOP, s_arsr2p->arsr2p_effect.sharp_lf_ctrl_slop, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_HF_SELECT, s_arsr2p->arsr2p_effect.sharp_hf_select, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_CFG2_H, s_arsr2p->arsr2p_effect.sharp_cfg2_h, 32, 0);
	dpufd->set_reg(dpufd, arsr2p_base + ARSR2P_SHARP_CFG2_L, s_arsr2p->arsr2p_effect.sharp_cfg2_l, 32, 0);
...
int hisi_dss_arsr2p_config(struct dpu_fb_data_type *dpufd, dss_layer_t *layer, dss_rect_t *aligned_rect, bool rdma_stretch_enable)
{
	...
	ret = hisi_dss_arsr2p_config_check_width(&dst_rect, src_width, en_hscl, en_vscl, src_height, dpufd->index);
	if (ret <= 0) {
		return ret;
	}
...
static int hisi_dss_check_wblayer_rect(struct dpu_fb_data_type *dpufd, dss_wb_layer_t *wb_layer)
{
	if (dpufd == NULL) {
...
static int hisi_dss_hfbcd_config_check_mm_base0(struct dpu_fb_data_type *dpufd, uint32_t mm_base0_y8,
...
static int hisi_dss_hfbcd_config_check_mm_base2(struct dpu_fb_data_type *dpufd, uint32_t mm_base2_y2,
...
int hisi_dss_hfbcd_config(struct dpu_fb_data_type *dpufd, int ovl_idx,
	dss_layer_t *layer, struct hisi_ov_compose_rect *ov_compose_rect,
	struct hisi_ov_compose_flag *ov_compose_flag)
{
	...
	if ((layer->img.hfbc_mmbuf_base0_y8 > 0) && (layer->img.hfbc_mmbuf_base1_c8 > 0)) {
		...
		ret = hisi_dss_mmbuf_config(dpufd, ovl_idx, layer, hfbcd_block_type, is_pixel_10bit);
		if (ret < 0) {
			DPU_FB_ERR("hisi_dss_mmbuf_config fail!\n");
			return -EINVAL;
		}
	...
	ret = hisi_dss_hfbcd_config_check_mm_base0(dpufd, mm_base0_y8, mm_base1_c8, chn_idx, layer->layer_idx);
	if (ret == -EINVAL) {
		return ret;
	}
	...
	if (is_pixel_10bit) {
		...
		ret = hisi_dss_hfbcd_config_check_mm_base2(dpufd, mm_base2_y2, mm_base3_c2, chn_idx, layer->layer_idx);
		if (ret == -EINVAL) {
			return ret;
		}
	}
	...
	aligned_rect.right = ALIGN_UP(new_src_rect.x + new_src_rect.w, hfbcd_block_width_align) - 1; //lint !e502
	aligned_rect.top = ALIGN_DOWN(new_src_rect.y, hfbcd_block_height_align);
	aligned_rect.bottom = ALIGN_UP(new_src_rect.y + new_src_rect.h, hfbcd_block_height_align) - 1; //lint !e502
	...
	if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
		...
		aligned_rect.right = ALIGN_UP(new_src_rect.y + new_src_rect.h, hfbcd_block_height_align) - 1; //lint !e502
		aligned_rect.top = ALIGN_DOWN(new_src_rect.x, hfbcd_block_width_align);
		aligned_rect.bottom = ALIGN_UP(new_src_rect.x + new_src_rect.w, hfbcd_block_width_align) - 1; //lint !e502
	}
	...
	if (hisi_adjust_clip_rect(layer, ov_compose_rect->clip_rect) < 0) {
...
int hisi_dss_hfbce_config(struct dpu_fb_data_type *dpufd, dss_wb_layer_t *layer,
	dss_rect_t aligned_rect, dss_rect_t *ov_block_rect, bool last_block)
{
	...
	if (wdma_format < 0) {
		DPU_FB_ERR("hisi_pixel_format_hal2dma failed!\n");
		return -EINVAL;
	}
	...
	if (wdma_transform < 0) {
		DPU_FB_ERR("hisi_transform_hal2dma failed!\n");
		return -EINVAL;
	}
	...
	char __iomem *post_scf_base, dss_arsr1p_t *s_post_scf)
{
	...
	dpufd->set_reg(dpufd, post_scf_base + ARSR_POST_SHARP_GAINCTRLSLOPH_MF, s_post_scf->sharp_gainctrl_sloph_mf, 32, 0);
	dpufd->set_reg(dpufd, post_scf_base + ARSR_POST_SHARP_GAINCTRLSLOPL_MF, s_post_scf->sharp_gainctrl_slopl_mf, 32, 0);
	dpufd->set_reg(dpufd, post_scf_base + ARSR_POST_SHARP_GAINCTRLSLOPH_HF, s_post_scf->sharp_gainctrl_sloph_hf, 32, 0);
	dpufd->set_reg(dpufd, post_scf_base + ARSR_POST_SHARP_GAINCTRLSLOPL_HF, s_post_scf->sharp_gainctrl_slopl_hf, 32, 0);
