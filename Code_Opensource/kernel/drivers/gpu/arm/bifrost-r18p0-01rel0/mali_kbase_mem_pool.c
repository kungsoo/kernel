...
static size_t kbase_mem_pool_capacity(struct kbase_mem_pool *pool)
{
	...
	return max(max_size - cur_size, (ssize_t)0); /* [false alarm]: thirdparty code */
}
...
#endif
#if !defined(CONFIG_MALI_2MB_ALLOC) && defined(CONFIG_SPLIT_SHRINKER)
	if (order == KBASE_MEM_POOL_2MB_PAGE_TABLE_ORDER)
		pool->reclaim.flags |=  SHRINKER_SKIP;
#endif
