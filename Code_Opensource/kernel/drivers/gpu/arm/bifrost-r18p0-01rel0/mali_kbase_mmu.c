	...
	if (grown) {
		...
		if (lb_pages_to_grow > 0) {
			...
			if (lb_grow_2mb_pool) {
				...
				ret = kbase_mem_pool_grow(lp_mem_pool, lb_pages_to_grow, kctx->task);
			} else {
#endif
				struct kbase_mem_pool *const mem_pool =
					&kctx->mem_pools.small[group_id];

				ret = kbase_mem_pool_grow(mem_pool, lb_pages_to_grow, kctx->task);
#ifdef CONFIG_MALI_2MB_ALLOC
			}
#endif
		}
	...
	int const level, int const group_id)
{
	...
	kbdev->mmu_mode->entry_set_ate(kbdev, &entry, phy, flags, level);
	return entry;
}
	...
	while (remain) {
		...
		if (cur_level == MIDGARD_MMU_LEVEL(2)) {
			...
			pgd_page[level_index] =
				kbase_mmu_create_ate(kbdev, *phys, flags, cur_level, 0);
		} else {
			for (i = 0; i < count; i++) {
				...
				if (!kbase_is_entry_decompressed(phys[i])) {
					kbdev->mmu_mode->entries_invalidate(target, 1);
				} else
#endif
