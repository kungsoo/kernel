...
static struct hisee_driver_function g_hisee_lpm3_function_list[] = {
	...
	{ "hisee_allow_down", hisee_allow_down_func},
#endif
	{ NULL, NULL },
};
...
#ifdef CONFIG_GENERAL_SEE_PINCODE
	const uint8_t *time = " 60000 1"; /* 60000 means 60s, 1 means vote id. */
#endif
...
#endif
#if !defined(CONFIG_GENERAL_SEE_PINCODE_DISABLE) && defined(CONFIG_GENERAL_SEE_PINCODE_POWER)
	ret = hisee_pincode_vote(GENERAL_SEE_PINCODE_VOTE_ON);
	if (ret != HISEE_OK)
		pr_err("%s:pincode vote failed:%x\n", __func__, ret);
#endif
