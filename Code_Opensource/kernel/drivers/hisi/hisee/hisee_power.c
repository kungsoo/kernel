...
s32 hisee_allow_down_func(const void *buf, s32 para)
{
	...
	return hisee_pincode_vote(GENERAL_SEE_PINCODE_VOTE_OFF);
}
...
s32 hisee_pincode_vote(u32 op_type)
{
	...
	if (op_type != GENERAL_SEE_PINCODE_VOTE_ON && op_type != GENERAL_SEE_PINCODE_VOTE_OFF) {
		pr_err("%s(): Invalid op_type=%d\n", __func__, op_type);
		return set_errno_then_exit(ret);
	}
	...
	ret = atfd_hisee_smc((u64)HISEE_FN_MAIN_SERVICE_CMD,
			     GENERAL_SEE_PINCODE_VOTE, (u64)op_type, 0);
	if (ret != HISEE_OK)
		pr_err("%s(): op_type=%d to atf failed, ret=%d\n",
		       __func__, op_type, ret);
...
#ifdef CONFIG_GENERAL_SEE_PINCODE_POWER
	if (hisee_pincode_vote(GENERAL_SEE_PINCODE_VOTE_OFF) != HISEE_OK)
		pr_err("%s:pincode vote failed!\n", __func__);
#endif
	return HISEE_OK;
}
