...
int slam_eof_handler(msg_req_slam_request_t *slam_request)
{
	...
	if (g_slam_stat_addr == 0) {
		pr_err("[%s] failed : g_slam_stat_addr  is invalid", __func__);
		return -CPE_FW_ERR;
	}
...
static int slam_request_dump(msg_req_slam_request_t *req)
{
	...
	pr_info("coeff_gauss = 0x%x,0x%x,0x%x,0x%x\n", req->gsblur_coef_cfg.coeff_gauss_0, req->gsblur_coef_cfg.coeff_gauss_1,
			req->gsblur_coef_cfg.coeff_gauss_2, req->gsblur_coef_cfg.coeff_gauss_3);
