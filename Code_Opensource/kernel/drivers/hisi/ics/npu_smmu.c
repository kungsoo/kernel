...
static int npu_enable_iommu(struct device *dev)
{
	...
	NPU_DEBUG("ttbr phys_addr is %lx", ttbr);
	return 0;
}
...
void npu_mem_mngr_exit(void) {
	...
	list_for_each_safe(pos, n, &memory_manager.head) {
		...
		if (memcpy_s(&map, sizeof(struct map_data), &memory_node->map, sizeof(memory_node->map)) != EOK) {
			NPU_ERR("memcpy_s failed\n");
			mutex_unlock(&npu_mem_mngr_mutex);
			return;
		}
...
static void npu_mem_mngr_add(struct map_data *map) {
	...
	if (memset_s(node, sizeof(*node), 0, sizeof(*node)) != EOK) {
		NPU_ERR("memset_s failed\n");
	}
	if (memcpy_s(&node->map, sizeof(node->map), map, sizeof(*map)) != EOK) {
		NPU_ERR("memcpy_s failed\n");
		kfree(node);
		node = NULL;
		return;
		...
		if ((inst_addr >= memory_node->map.format.iova_start) &&
			...
			if (((unsigned long)inst_addr + (unsigned long)inst_size) > (memory_node->map.format.iova_start + memory_node->map.format.iova_size)) {
				NPU_ERR("invalid inst_size, addr/size = [0x%pK, 0x%x] not in [0x%pK, 0x%lx]",
					(void *)(uintptr_t)inst_addr, inst_size, (void *)(uintptr_t)memory_node->map.format.iova_start, memory_node->map.format.iova_size);
				ret = -EINVAL;
			}
			mutex_unlock(&npu_mem_mngr_mutex);
			return ret;
...
{
	if (memset_s(bandwidth_manager, sizeof(bandwidth_manager), 0, sizeof(bandwidth_manager)) != EOK) {
		NPU_ERR("memset_s failed\n");
	}
...
static int npu_smmu_get_base_phy(struct device *dev)
{
	...
	smmu_manager.smmu_ttbr0 = hisi_domain_get_ttbr(dev);
	return 0;
}
#else  // LINUX_VERSION_CODE
static int npu_smmu_get_base_phy(struct device *dev)
...
static long npu_smmu_map_va(struct map_data *map)
{
	...
	if (!map) {
		NPU_ERR("map_data is NULL!");
		return -EINVAL;
	}
	format = &(map->format);

	if (!npu_dev) {
		NPU_ERR("npu_dev is NULL!");
		return -EFAULT;
	}

	dma_buf = dma_buf_get(map->share_fd);
	if (IS_ERR_OR_NULL(dma_buf)) {
		NPU_ERR("dma_buf_get fail, share_fd=%d, dma_buf=%pK", map->share_fd, dma_buf);
		return -EFAULT;
	}

	iova_start = hisi_iommu_map_dmabuf(npu_dev, dma_buf, format->prot, &iova_size);
	if (iova_start == 0 || iova_size == 0) {
		NPU_ERR("hisi_iommu_map_dmabuf fail, share_fd=%d, iova_start=%lx, iova_size=%lx", map->share_fd, iova_start, iova_size);
		if (iova_start) {
			ret = hisi_iommu_unmap_dmabuf(npu_dev, dma_buf, iova_start);
			if (ret != 0){
				NPU_ERR("hisi_iommu_unmap_dmabuf fail, share_fd=%d, iova_start=%lx, iova_size=%lx",
...
long npu_smmu_cancel_reset_virt(struct map_data *map)
{
	if (!map) {
		NPU_ERR("input parameter arg is NULL, FATAL arg and ignore\n");
		return -EINVAL;
...
static long npu_smmu_unmap_va(struct map_data *map)
{
	...
	if (!current->files) {
		 ...
		 */
		NPU_ERR("The current abnormal process is \"%s\" (pid=%i, tgid=%i, flags=0x%x, exit_code=%d, exit_signal=%d)",
	...
	format = &(map->format);
	ret = hisi_iommu_unmap_dmabuf(npu_dev, dma_buf, format->iova_start);
	if (ret != 0) {
		NPU_ERR("hisi_iommu_unmap_dmabuf fail, share_fd=%d, iova_start=%lx, iova_size=%lx",
...
int npu_smmu_mngr_init(struct device *dev, unsigned int bandwidth_lmt_strategy)
{
	...
	if (memset_s(&smmu_manager, sizeof(smmu_manager), 0, sizeof(smmu_manager)) != EOK) {
		NPU_ERR("memset_s failed\n");
	}
...
bool npu_smmu_interrupt_handler(unsigned int coreID)
{
	...
	if (0 != reg_smmu_mstr_status) {
		...
		NPU_ERR("Rd_Inst_SID=0x%pK, RdAddr=0x%pK, Wr_Inst_SID=0x%pK, WrAddr=0x%pK\n",
			(void *)(uintptr_t)ioread32((void *)(uintptr_t)SOC_SMMU_MSTR_DBG_0_ADDR(mstr_io_addr)), (void *)(uintptr_t)ioread32((void *)(uintptr_t)SOC_SMMU_MSTR_DBG_1_ADDR(mstr_io_addr)),
			(void *)(uintptr_t)ioread32((void *)(uintptr_t)SOC_SMMU_MSTR_DBG_2_ADDR(mstr_io_addr)), (void *)(uintptr_t)ioread32((void *)(uintptr_t)SOC_SMMU_MSTR_DBG_3_ADDR(mstr_io_addr))); //lint !e571

		NPU_ERR("RW_Burst_len=0x%pK, Awaddr=0x%pK\n",
			(void *)(uintptr_t)ioread32((void *)(uintptr_t)SOC_SMMU_MSTR_DBG_4_ADDR(mstr_io_addr)), (void *)(uintptr_t)ioread32((void *)(uintptr_t)SOC_SMMU_MSTR_DBG_5_ADDR(mstr_io_addr))); //lint !e571
	...
	if (0 != reg_smmu_comm_status) {
		...
		if (reg_smmu_comm_status & SMMU_INTCLR_NS_PERMIS_STAT) {
			ret = true;
			/* When the attributes of the input transaction do not match the attributes descripted in the page table, the mmu will raise a fault for this. */
...
{
	int i;
	void *dbg_port_in = NULL;

	CHECK_COREID_INVALID_RETURN_VOID(coreID);
	dbg_port_in = (void *)SOC_SMMU_MSTR_DBG_PORT_IN_0_ADDR((uintptr_t)smmu_manager.master_io_addr[coreID]);
	if (memset_s(&smmu_manager.smmu_stat[coreID], sizeof(smmu_manager.smmu_stat[coreID]), 0, sizeof(smmu_manager.smmu_stat[coreID])) != EOK) {
		NPU_ERR("memset_s failed\n");
	}
...
{
	uintptr_t mstr_io_addr;
	struct smmu_statistic *statistic = NULL;
...
long npu_smmu_report_statistic(SMMU_STAT_S *statistic)
{
	...
	if (memcpy_s(statistic, sizeof(*statistic), &smmu_manager.smmu_stat[coreID], sizeof(smmu_manager.smmu_stat[coreID])) != EOK) {
		NPU_ERR("memcpy_s failed\n");
	}
	return 0;
}
...
void npu_smmu_dump_strm(unsigned int coreID)
{
	...
	for (i = 0; i < NPU_SMMU_MSTR_DEBUG_BASE_NUM; i++) {
		...
		port_out[24], port_out[25], port_out[26], port_out[27], port_out[28], port_out[29], port_out[30], port_out[31]);
		if (dsm_offset < 0) {
			NPU_ERR("snprintf_s failed.\n");
		}
		#endif /* CONFIG_HUAWEI_DSM */

	}

	if (memset_s(&mstr_reg, sizeof(struct npu_mstr_reg_s), 0, sizeof(struct npu_mstr_reg_s)) != EOK) {
		NPU_ERR("memset_s failed.\n");
	}
