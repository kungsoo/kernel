...
static struct cgroup *cgroup_for_schedtune_top_app;
...
#ifdef CONFIG_HW_FORK_THREAD_CHANGE_CGROUP
void record_cgroup_for_schedtune_top_app(struct cgroup *cgrp, const char *name)
{
	if (cgroup_for_schedtune_top_app == NULL) {
		if (cgrp == NULL)
			return;
		if (cgrp->subsys[schedtune_cgrp_id] == NULL)
			return;
		if (strcmp(name, "top-app") == 0) {
			cgroup_for_schedtune_top_app = cgrp;
			pr_info("[HW_VIP %s]cgroup_for_schedtune_top_app: %p\n",
				__func__, cgroup_for_schedtune_top_app);
		}
	}
}

void clear_cgroup_for_schedtune_top_app(struct cgroup *cgrp)
{
	if (cgrp == cgroup_for_schedtune_top_app)
		cgroup_for_schedtune_top_app = NULL;
}
...
static void top_app_attach_handler(struct work_struct *work)
{
	...
	ret = attach_cgroup_to_task(cgroup_for_schedtune_top_app, task);
...
{
	struct top_app_attach_struct_t *worker_node = NULL;
	...
	worker_node = (struct top_app_attach_struct_t *)
