...
#ifdef CONFIG_PROTECT_LRU_ENHANCED
static unsigned long protect_lru_hot_area __read_mostly = MIN_LOCKED;
static unsigned long protect_refault_distance __read_mostly = ULONG_MAX;
unsigned long vip_fault_around_bytes __read_mostly = 65536;
#endif
#endif
...
#ifdef CONFIG_PROTECT_LRU_ENHANCED
	if (!limit || limit < protect_lru_hot_area)
#else
	if (!limit)
#endif
		return false;
...
#ifdef CONFIG_PROTECT_LRU_ENHANCED
	if (protect_reclaim_ratio &&
		prot_file > protect_lru_hot_area &&
		(prot_file * PERCENT_DENOMINATOR >
		protect_reclaim_ratio * all_file))
#else
	if (!protect_reclaim_ratio ||
	...
	struct mem_cgroup *from, struct mem_cgroup *to, struct page *page)
{
	...
	if (!protect_memcg_move_account(page, 0, from, to)) {
		...
		update_nr_protect_pages(page, -1);
#endif
	}
...
struct ctl_table protect_lru_table[] = {
...
#ifdef CONFIG_PROTECT_LRU_ENHANCED
	{
		.procname	= "protect_lru_hot_area",
		.data		= &protect_lru_hot_area,
		.maxlen		= sizeof(int),
		.mode		= 0640,
	...
	{
		.procname	= "protect_refault_distance",
		.data		= &protect_refault_distance,
	...
	{
		.procname	= "vip_fault_around_bytes",
		.data		= &vip_fault_around_bytes,
		.maxlen		= sizeof(unsigned long),
		.mode		= 0640,
		.proc_handler	= proc_doulongvec_minmax,
		.extra1		= &sysctl_zero,
...
#ifdef CONFIG_PROTECT_LRU_ENHANCED
// g_charge_fail indicates the fail times of "mem_cgroup_charge_nonprotect"
atomic_t g_charge_fail = ATOMIC_INIT(0);

void inc_enhanced_lru_charge_fail(void)
{
	atomic_inc(&g_charge_fail);
}
...
static void file_page_move_memcg_account(struct page *page, struct lruvec *from_vec, struct lruvec *to_vec, int charge_status)
{
	/*
	 * charge_status != 0 means mem_cgroup_charge_nonprotect failed, we only stat memcg but not pgdat,
	 * because delete_from_page_cache will call __mod_lruvec_page_state to stat pgdat when
	 * page->mem_cgroup is NULL.
	 */
	if (unlikely(charge_status) && !mem_cgroup_disabled()) {
		__mod_memcg_lruvec_state(from_vec, NR_FILE_PAGES, -1);
		return;
	}
	__mod_lruvec_state(from_vec, NR_FILE_PAGES, -1);
	__mod_lruvec_state(to_vec, NR_FILE_PAGES, 1);
}

int rejudge_protect_page(struct page *page, struct address_space *mapping,
	gfp_t gfp_mask, void *shadow)
{
	unsigned long distance;
	int ret = 0;
	struct lruvec *from_vec = NULL;
	struct lruvec *to_vec = NULL;
	struct mem_cgroup *memcgp = NULL;

	distance = workingset_refault(page, shadow);
	if (!mapping || !mapping->host || !mapping->host->i_protect) {
		return ret;
	}

	if (distance > protect_refault_distance) {
		from_vec = page_lruvec(page);
		mem_cgroup_uncharge(page);
		protect_add_page_cache_rollback(page);
		ret = mem_cgroup_try_charge(page, current->mm,
			gfp_mask, &memcgp, false);
		if(ret) {
			protect_add_page_cache_rollback(page);
			return ret;
		}
		mem_cgroup_commit_charge(page, memcgp, true, false);
		to_vec = page_lruvec(page);
		file_page_move_memcg_account(page, from_vec, to_vec, ret);
	}
	return ret;
}
#endif
...
#ifdef CONFIG_PROTECT_LRU_DEBUG
static void protect_file_count_show(struct super_block *sb, void *arg)
{
	struct inode *inode = NULL;
	...
	list_for_each_entry(inode, &sb->s_inodes, i_sb_list) {
		...
		if (unlikely(is_valid_protect_level(inode->i_protect))) {
			dentry = d_find_any_alias(inode);
			if (dentry) {
				seq_printf(m, "file %s in_memory pages is %lu, in_protect pages is %d\n", \
						dentry->d_iname, inode->i_mapping->nrpages, \
						atomic_read(&inode->nr_protect_pages));
				dput(dentry);
			}
		}
		iput(toput_inode);
...
static int protect_file_show(struct seq_file *m, void *v)
{
	iterate_supers(protect_file_count_show, (void *)m);
	return 0;
}
#endif
...
#ifdef CONFIG_PROTECT_LRU_ENHANCED
static int enhanced_lru_panic_dump(struct notifier_block *self,
			  unsigned long v, void *p)
{
	pr_err("enhanced_lru_charge_fail=%d\n",
	       atomic_read(&g_charge_fail));
	return 0;
}

static struct notifier_block enhanced_lru_panic_block = {
	.notifier_call = enhanced_lru_panic_dump
};

static int __init register_enhanced_lru_panic_block(void)
{
	atomic_notifier_chain_register(&panic_notifier_list,
				       &enhanced_lru_panic_block);
	return 0;
}
device_initcall(register_enhanced_lru_panic_block);
#endif
#endif
