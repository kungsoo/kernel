...
{
	return  mb_cache_create_sp(bucket_bits, 0);
}
struct mb_cache *mb_cache_create_sp(int bucket_bits, unsigned long flags)
#else
struct mb_cache *mb_cache_create(int bucket_bits)
#endif
{
...
#ifdef CONFIG_SPLIT_SHRINKER
	set_delay_fs_shrinker(&cache->c_shrink, flags);
#endif
